...
void solve() {
    read(int, a, b, c, d);
    auto check = [] (ll a, ll c, ll d, ll b) {
        if (a >= d) return false;
        if ((d - a) * (d - a) >= abs(b * b - c * c) && d - a < b + c) return true;
        if ((d - a) * (d - a) <= abs(b * b - c * c) && d - a > abs(b - c)) return true;
        return false;
    };
    if (check(a, b, c, d) || check(b, c, d, a) || check(c, d, a, b) || check(d, a, b, c)) {
        cout << "yes\n";
    } else {
        cout << "no\n";
    }
