...
void solve() {
    ...
    auto check = [&] (int alice_now, int bob_now, int sub) -> pii {
        int alice_enable = 0, draw_enable = 0;
        if (alice_now < bob_now) {
            if (sub >= 0) alice_enable += 1;
        } else if (alice_now == bob_now) {
            if (sub == 1) alice_enable += 1;
            if (sub == 0) draw_enable += 1;
        } else {
            if (sub == 1) alice_enable += 1;
        }
        return {alice_enable, draw_enable};
    };
    for (int i = n - 1; ~i; --i) {
        for (int j = i + 1; j < n; ++j) {
            if ((j - i) % 2 == 0) continue;
            ...
            int alice_enable = 0, draw_enable = 0;
            auto [a1, d1] = check(a[i], a[i + 1], dp[i + 2][j]);
            auto [a2, d2] = check(a[i], a[j], dp[i + 1][j - 1]);
            if (a1 | a2) {
                if (d1 | d2) draw_enable = 1;
                else alice_enable = 1;
            } else {
                if (d1 | d2) draw_enable = 1;
            }
            auto [a3, d3] = check(a[j], a[i], dp[i + 1][j - 1]);
            auto [a4, d4] = check(a[j], a[j - 1], dp[i][j - 2]);
            if (a3 | a4) {
                if (d3 | d4) draw_enable = 1;
                else alice_enable = 1;
            } else {
                if (d3 | d4) draw_enable = 1;
            }
            if (alice_enable) dp[i][j] = 1;
            else if (draw_enable) dp[i][j] = 0;
            else dp[i][j] = -1;
        }
    }
    // debugvec(dp);
