...
void solve() {
    ...
    read(int, n);
    readvec(int, a, n);
    transform(a.begin(), a.end(), a.begin(), expr(x / 1000, int x));
    vector dp(N, vector<int>(2, -INF));
    dp[0] = { 0, 0 };
    auto work = [&] (int len) -> void {
        vector ndp(N, vector<int>(2, -INF));
        if (len == 1) {
            for (int i = 0; i < N; ++i) {
                for (int j = 0; j < 2; ++j) {
                    int cap = (1 - j) * 10;
                ...
                for (int j = 0; j < 2; ++j) {
                    for (int k = 0; k < 2; ++k) {
                        for (int l = 0; l <= len - 2; ++l) {
                            int cap = (len - j - k - l) * 10;
                            int use = min(i + j + k + l, cap);
                            int rem = i + j + k + l - use;
                            if (rem < N) {
                                chmax(ndp[rem][k], dp[i][j] + use);
                            }
                        }
                    }
                }
    ...
    // debug(a);
    cout << accumulate(a.begin(), a.end(), 0) * 1000 - res * 100 << '\n';
}
