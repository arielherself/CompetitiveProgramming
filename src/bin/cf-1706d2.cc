...
void solve() {
    ...
    vector<int> curr(a.begin(), a.end());
    for (int i = 0; i < n; ++i) {
        int nxt = (a[i] + curr[i]) / curr[i];
        if (nxt <= k) {
            q.emplace(a[i] / nxt, i);
        }
    }
    ...
    for (int i = mn; ~i; --i) {
        // debugvec(q);
        while (q.size() and q.rbegin()->first >= i) {
            auto [val, idx] = *q.rbegin();
            q.erase(prev(q.end()));
            st.erase(st.lower_bound(curr[idx]));
            curr[idx] = val;
            st.emplace(val);
            if (val) {
                int nxt = (a[idx] + val) / val;
                if (nxt <= k) {
                    q.emplace(a[idx] / nxt, idx);
                }
            }
        }
        // debugvec(q);
        // deb(i, res);
        chmin(res, *st.rbegin() - i);
    }
