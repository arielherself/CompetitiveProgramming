...
void solve() {
    ...
    if (mark[root]) {
        q.emplace(1, 0);
        bk.emplace_back(vector{ root });
    }
    for (auto&& v : ch[root]) {
        vector<int> curr;
        static auto dfs = [&] (auto dfs, int v, int pa) -> void {
            if (mark[v]) {
                curr.emplace_back(v);
            }
            for (auto&& u : ch[v]) {
        ...
        dfs(dfs, v, root);
        if (curr.size()) {
            q.emplace(curr.size(), bk.size());
            bk.emplace_back(std::move(curr));
        }
    }
    ...
    while (q.size()) {
        int i = poptop(q).second;
        int u = popback(bk[i]);
        int j = poptop(q).second;
        int v = popback(bk[j]);
        if (bk[i].size()) {
            q.emplace(bk[i].size(), i);
        }
        if (bk[j].size()) {
            q.emplace(bk[j].size(), j);
        }
        cout << u << ' ' << v << ' ' << root << '\n';
    }
}
