...
void solve() {
    constexpr int N = 2e5;
    read(int, n, m, t);
    vector<vector<pii>> bk(m);
    segtree<Info, Tag> tr(N);
    vector<pii> pts(t + 2);
    for (int i = 1; i <= t; ++i) {
        read(int, x, y);
        --x, --y;
        bk[y].emplace_back(x, i);
        pts[i] = { x, y };
    }
    bk[0].emplace_back(0, 0);
    pts[0] = { 0, 0 };
    bk[m - 1].emplace_back(n - 1, t + 1);
    pts[t + 1] = { n - 1, m - 1 };
    vector<int> prev(t + 2);
    for (int i = 0; i < m; ++i) {
        sort(bk[i].begin(), bk[i].end());
        for (auto&& [j, idx] : bk[i]) {
            auto [val, pos] = tr.query(j);
            ++val;
            prev[idx] = pos;
            tr.range_apply(j, N, {
                .val = val,
    ...
    cout << tr.query(n).val - 2 << '\n';
    auto get = [&] (int i, int j) {
        return string(pts[j].first - pts[i].first, 'D') + string(pts[j].second - pts[i].second, 'R');
    };
    int ptr = t + 1;
    vector<string> seq;
    while (prev[ptr] != ptr) {
        seq.emplace_back(get(prev[ptr], ptr));
        ptr = prev[ptr];
    }
