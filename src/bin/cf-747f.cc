...
void solve() {
    ...
    auto calc = [&] (int n, const array<int, 16>& rem) -> ll {
        vector<mll> dp(n + 1);
        dp[0] = 1;
        for (int i = 0; i < 16; ++i) {
            vector<mll> ndp(n + 1);
            for (int j = 0; j <= n; ++j) {
                for (int k = 0; k <= rem[i] and j + k <= n; ++k) {
                    ndp[j + k] += dp[j] / fact[k];
                }
            }
            dp = std::move(ndp);
        }
        return (dp[n] * fact[n]).val;
    };

    auto work = [&] (ll val) -> ll {
        vector<int> seq;
        while (val) {
            seq.emplace_back(val % 16);
            val /= 16;
        }
        int m = seq.size();
