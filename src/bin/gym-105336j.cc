...
void solve() {
    ...
    readvec(ll, a, n);
    readvec(ll, b, n);
    ll x = accumulate(a.begin(), a.end(), 0, expr(p ^ q, int p, int q));
    ll y = accumulate(b.begin(), b.end(), 0, expr(p ^ q, int p, int q));
    // deb(x, y);
    vector<ll> cand;
    for (int i = 0; i < n; ++i) {
    ...
    int sgn = 0;
    for (int i = 30; ~i; --i)  {
        while (ptr < m and cand[ptr] >= (ll(1) << i + 1)) {
            ++ptr;
        }
        if (ptr >= m) break;
        int left = x >> i & 1;
        int right = y >> i & 1;
        int curr = cand[ptr] >> i & 1;
        if (curr) {
            // deb(i);
            if (left and right) {
                // deb(ptr, cand[ptr]);
