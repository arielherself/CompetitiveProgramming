...
void solve() {
	...
	for (int i = 0; i < n; ++i) {
		auto&& [x, y] = a[i];
		ocs.emplace_back(x - y);
		ocs.emplace_back(x - y - d);
		ocs.emplace_back(x - y + d);

		oct.emplace_back(x + y);
		oct.emplace_back(x + y - d);
		oct.emplace_back(x + y + d);
	}

	sort(ocs.begin(), ocs.end());
	int ms = unique(ocs.begin(), ocs.end()) - ocs.begin();
	ocs.resize(ms);
	auto gets = [&] (int x) { return lower_bound(ocs.begin(), ocs.end(), x) - ocs.begin(); };

	sort(oct.begin(), oct.end());
	int mt = unique(oct.begin(), oct.end()) - oct.begin();
	oct.resize(mt);
	auto gett = [&] (int x) { return lower_bound(oct.begin(), oct.end(), x) - oct.begin(); };

	vector<vector<tiii>> ps(ms), pt(mt);
	...
	for (int i = 0; i < n; ++i) {
		...
		if (mp.count({x - d / 2, y - d / 2})) {
			ps[gets(x - y)].emplace_back(x, i, mp[{x - d / 2, y - d / 2}]);
		}

		if (mp.count({x - d / 2, y + d / 2})) {
			pt[gett(x + y)].emplace_back(x, i, mp[{x - d / 2, y + d / 2}]);
		}
	}
	...
	for (int i = 0; i < n; ++i) {
		auto&& [x, y] = a[i];
		array<int, 4> c = {
			mp.count({x - d / 2, y - d / 2}) ? mp[{x - d / 2, y - d / 2}] : -1,
			mp.count({x - d / 2, y + d / 2}) ? mp[{x - d / 2, y + d / 2}] : -1,
			mp.count({x + d / 2, y + d / 2}) ? mp[{x + d / 2, y + d / 2}] : -1,
			mp.count({x + d / 2, y - d / 2}) ? mp[{x + d / 2, y - d / 2}] : -1,
		};
		...
		{
			auto idx = gets(x - y - d);
			auto it = lower_bound(ps[idx].begin(), ps[idx].end(), tiii(x + 1, 0, 0));
			if (it != ps[idx].begin()) {
				auto [_, i1, i2] = *prev(it);
				if (a[i2].first >= x - d) {
					cout << i + 1 << ' ' << i1 + 1 << ' ' << i2 + 1 << '\n';
					return;
				}
		...
		{
			auto idx = gets(x - y + d);
			auto it = lower_bound(ps[idx].begin(), ps[idx].end(), tiii(x + d + 1, 0, 0));
			if (it != ps[idx].begin()) {
				auto [_, i1, i2] = *prev(it);
				if (a[i2].first >= x) {
					cout << i + 1 << ' ' << i1 + 1 << ' ' << i2 + 1 << '\n';
		...
		{
			auto idx = gett(x + y - d);
			auto it = lower_bound(pt[idx].begin(), pt[idx].end(), tiii(x + 1, 0, 0));
			if (it != pt[idx].begin()) {
				auto [_, i1, i2] = *prev(it);
				if (a[i2].first >= x - d) {
					cout << i + 1 << ' ' << i1 + 1 << ' ' << i2 + 1 << '\n';
		...
		{
			auto idx = gett(x + y + d);
			auto it = lower_bound(pt[idx].begin(), pt[idx].end(), tiii(x + d + 1, 0, 0));
			if (it != pt[idx].begin()) {
				auto [_, i1, i2] = *prev(it);
				if (a[i2].first >= x) {
					cout << i + 1 << ' ' << i1 + 1 << ' ' << i2 + 1 << '\n';
