...
void solve() {
	...
	min_heap<ll> qa, qb;
	for (int k = 1; ; ++k) {
		int amax = n - k, bmax = m - k;
		if (amax < 0 or bmax < 0) break;
		while (qa.size() > amax) {
			curr -= poptop(qa);
		}
		while (qb.size() > bmax) {
		...
		while (qa.size() + qb.size() < k) {
			if (i < pa.size() and qa.size() < amax and j < pb.size() and qb.size() < bmax) {
				if (pa[i] > pb[j]) {
					qa.emplace(pa[i]);
					curr += pa[i];
					i += 1;
			...
			} else if (i < pa.size() and qa.size() < amax) {
				qa.emplace(pa[i]);
				curr += pa[i];
				i += 1;
			} else if (j < pb.size() and qb.size() < bmax) {
				qb.emplace(pb[j]);
				curr += pb[j];
				j += 1;
			} else {
