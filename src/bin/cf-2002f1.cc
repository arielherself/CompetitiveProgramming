...
void solve() {
	...
	read(int, q);
	vector<tiii> queries(N);
	for (int i = 0; i < q; ++i) {
		read(int, n, m, l, f);
		queries[n] = { max(l, f), min(l, f), i };
	}
	...
	int ptr = 2;
	for (int i = 1; i < N; ++i) {
		while (i >= stack.back() + minp[stack.back()]) {
			stack.pop_back();
		}
		int curr = stack.back();
		if (i == 518) {
		...
		dp[i] = curr;
		{
			...
			while (x > 1) {
				int p = minp[x];
				while (x % p == 0) x /= p;
				chmin(better, ll(curr + p - 1) / p * p - 1);
			}
			chmax(dp[i], better);
		}
		while (ptr <= dp[i]) {
		...
		point v(i, dp[i]);
		while (k >= 2 and (hull[k - 1] - hull[k - 2]) * (v - hull[k - 1]) >= 0) {
			k -= 1;
			hull.pop_back();
		}
		k += 1;
		...
		if (get<0>(queries[i]) != 0) {
			auto [l, f, idx] = queries[i];
			point unit(f, -l);
			{
				int left = 0, right = k - 1;
				while (left < right) {
					int mid = left + right >> 1;
					if (mid != k - 1 and unit * (hull[mid + 1] - hull[mid]) > 0) {
						left = mid + 1;
					} else {
						right = mid;
					}
				}
				res[idx] = l * hull[left].x + f * hull[left].y;
				deb(idx, hull[left]);
			}
		}
	}
