...
void solve() {
    read(int, n, m);
    unordered_map<string, int> scorea, scoreb;
    int N = 0;
    for (int i = 0; i < n; ++i) {
        read(string, s);
        if (scorea.count(s)) continue;
        scorea[s] = ++N;
    }
    N = 0;
    for (int i = 0; i < m; ++i) {
        read(string, s);
        if (scoreb.count(s)) continue;
        scoreb[s] = ++N;
    }
    vector<tuple<int, int, string>> a;
    for (auto&& [name, score] : scorea) {
        a.emplace_back(score, 1, name);
    }
    for (auto&& [name, score] : scoreb) {
        a.emplace_back(score, 2, name);
    }
    sort(a.begin(), a.end());
    unordered_set<string> st;
    for (auto&& [score, group, name] : a) {
