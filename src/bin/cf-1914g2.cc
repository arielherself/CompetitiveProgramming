...
void solve() {
	...
	for (int i = 0; i < 2 * n; ++i) {
		if (other[i] < i) {
			// right
			while (q.size() and not qu.connected(i, q.back())) {
				int j = popback(q);
				cnt[qu.query(j)] += cnt[qu.query(i)];
				qu.merge(i, j);
			}
			assert(q.size() and qu.connected(i, q.back()));
			if (--cnt[qu.query(i)] == 0) {
				q.pop_back();
			}
		} else {
	...
	vector<int> l(2 * n, INF), r(2 * n, -INF);
	for (int i = 0; i < 2 * n; ++i) {
		chmin(l[qu.query(i)], i);
		chmax(r[qu.query(i)], i);
	}

	vector<int> left(2 * n), right(2 * n);
	for (int i = 0; i < 2 * n; ++i) {
		left[i] = l[qu.query(i)];
		right[i] = r[qu.query(i)];
	}
	// debug(left);
	// debug(right);
