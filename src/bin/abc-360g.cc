...
void solve() {
    ...
    vector<int> dp(n);
    for (int i = 0; i < n; ++i) {
        dp[i] = tr.range_query(0, mp[a[i]] - 1).val + 1;
        tr.set(mp[a[i]], { max(dp[i], tr.query(mp[a[i]]).val) });
    }
    for (int i = 0; i <= N; ++i) {
        tr.set(i, {});
    }
    ...
    for (int i = n; ~i; --i) {
        if (i < n) {
            int curr = 1;
            if (mp[a[i]] < N) {
                curr += tr.range_query(mp[a[i]] + 1, N).val;
            }
            tr.set(mp[a[i]], { max(curr, tr.query(mp[a[i]]).val) });
        }
        if (i - 1 >= 0) {
            auto it = i - 2 >= 0 ? mp.lower_bound(a[i - 2] + 2) : mp.begin();
            int right = 0;
            if (it != mp.end()) {
                right = tr.range_query(it->second, N).val;
            }
            int left = i - 2 >= 0 ? dp[i - 2] : 0;
            chmax(res, left + 1 + right);
        }
    }
    cout << res << '\n';
}
