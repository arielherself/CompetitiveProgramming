...
void solve() {
	...
	sparse_table<ll> rmq(c.rbegin(), c.rend(), functor(max));
	...
	for (int i = 0; i < n; ++i) {
		...
		while (ptr != 0) {
			int nextidx = ({
				...
				while (l < r) {
					int mid = l + r >> 1;
					if (mid == idx or right >= a[stack[mid + 1]] + ps[stack[mid + 1] + 1]) {
						r = mid;
					} else {
						l = mid + 1;
					}
			...
			int newptr = nextidx == -1 ? 0 : stack[nextidx] + 1;
			left += ps[ptr] - ps[newptr];
			ptr = newptr;
			idx = nextidx;
			...
			ll need = a[stack[idx]] + ps[stack[idx] + 1] - right;
			int newrptr = ({
				int l = rptr, r = n;
				while (l < r) {
					int mid = l + r >> 1;
					if (ps[mid] - ps[rptr] >= need) {
						r = mid;
					} else {
						l = mid + 1;
					}
			...
			if (ps[newrptr] - ps[rptr] < need or left < rmq.query(rptr, newrptr - 1)) {
				// impossible
				break;
			}

			right += ps[newrptr] - ps[rptr];
			rptr = newrptr;
		}
		...
		if (ptr == 0) {
			...
			int R = ({
				...
				while (l < r) {
					int mid = l + r + 1 >> 1;
					if (mid == rptr or left >= rmq.query(rptr, mid - 1)) {
						l = mid;
					} else {
						r = mid - 1;
					}
		...
		while (stack.size() and a[i] + ps[i + 1] >= a[stack.back()] + ps[stack.back() + 1]) {
			stack.pop_back();
		}
