...
void solve() {
	...
	int m;
	{
		...
		for (int i = 1; i <= n; ++i) {
			if (a[i - 1] == 1) {
				h ^= mapping[i - 1];
				stack.emplace_back(mapping[i - 1]);
			} else if (stack.size()) {
				h ^= popback(stack);
			} else {
				h ^= mapping[i - 1];
		...
		m = unique(oc.begin(), oc.end()) - oc.begin();
		oc.resize(m);
		auto get = [&] (ull x) { return lower_bound(oc.begin(), oc.end(), x) - oc.begin(); };
	...
	auto res = mo(solver, queries, n + 1);
