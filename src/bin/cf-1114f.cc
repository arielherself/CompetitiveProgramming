...
ll qpow(ll b, ll p) {
    ...
    if (p == 0) return 1;
    ll half = qpow(b, p / 2);
    if (p % 2 == 1) return (((half * half) % MDL) * b) % MDL;
    else return (half * half) % MDL;
}
...
constexpr int N = 302;
int phi[N + 10], rev[N + 10], minp[N + 10];
vector<int> primes;
...
void solve() {
    ...
    auto factorize = [&] (int x) -> pair<ll, ll> {
    ...
    while (q--) {
        ...
        if (op == "MULTIPLY") {
            read(int, l, r, x);
            --l, --r;
            auto [mask, val] = factorize(x);
            tr.range_apply(l, r, { mask, val });
        } else {
            read(int, l, r);
            --l, --r;
            auto [mask, val] = tr.range_query(l, r);
            for (int j = 0; j < M; ++j) {
                if (mask >> j & 1) {
                    val *= ((primes[j] - 1) * inverse(primes[j], MDL)) % MDL;
                    val %= MDL;
                }
            }
            cout << val << '\n';
