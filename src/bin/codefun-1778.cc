...
void solve() {
    read(int, n, m);
    vector<vector<pil>> e(2 * n + 1);
    for (int i = 1; i <= n; ++i) {
        read(int, x);
        e[2 * i - 1].emplace_back(2 * i, x);
        e[2 * i].emplace_back(2 * i - 1, x);
    }
    while (m--) {
        read(int, u, v, w, t);
        e[2 * u - t + 1].emplace_back(2 * v - t + 1, w);
        e[2 * v - t + 1].emplace_back(2 * u - t + 1, w);
    }
    vector<ll> dis(2 * n + 1, INFLL);
    vector<bool> vis(2 * n + 1);
    dis[1] = 0, dis[2] = 0;
    priority_queue<pli, vector<pli>, greater<>> pq;
    pq.emplace(0, 1), pq.emplace(0, 2);
    while (pq.size()) {
        poptop(pq, d, v);
        continue_or(vis[v], 1);
        for (auto&& [u, w] : e[v]) {
