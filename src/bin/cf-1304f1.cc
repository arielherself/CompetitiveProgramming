...
void solve() {
    ...
    vector<int> dp(m - k + 1);
    for (int j = 0; j <= m - k; ++j) {
        dp[j] = ps[0][j + k] - ps[0][j];
    }
    for (int i = 1; i < n; ++i) {
        vector<int> curr(m - k + 1);
        for (int j = 0; j <= m - k; ++j) {
            for (int l = 0; l < k; ++l) {
                if (j + l - k + 1 < 0) continue;
                chmax(curr[j], dp[j + l - k + 1] + ps[i][j + k] - ps[i][j + l - k + 1]);
            }
            for (int l = 0; l < k; ++l) {
                if (j + l > m - k) continue;
                chmax(curr[j], dp[j + l] + ps[i][j + l + k] - ps[i][j]);
            }
        }
        vector<int> pp(m - k + 2), ss(m - k + 2);
        for (int j = 1; j <= m - k + 1; ++j) {
            pp[j] = max(pp[j - 1], dp[j - 1] + ps[i][j + k - 1] - ps[i][j - 1]);
        }
        for (int j = m - k; ~j; --j) {
            ss[j] = max(ss[j + 1], dp[j] + ps[i][j + k] - ps[i][j]);
        }
        for (int j = 0; j <= m - k; ++j) {
            chmax(curr[j], ps[i][j + k] - ps[i][j] + max(pp[max(0, j - k + 1)], ss[min(m - k + 1, j + k)]));
        }
        swap(curr, dp);
    }
