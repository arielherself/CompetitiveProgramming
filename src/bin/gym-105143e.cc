...
void solve() {
    ...
    auto dfs = [&] (auto dfs, int v, int pa, int depth) -> void {
        chmax(max_layer, depth);
        layers[depth].emplace_back(v);
        fa[v] = pa;
        for (auto&& u : ch[v]) {
            if (u == pa) continue;
            dfs(dfs, u, v, depth + 1);
    ...
    for (int i = 0; i <= max_layer; ++i) {
        ...
        for (auto&& v : l) {
            d[v] = max(d[fa[v]] + 1, (lca.depth[v] - lca.depth[lp]) * 2);
            chmin(dl[i], (d[v] + 1) / 2);
        }
    }
    vector<int> res(n + 1);
    for (int i = 1; i <= n; ++i) {
        ...
        while (l < r) {
            int mid = l + r >> 1;
            if (ll(1) * i * mid >= dl[min(max_layer, mid + t)]) {
                r = mid;
            } else {
                l = mid + 1;
            }
