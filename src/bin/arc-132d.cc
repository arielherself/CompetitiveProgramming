...
void solve() {
    ...
    auto work = [] (vector<int> a, vector<int> b, bool link = false) -> pii {
        // debug(a), debug(b);
        int n = a.size();
        ...
        assert(posa.size() == posb.size());

        int prev = link ? -1 : -INF;
        for (int i = 0; i < n; ++i) {
            if (a[i] == 0) {
                if (posb[reva[i]] <= prev + 1) {
                    swap(a[i], a[prev + 1]);
                    prev += 1;
                } else {
                    prev = i;
    ...
    int prev_start = 0;
    for (int i = 0; i <= n; ++i) {
        if (i == n or dir[i] != dir[prev_start]) {
            array<int, 4> curr = { -INF, -INF, -INF, -INF };
            int left = prev_start, right = i - 1;
            if (dir[left] == 0) {
                reverse(a.begin() + left, a.begin() + right + 1);
                reverse(b.begin() + left, b.begin() + right + 1);
                for (int j = 0; j < 2; ++j) {
                    auto [mask, val] = work(vector(a.begin() + left, a.begin() + right + 1), vector(b.begin() + left, b.begin() + right + 1), j);
                    chmax(curr[(mask & 1) << 1 | mask >> 1], val);
                }
                reverse(a.begin() + left, a.begin() + right + 1);
                reverse(b.begin() + left, b.begin() + right + 1);
            } else {
            ...
            segs.emplace_back(curr);
            prev_start = i;
        }
    }

    m = segs.size();
    vector<int> dp = { max(segs[0][0], segs[0][2]), max(segs[0][1], segs[0][3]) };
    for (int i = 1; i < m; ++i) {
        ...
        for (int j = 0; j < 4; ++j) {
            ...
            int right = j & 1;
            for (int k = 0; k < 2; ++k) {
                chmax(ndp[right], dp[k] + segs[i][j] + (left == k));
            }
        }
        dp = std::move(ndp);
    }
