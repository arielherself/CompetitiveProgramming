...
void solve() {
    ...
    for (auto&& u : ch[root]) {
        curr = {};
        dfs2(dfs2, u, root);
        bk.emplace_back(curr);
    }
    set<pii, greater<>> st;
    int m = bk.size();
    for (int i = 0; i < m; ++i) {
        st.emplace(bk[i].size(), i);
    }
    vector<pii> res;
    while (st.size() > 1) {
        ...
        st.erase(st.begin());
        res.emplace_back(bk[i].back(), bk[j].back());
        bk[i].pop_back(), bk[j].pop_back();
        if (bk[i].size()) st.emplace(bk[i].size(), i);
        if (bk[j].size()) st.emplace(bk[j].size(), j);
    }
    if (st.size()) {
        res.emplace_back(root, bk[st.begin()->second].back());
    }
    for (auto&& [u, v] : res) {
        cout << u << ' ' << v << '\n';
    }
