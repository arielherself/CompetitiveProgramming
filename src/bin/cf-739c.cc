...
struct Info {
    ...
    void apply(const Tag& rhs, size_t len) {
        left += rhs.left;
        right += rhs.right;
        val += rhs.left + rhs.right;
        tot += rhs.tot * len;
    }
};

Info operator+(const Info &a, const Info &b) {
    return a.val > b.val ? Info {
        .left = a.left,
        .right = a.right,
        .val = a.val,
        .tot = a.tot + b.tot,
...
void solve() {
    int n = fastread();
    for (int i = 0; i < n; ++i) a[i] = fastread();

    segtree<Info, Tag> tr(n);

    set<int> notleft, notright;
    for (int i = 0; i < n - 1; ++i) {
        if (a[i] >= a[i + 1]) {
            notleft.emplace(i);
        }
    ...
    for (int i = 0; i < n; ++i) {
        ...
        {
            auto it = notright.lower_bound(i);
            if (it == notright.end()) {
                right = n;
            } else {
                right = *it + 1;
        ...
        // deb(i, left, right);
        // deb(i, tr.query(i).left, tr.query(i).right, tr.query(i).val);
    }

    int q = fastread();
    while (q--) {
        int l = fastread(), r = fastread(), d = fastread();
        --l, --r;

        auto [old_r1l, old_r1r, _3, old_r1] = tr.query(min(n - 1, r + 1));
        auto [old_l1l, old_l1r, _4, old_l1] = tr.query(max(0, l - 1));

        auto [old_rl, old_rr, _1, old_r] = tr.query(r);
        auto [old_ll, old_lr, _2, old_l] = tr.query(l);
...

        {
            ...
            if (l != 0 and l < ll and old_l1 >= old_l and old_l1 < old_l + d) {
                int curr = +old_l1l;
                if (ll > r) left_delta = curr;
                tr.range_apply(l, ll - 1, {
                    .left = curr,
                    .right = 0,
                    .tot = 0,
                });
            ...
            int rr = -1;  // exclusive
            {
                auto it = notleft.lower_bound(r + 1);
                if (it == notleft.end()) {
                    rr = n;
                } else {
                    rr = *it + 1;
            ...
            if (l != 0 and old_l1 >= old_l and old_l1 < old_l + d) {
                notleft.erase(l - 1);
            }
            if (r != n - 1 and old_r < old_r1 and old_r + d >= old_r1) {
                notleft.emplace(r);
            }
...

        {
            ...
            int rl = -1;
            {
                auto it = notright.lower_bound(r);
                if (it != notright.begin()) {
                    rl = max(l - 1, *prev(it));
                } else {
                    rl = l - 1;
                }
            }
            if (r != n - 1 and rl < r and old_r <= old_r1 and old_r + d > old_r1) {
                int curr = +old_r1r;
                if (rl < l) right_delta = curr;
                tr.range_apply(rl + 1, r, {
                    .left = 0,
                    .right = curr,
                    .tot = 0,
                });
            ...
            int ll = -1;  // exclusive
            {
                auto it = notright.lower_bound(l - 1);
                if (it != notright.begin()) {
                    ll = *prev(it);
                } else {
                    ll = -1;
        ...
        // debugvec(notleft);
        // debugvec(notright);
        fastwrite(tr.range_query(0, n - 1).val - 1);
        putchar('\n');
    }
}
