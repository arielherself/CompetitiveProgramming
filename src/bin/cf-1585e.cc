...
void solve() {
    ...
    vector<vector<tiii>> Q(n + 1);
    vector<int> R(q);
    for (int i = 0; i < q; ++i) {
        read(int, v, l, k);
        Q[v].emplace_back(l, k, i);
    }
    auto u_prev = [] (const pii& x) { return make_pair(x.first, x.second - 1); };
    auto u_next = [] (const pii& x) { return make_pair(x.first, x.second + 1); };
    treap<pii, less<pii>, decltype(u_prev), decltype(u_next)> T(less<pii>(), u_prev, u_next);
    vector<int> B(n + 1);
    auto dfs = [&] (auto dfs, int v, int pa) -> void {
        ...
        for (auto&& [l, k, i] : Q[v]) {
            pii lb = T.next_element({ l, 0 });
            if (lb == make_pair(0, 0) or T.index_of(lb) + k - 1 > T.size()) {
                R[i] = -1;
            } else {
                R[i] = T.at(T.index_of(lb) + k - 1).second;
            }
        }
