...
void solve() {
    read(int, n, m);
    vector<vector<pii>> open(n);
    for (int i = 0; i < m; ++i) {
        read(int, l, r, x);
        --l, --r, --x;
        open[l].emplace_back(r, x);
    }
    vector dp(n, vector<mll>(n));
    for (int i = n - 1; ~i; --i) {
        vector<vector<int>> close(n);
        vector<bool> block(n);
        for (int j = i; j < n; ++j) {
            for (auto&& [r, x] : open[j]) {
                close[r].emplace_back(x);
            }
            for (auto&& x : close[j]) {
                block[x] = 1;
            }
            for (int k = i; k <= j; ++k) {
                if (block[k]) continue;
                dp[i][j] += fastcomb(j - i, k - i) * (i <= k - 1 ? dp[i][k - 1] : 1) * (k + 1 <= j ? dp[k + 1][j] : 1);
            }
        }
    }
    cout << dp[0][n - 1] << '\n';
