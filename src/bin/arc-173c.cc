...
void solve() {
    ...
    for (int i = 0; i < n - 1; ++i) {
        tie(mn[i], mx[i]) = minmax(a[i], a[i + 1]);
    }

    sparse_table<int> range_max(mn.rbegin(), mn.rend(), functor(max));
    sparse_table<int> range_min(mx.rbegin(), mx.rend(), functor(min));
    ...
    for (int i = 1; i < n - 1; ++i) {
        if ((a[i - 1] > a[i]) == (a[i + 1] > a[i])) {
            ...
            if (i - 2 >= 0) {
                ...
                while (l < r) {
                    int mid = l + r + 1 >> 1;
                    if (range_max.query(mid, i - 2) > a[i] or range_min.query(mid, i - 2) < a[i]) {
                        l = mid;
                    } else {
                        r = mid - 1;
                    }
                }
                if (range_max.query(l, i - 2) > a[i] or range_min.query(l, i - 2) < a[i]) {
                    left_pos = l;
                }
            }
            if (i + 2 < n) {
                ...
                while (l < r) {
                    int mid = l + r >> 1;
                    if (range_max.query(i + 1, mid) > a[i] or range_min.query(i + 1, mid) < a[i]) {
                        r = mid;
                    } else {
                        l = mid + 1;
                    }
                }
                if (range_max.query(i + 1, l) > a[i] or range_min.query(i + 1, l) < a[i]) {
                    right_pos = l + 1;
                }
            }
            // deb(i, left_pos, right_pos);
            if (left_pos != -1) {
                int curr = i - left_pos + 1;
                if (curr % 2 == 0) {
                    curr += 1;
                }
                chmin(res[i], curr);
    ...
    for (int i = n - 2; ~i; --i) {
        ...
        if (cnt_gt != n - 1 - i - cnt_gt and (n - 1 - i) % 2 == 0) {
            res[n - 1] = n - i;
            break;
        }
    }
