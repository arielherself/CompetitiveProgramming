...
void solve() {
    ...
    vector dp(N, vector(n + 1, pii(INF, -INF)));
    dp[0][0] = { INF - 1, -INF };
    for (int i = 0; i < n; ++i) {
        cnt[a[i]] += 1;
        vector ndp = dp;
        for (int j = 0; j + a[i] < N; ++j) {
            for (int k = 0; k < n; ++k) {
                if (dp[j][k].first < INF) {
                    chmin(ndp[j + a[i]][k + 1].first, min(dp[j][k].first, a[i]));
                    chmax(ndp[j + a[i]][k + 1].second, max(dp[j][k].second, a[i]));
                }
            }
        }
        dp = std::move(ndp);
