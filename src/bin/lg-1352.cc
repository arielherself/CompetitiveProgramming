...
int main() {
    ...
    ll res = 0;
    auto dfs = [&] (auto dfs, int v, int pa) -> upair {
        // return (permissive, restricted)
        ll inclusive = r[v - 1], exclusive = 0;
        for (auto&& u : ch[v]) {
            if (u != pa) {
                auto curr = dfs(dfs, u, v);
                ll cp = u1(curr), cr = u2(curr);
                inclusive += cr;
                exclusive += cp;
            }
        }
        return umake(max(inclusive, exclusive), exclusive);
    };
    cout << u1(dfs(dfs, 1, 0)) << endl;
}