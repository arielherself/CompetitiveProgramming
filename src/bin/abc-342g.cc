...
private:
    ...
    int range_query(size_type s, size_type t, size_type p, size_type l, size_type r) {
        if (l <= s && t <= r) {
            return d[p].history.empty() ? 0 : *d[p].history.rbegin();
        }
        size_type m = s + (t - s >> 1);
        int res = d[p].history.empty() ? 0 : *d[p].history.rbegin();
        if (l <= m) chmax(res, range_query(s, m, p * 2, l, r));
        if (r > m)  chmax(res, range_query(m + 1, t, p * 2 + 1, l, r));
        return res;
    }
public:
    ...
    int range_query(size_type l, size_type r) {
        return range_query({}, _max, 1, l, r);
    }
    int query(size_type i) {
...
struct Tag {
    multiset<int> remove, add;
    void apply(const Tag& rhs) {
        remove.insert(rhs.remove.begin(), rhs.remove.end());
        add.insert(rhs.add.begin(), rhs.add.end());
    }
};
struct Info {
    multiset<int> history;
    void apply(const Tag& rhs, size_t len) {
        for (auto&& x : rhs.remove) {
            history.erase(history.find(x));
        }
        for (auto&& x : rhs.add) {
            history.emplace(x);
        }
...
void solve() {
    ...
    for (int i = 0; i < q; ++i) {
        ...
        if (op == 1) {
            read(int, l, r, x);
            --l, --r;
            tr.range_apply(l, r, { {}, { x } });
            operations[i] = { l, r, x };
        } else if (op == 2) {
            read(int, j);
            --j;
            auto [l, r, x] = operations[j];
            tr.range_apply(l, r, { { x }, {} });
        } else {
            read(int, j);
            --j;
            cout << max(a[j], tr.query(j)) << '\n';
        }
    }
}
