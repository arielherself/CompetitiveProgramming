...
void solve() {
    ...
    read(int, n, m);
    vector<vector<pii>> bk(n + 1);
    vector<unordered_set<int, safe_hash>> oc(n + 1);
    vector<unordered_multiset<int, safe_hash>> edges(n + 1);
    for (int i = 0; i < m; ++i) {
        read(int, u, v, w);
        bk[w].emplace_back(u, v);
        oc[w].emplace(u), oc[w].emplace(v);
        edges[u].emplace(w), edges[v].emplace(w);
    }
    int island_cnt = 0;
    for (int i = 1; i <= n; ++i) {
        if (edges[i].empty()) {
    ...
    for (int i = n; i; --i) {
        int max_deg = 0;
        unordered_set<int, safe_hash> cand;
        for (auto&& v : oc[i]) {
            if (edges[v].count(i) == edges[v].size()) {
                if (max_deg > 1) {
                    if (edges[v].size() > 1) {
                        cout << 0 << '\n';
                        return;
        ...
        if (max_deg > 1) {
            ...
            for (auto&& [u, v] : bk[i]) {
                ...
                edges[u].erase(edges[u].find(i));
                if (u != t and edges[u].empty()) {
                    island_cnt += 1;
                }
                edges[v].erase(edges[v].find(i));
                if (v != t and edges[v].empty()) {
                    island_cnt += 1;
                }
            }
        } else if (max_deg == 1) {
            ...
            int t = *cand.begin();
            for (auto&& [u, v] : bk[i]) {
                int x = cand.count(u), y = cand.count(v);
                if (x + y == 0) {
                    cout << 0 << '\n';
                    return;
                }
