...
WInfo operator+(const WInfo &a, const WInfo &b) {
    return {a.val + b.val, a.cnt + b.cnt, 0};
}
...
void solve() {
    ...
    ll res = 0;
    for (int i = 0; i < n; ++i) {
        res += a[i] * i - tr.query(a[i]).val;
        for (ll j = 0; j < N; j += a[i]) {
            tr.range_apply(j, min(N - 1, j + a[i] - 1), { j });
            auto [sum, cnt, _] = tr2.range_query(j, min(N - 1, j + a[i] - 1));
            res += sum - cnt * j;
        }
        tr2.apply(a[i], { 1 });
        cout << res << " \n"[i + 1 == n];
