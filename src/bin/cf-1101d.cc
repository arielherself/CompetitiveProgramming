...
unordered_set<int, safe_hash> facts[MAXN];
int st[MAXN], all[MAXN], vis[MAXN];
...
void solve() {
    ...
    for (int i = 1; i <= n; ++i) {
        ...
        for (int t = 1; t <= sq; ++t) {
            if (a[i] % t == 0) {
                if (t != 1) facts[i].insert(t);
                facts[i].insert(a[i] / t);
            }
        }
    }
    ...
    auto work = [&] (int f) -> int {
        ...
        auto check_all = [&] (auto check_all, int v, int pa) -> void {
            if (facts[v].count(f)) all[v] = 1;
            for (auto&& u : ch[v]) {
                if (u == pa) continue;
                check_all(check_all, u, v);
            }
        };
        check_all(check_all, 1, 0);
        auto dfs = [&] (auto dfs, int v, int pa, int dis) -> void {
            if (!all[v]) return;
            vis[v] = 1;
            if (dis >= dis_x) dis_x = dis, x = v;
            for (auto&& u : ch[v]) {
                if (u == pa) continue;
                dfs(dfs, u, v, dis + 1);
            }
