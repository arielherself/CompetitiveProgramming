...
void solve() {
	constexpr ll INFLL = 1e16;
	read(int, n, m);
	vector dp0(n + 1, vector<ll>(n + 1, INFLL));
	vector dp1(n + 1, vector<ll>(n + 1, INFLL));
	vector dp2(n + 1, vector<ll>(n + 1, INFLL));
	for (int i = 0; i < m; ++i) {
		read(int, u, v, w, d);
		chmin(dp0[u][v], w);
		if (d >= 1) chmin(dp1[u][v], w);
		if (d >= 2) chmin(dp2[u][v], w);
	}
	for (int i = 1; i <= n; ++i) {
		dp0[i][i] = dp1[i][i] = dp2[i][i] = 0;
	}
	for (int k = 1; k <= n; ++k) {
		for (int i = 1; i <= n; ++i) {
			for (int j = 1; j <= n; ++j) {
				chmin(dp0[i][j], dp0[i][k] + dp0[k][j]);
				chmin(dp1[i][j], dp1[i][k] + dp1[k][j]);
				chmin(dp2[i][j], dp2[i][k] + dp2[k][j]);
			}
		}
	}
	vector ndp(n + 1, vector<ll>(n + 1, INFLL));
	for (int i = 1; i <= n; ++i) {
		ndp[i][i] = 0;
		for (int j = 1; j <= n; ++j) {
			chmin(ndp[i][j], dp1[i][j] * 2 + dp0[j][i]);
			chmin(ndp[i][j], dp2[i][j]);
		}
	}
	for (int k = 1; k <= n; ++k) {
		for (int i = 1; i <= n; ++i) {
