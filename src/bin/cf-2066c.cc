...
void solve() {
	...
	segtree<Info, Tag> tr0(m), tr1(m);
	tr0.set(mp[0], { 1 });
	for (int i = 1; i <= n; ++i) {
		ll eq = tr0.query(mp[i - 1]).val;
		ll neq = tr1.query(mp[i]).val;
		if (0 <= mp[i] - 1) {
			tr0.range_apply(0, mp[i] - 1, {
					.set = { 0 },
					.add = 0,
					});
		}
		...
		// debug(i);
		// cerr << "tr0: ";
		// for (auto&& [x] : tr0.serialize()) {
		// 	cerr << x << ' ';
		// }
		// cerr << endl;
		// cerr << "tr1: ";
		// for (auto&& [x] : tr1.serialize()) {
		// 	cerr << x << ' ';
		// }
		// cerr << endl;
	}
	...
	ll res = 0;
	for (int i = 0; i < m; ++i) {
		(res += (tr0.query(i).val + tr1.query(i).val) % MDL) %= MDL;
	}
