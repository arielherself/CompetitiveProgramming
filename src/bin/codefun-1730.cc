...
void solve() {
    ...
    LCA lca(ch);
    vector<MLL<MDL>> dgl(n + 1);
    vector<int> father(n + 1);
    auto dfs = [&] (auto dfs, int v, int pa, MLL<MDL> acc) -> void {
        father[v] = pa;
        acc *= max(1UL, ch[v].size() - 1);
        dgl[v] = acc;
        for (auto&& u : ch[v]) {
            if (u == pa) continue;
            dfs(dfs, u, v, acc);
    ...
    while (q--) {
        read(int, u, v);
        int p = lca.query(u, v);
        MLL up_exclude = p == 1 ? 1 : dgl[father[p]];
        MLL res = dgl[v] / up_exclude * dgl[u] / up_exclude / max(1UL, ch[v].size() - 1) / max(1UL, ch[u].size() - 1) * ch[u].size() / max(1UL, ch[p].size() - 1);
        // debug(res);
        cout << (( MLL<MDL>(1) ) / res).val << '\n';
    }
}
