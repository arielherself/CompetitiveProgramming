...
Info operator+(const Info &a, const Info &b) {
	return {
		...
		.cnt = a.maxval == b.maxval ? a.cnt + b.cnt : a.maxval > b.maxval ? a.cnt : b.cnt,
	};
}
...
void solve() {
	...
	auto del = [&] (int i) {
		int left = max(i, a[i] - 1);
		tr.range_apply(left, n - 1, { -1 });
	};
	auto add = [&] (int i) {
		int left = max(i, a[i] - 1);
		tr.range_apply(left, n - 1, { 1 });
