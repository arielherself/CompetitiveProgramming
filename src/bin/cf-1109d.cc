...
void solve() {
    ...
    for (int i = 1; i < n; ++i) {
        res += (fastcomb(n - 2, i - 1) * fact[i - 1])  // choose vertices on the chain
               * fastcomb(m - 1, i - 1)  // edge weight on chain
               * ss[(i + 1) % 2][i + 1] / ss[(i + 1) % 2][i + 2 * (n - 1 - i) + 1]  // tree structure
               * qpow<mll>(m, n - 1 - i);  // edge weight
    }
