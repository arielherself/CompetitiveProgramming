...
void solve() {
    ...
    unordered_map<ll, vector<ll>, safe_hash> need;
    set<pll> cand;
    unordered_map<ll, ll, safe_hash> curr;
    for (int i = 0; i < n; ++i) {
        ...
        while (cnt.count(target)) {
            f = 1;
            need[target].emplace_back(a[i]);
            target *= 2;
        }
        if (f) {
            cand.emplace(target - 1, a[i]);
            curr[a[i]] = target - 1;
        }
    }
    auto remove = [&] (ll x) -> void {
        debug(x);
        cnt.erase(cnt.lower_bound(x));
        if (not cnt.count(x) and curr.count(x)) {
            cand.erase({ curr[x], x });
            curr.erase(x);
        }
        for (auto&& y : need[x]) {
    ...
    vector<ll> res;
    while (cnt.size()) {
        if (cand.size() and cand.rbegin()->second > *cnt.rbegin()) {
            // use cand
            auto [val, start] = *cand.rbegin();
            debug(1);
            remove(start);
            ll p = start + 1;
            while (p <= val) {
