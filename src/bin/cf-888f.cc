...
void solve() {
    ...
    vector cache(n, vector(n, vector<ll>(2, -1)));
    auto dfs = [&] (auto dfs, int l, int r, int left) -> ll {
        if (l == r) return (left and a[l][l - 1]) + a[r][(r + 1) % n];
        else if (l > r) return 1;
        if (cache[l][r][left] != -1) {
            return cache[l][r][left];
        }
        ll res = 0;
        if (left) {
            int u = (l - 1) % n;
            for (int i = l; i <= r; ++i) {
                if (a[u][i]) {
                    res = (res + dfs(dfs, l, i - 1, true) * dfs(dfs, i + 1, r, true) % MDL) % MDL;
                    // deb(l, r, u, i, res);
                }
            }
        }
        // connect to the right bound
        {
            int u = (r + 1) % n;
            for (int i = l; i <= r; ++i) {
                if (a[u][i]) {
                    res = (res + dfs(dfs, l, i - 1, false) * dfs(dfs, i + 1, r, true) % MDL) % MDL;
                    // deb(l, r, i, u, res);
                }
            }
        }
        return cache[l][r][left] = res;
    };
    ll res = 0;
    for (int i = 1; i < n; ++i) {
        if (a[0][i]) {
            res = (res + dfs(dfs, 1, i - 1, false) * dfs(dfs, i + 1, n - 1, true) % MDL) % MDL;
        }
    }
    cout << res << '\n';
}
