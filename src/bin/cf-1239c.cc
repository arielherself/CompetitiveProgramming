...
void solve() {
    ...
    for (int i = 0; i < n; ++i) {
        read(int, x);
        events.emplace(x, 0, i);
    }
    deque<int> q;
    set<int> waiting, in_queue;
    vector<ll> res(n);
    int lock = 0;
    while (events.size()) {
        auto [t, type, i] = *events.begin();
        ...
        if (waiting.size() and (in_queue.empty() or *in_queue.begin() > *waiting.begin())) {
            q.emplace_back(*waiting.begin());
            in_queue.emplace(*waiting.begin());
            waiting.erase(waiting.begin());
        }
        if (lock == 0 and q.size()) {
            lock = 1;
            events.emplace(t + p, 1, q.front());
            q.pop_front();
        }
    }
    putvec(res);
