...
void solve() {
    ...
    for (int i = 0; i < n; ++i) {
        if (a[i] % 2 == 1) {
            net.add_edge(s, i + 1, b[i], 0);
        } else {
            net.add_edge(i + 1, t, b[i], -2);
        }
        if (a[i] == 1) {
            net.add_edge(i + 1, t, b[i], -1);
        }
    }
    ...
    for (int i = 0; i < n; ++i) {
        for (int j = i + 1; j < n; ++j) {
            if (is_prime[a[i] + a[j]]) {
                if (a[i] % 2 == 1) {
                    net.add_edge(i + 1, j + 1, INFLL, 0);
                } else {
                    net.add_edge(j + 1, i + 1, INFLL, 0);
                }
            }
        }
    }

    ll res = (-net.run(s, t).second) / 2;
