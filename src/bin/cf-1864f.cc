...
void solve() {
    ...
    segtree<Info, Tag> a(n), dp(n + 1);

    vector<vector<pii>> qbk(n + 1);
    for (int i = 0; i < q; ++i) {
        read(int, l, r);
        qbk[r].emplace_back(l, i);
    }
    ...
    for (int i = 1; i <= n; ++i) {
        if (bk[i].size())
            dp.range_apply(1, i, {1});

        int prev = -1;
        for (auto&& j : bk[i]) {
            if (prev != -1 and prev != j - 1) {
                int x = a.range_query(prev + 1, j - 1).val;
                dp.range_apply(0, x, {1});
            }
