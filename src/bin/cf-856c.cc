...
void solve() {
    ...
    int m = odd.size();
    vector dpo(m / 2 + 1, vector<mll>(11));
    dpo[0][0] = 1;
    for (int i = 0; i < m; ++i) {
        for (int j = m / 2 - 1; ~j; --j) {
            for (int k = 0; k < 11; ++k) {
                dpo[j + 1][mod(k - 2 * odd[i], 11)] += dpo[j][k];
            }
        }
    }
    vector<mll> left(11);
    for (int i = 0; i < 11; ++i) {
        left[(oddsum + i) % 11] = dpo[m / 2][i] * fact[m / 2] * fact[m - m / 2];
    }
    // debug(left);

    int b = (m + 1) / 2;
    int a = m + 1 - b;
    vector dpe(n - m + 1, vector<mll>(11));
    dpe[0][0] = 1;
    for (int i = 0; i < n - m; ++i) {
        for (int j = n - m - 1; ~j; --j) {
            for (int k = 0; k < 11; ++k) {
                dpe[j + 1][mod(k - 2 * even[i], 11)] += dpe[j][k];
            }
        }
    }
    ...
    vector<mll> right(11);
    if (b == 0) {
        right[evensum] = fact[n - m] * comb(n - m + a - 1, n - m);
    } else {
        for (int i = 0; i <= n - m; ++i) {
            for (int j = 0; j < 11; ++j) {
                right[(evensum + j) % 11] += dpe[i][j] * fact[i] * comb(i + b - 1, i) * fact[n - m - i] * comb(n - m - i + a - 1, n - m - i);
            }
        }
    }
    // debug(left);
