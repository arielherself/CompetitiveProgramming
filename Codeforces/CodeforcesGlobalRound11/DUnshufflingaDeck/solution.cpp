...
void solve() {
    ...
    auto work = [&] (vector<int>& seq) -> void {
        // debug(seq);
        // debug(accumulate(seq.begin(), seq.end(), 0)), debug(n);
        assert(accumulate(seq.begin(), seq.end(), 0) == n);
        res.push_back(seq);
    ...
    int direction = 1;  // 1 for forward, 0 for backward
    for (int i = 2; i <= n; ++i) {
        if (direction == 1) {
            if (rev[i] == rev[i - 1] + 1) continue;
            if (rev[i] > rev[i - 1]) {
                vector<int> op;
                if (rev[1]) op.push_back(rev[1]);
                for (int j = 1; j <= i - 1; ++j) {
                    op.push_back(1);
                }
                op.push_back(rev[i] - rev[i - 1]);
                if (n - 1 - rev[i]) op.push_back(n - 1 - rev[i]);
                direction = 0;
                work(op);
            } else {
                vector<int> op;
                if (rev[i]) op.push_back(rev[i]);
                op.push_back(rev[1] - rev[i]);
                op.push_back(i - 1);
                if (n - 1 - rev[i - 1]) op.push_back(n - 1 - rev[i - 1]);
                work(op);
            }
        } else {
            if (rev[i - 1] == rev[i] + 1) continue;
            if (rev[i - 1] > rev[i]) {
                vector<int> op;
                if (rev[i]) op.push_back(rev[i]);
                op.push_back(rev[i - 1] - rev[i]);
                for (int j = i - 1; j >= 1; --j) {
                    op.push_back(1);
                }
                if (n - 1 - rev[1]) op.push_back(n - 1 - rev[1]);
                direction = 1;
                work(op);
            } else {
                vector<int> op;
                if (rev[i - 1]) op.push_back(rev[i - 1]);
                op.push_back(i - 1);
                op.push_back(rev[i] - rev[1]);
                if (n - 1 - rev[i]) op.push_back(n - 1 - rev[i]);
                work(op);
            }
        }
