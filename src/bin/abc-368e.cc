...
void solve() {
    ...
    vector<ll> x(m);
    cin >> x[0];
    vector<tuple<int, int, int, int>> events(m);
    set<int> st;
    for (int i = 0; i < m; ++i) {
        cin >> get<0>(events[i]) >> get<1>(events[i]) >> get<2>(events[i]) >> get<3>(events[i]);
        st.emplace(get<2>(events[i]));
        st.emplace(get<3>(events[i]));
    }

    int N = 0;
    map<int, int> mp;
    vector<int> rev(1);
    for (auto&& x : st) mp[x] = ++N, rev.emplace_back(x);

    vector<vector<tiii>> open(N + 1);
    vector<vector<pli>> close(N + 1);
    vector<int> raw_close(m);
    for (int i = 0; i < m; ++i) {
        auto&& [u, v, s, t] = events[i];
        s = mp[s], t = mp[t];
        --u, --v;
        open[s].emplace_back(u, v, i);
        raw_close[i] = rev[t];
    }
    ...
    for (int i = 1; i <= N; ++i) {
        // deb(i);
        for (auto&& [tm, v] : close[i]) {
            // deb(tm, v);
            chmax(last[v], tm);
        }
        for (auto&& [u, v, idx] : open[i]) {
            chmax(x[idx], last[u] - rev[i]);
            // deb(u, v, idx, x[idx]);
            // auto it = mp.lower_bound(raw_close[idx] + x[idx]);
            // if (it != mp.end()) {
            close[mp[raw_close[idx]]].emplace_back(raw_close[idx] + x[idx], v);
            // }
        }
    }
