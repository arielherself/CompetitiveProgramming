...
void solve() {
    auto deploy = [] (const vector<vector<ll>>& grid) -> int {
        for (auto&& v : grid) {
            for (auto&& x : v) {
                cout << x << ' ';
            }
    ...
    auto claim = [] (const vector<pii>& seq) -> void {
    ...
    while (q--) {
        ...
        while (path.size() < 2 * n - 1) {
            auto [i, j] = path.back();
            // debug(i), debug(j);
            ll fact = s & ((1LL << i + j + 2) - 1);
            if (i + 1 < n && fact == sum + grid[i + 1][j]) {
                sum += grid[i + 1][j];
                path.emplace_back(i + 1, j);
            } else if (j + 1 < n && fact == sum + grid[i][j + 1]) {
                sum += grid[i][j + 1];
        ...
        assert(path.back() == make_pair(n - 1, n - 1));
        claim(path);
    }
}
