...
always_inline void merge(basis<int>& a, const basis<int>& b) {
	for (auto&& x : b.p) {
		a.insert(x);
	}
}
...
void solve() {
	...
	vector<int> res(m, -1);
	{
		...
		vector<basis<int>> b(n + 1);
		vector<int> open(n + 1, -1);
		sparse_table<int> rmq;
		vector<int> seq;
		auto find_root = [&] (int s) -> int {
			{
		...
		auto dfs = [&] (auto dfs, int v) -> void {
			...
			rmq = sparse_table<int>(seq.rbegin(), seq.rend(), [&] (int u, int v) {
			...
			{
				auto calc = [&] (int idx) {
					auto&& [u, v, x] = queries[idx];
					int i = open[u], j = open[v];
					if (res[idx] != -1 or i == -1 or j == -1) return;
					if (i > j) swap(i, j);
					int l = rmq.query(i, j);
					if (l != new_root) return;
					if (depth[u] > depth[v]) swap(u, v);
					basis<int> curr = b[v];
					if (l != u) {
						merge(curr, b[u]);
					}
					for (auto&& y : curr.value()) {
						if (x == 0) break;
						if (msp(x) == msp(y)) {
							x ^= y;
						}
