...
void solve() {
	...
	while (q--) {
		read(int, k);
		readvec(int, ii, k);
		transform(ii.begin(), ii.end(), ii.begin(), expr(x - 1, auto&& x));
		...
		sort_by_key(roots.begin(), roots.end(), expr(dfn[v], auto&& v));
		k = unique(roots.begin(), roots.end()) - roots.begin();
		roots.resize(k);
		vector<ull> vals;
		vals.reserve(k);
		for (auto&& v : roots) {
			vals.emplace_back(tr.range_query(dfn[v], dfn[v] + sz[v] - 1).val);
		}
		vector<ull> realvals(k);
		for (int i = k - 1; ~i; --i) {
			realvals[i] = vals[i];
			for (int j = i + 1; j < k; ++j) {
				if (j == i) continue;
				if (dfn[roots[j]] >= dfn[roots[i]] and dfn[roots[j]] < dfn[roots[i]] + sz[roots[i]]) {
					realvals[i] ^= realvals[j];
				}
			}
		}
		int f = 1;
