...
void solve() {
    ...
    vector<bool> vis(1 << n + 5);
    int start;
    auto dfs = [&] (auto dfs, int v, int mask) -> bool {
        if (vis[mask << 5 | v]) return false;
        vis[mask << 5 | v] = 1;
        if (c[mask] == (1 << n) - 1) {
            for (auto&& u : ch[v]) {
                if (u == start) {
        ...
        if (dfs(dfs, i + 1, 1 << i)) {
            ...
            if (m > 1) {
                for (int i = 0; i < m; ++i) {
                    res[seq[i]] = seq[(i + 1) % m];
                    for (auto&& u : ch[seq[i]]) {
                        if (not count(seq.begin(), seq.end(), u)) {
                            res[u] = seq[i];
                        }
                    }
                }
