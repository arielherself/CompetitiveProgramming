...
auto u_max = [] (int x, int y) { return max(x, y); };
void solve() {
    read(int, n, m);
    readvec(int, a, m);
    sparse_table<int> st(a.rbegin(), a.rend(), u_max);
    read(int, q);
    while (q--) {
        read(int, xs, ys, xf, yf, k);
        --ys, --yf;
        if (ys > yf) swap(xs, xf), swap(ys, yf);
        int need = yf - ys >= 2 and st.query(ys + 1, yf - 1) >= min(xs, xf);
        // debug(st.query(ys, yf)), debug((xs % k) + (n - 1 - (xs % k)) / k * k);
        if ((yf - ys) % k == 0 and abs(xs - xf) % k == 0 and (not need or st.query(ys + 1, yf - 1) < (xs % k) + (n - (xs % k)) / k * k)) {
            cout << "YES\n";
        } else {
            cout << "NO\n";
        }
