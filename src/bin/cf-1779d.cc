...
void solve() {
    ...
    readvec(int, b, n);
    read(int, m);
    unordered_map<int, int, safe_hash> has;
    unordered_map<int, int, safe_hash> need;
    unordered_map<int, int, safe_hash> prev;
    while (m--) {
        read(int, x);
        has[x] += 1;
    }
    auto u_max = [] (int x, int y) -> int { return max(x, y); };
    sparse_table<int> st(b.rbegin(), b.rend(), u_max);
    for (int i = 0; i < n; ++i) {
        if (a[i] < b[i] || a[i] > b[i] && !has.count(b[i])) {
            cout << "NO\n";
            return;
        }
        if (a[i] == b[i]) continue;
        else {
            if (!prev.count(b[i]) || st.query(prev[b[i]], i) > b[i]) {
                need[b[i]] += 1;
            }
            prev[b[i]] = i;
        }
