...
void solve() {
    ...
    vector ps(n + 1, vector<ll>(n + 1));
    for (int i = 1; i <= n; ++i) {
        for (int j = 1; j <= n; ++j) {
            ps[i][j] = ps[i - 1][j] + ps[i][j - 1] - ps[i - 1][j - 1] + a[i - 1][j - 1];
        }
    }
    vector b(n - m + 1, vector<ll>(n - m + 1));
    for (int i = 0; i < n - m + 1; ++i) {
        for (int j = 0; j < n - m + 1; ++j) {
            b[i][j] = ps[i + m][j + m] - ps[i][j + m] - ps[i + m][j] + ps[i][j];
        }
    }
    vector nw(n - m + 1, vector<ll>(n - m + 1));
    for (int i = 0; i < n - m + 1; ++i) {
        for (int j = 0; j < n - m + 1; ++j) {
            nw[i][j] = b[i][j];
            if (i - 1 >= 0) chmax(nw[i][j], nw[i - 1][j]);
            if (j - 1 >= 0) chmax(nw[i][j], nw[i][j - 1]);
        }
    }
    vector ne(n - m + 1, vector<ll>(n - m + 1));
    for (int i = 0; i < n - m + 1; ++i) {
        for (int j = n - m; ~j; --j) {
            ne[i][j] = b[i][j];
            if (i - 1 >= 0) chmax(ne[i][j], ne[i - 1][j]);
            if (j + 1 <= n - m) chmax(ne[i][j], ne[i][j + 1]);
        }
    }
    vector sw(n - m + 1, vector<ll>(n - m + 1));
    for (int i = n - m; ~i; --i) {
        for (int j = 0; j < n - m + 1; ++j) {
            sw[i][j] = b[i][j];
            if (i + 1 <= n - m) chmax(sw[i][j], sw[i + 1][j]);
            if (j - 1 >= 0) chmax(sw[i][j], sw[i][j - 1]);
        }
    }
    vector se(n - m + 1, vector<ll>(n - m + 1));
    for (int i = n - m; ~i; --i) {
        for (int j = n - m; ~j; --j) {
            se[i][j] = b[i][j];
            if (i + 1 <= n - m) chmax(se[i][j], se[i + 1][j]);
            if (j + 1 <= n - m) chmax(se[i][j], se[i][j + 1]);
        }
    }
    ll res = 0;
    for (int i = m; i < n - m + 1; ++i) {
        for (int j = m; j < n - m + 1; ++j) {
            // |-
            chmax(res, nw[n - m][j - m] + ne[i - m][j] + se[i][j]);
            // -|
            chmax(res, nw[i - m][j - m] + sw[i][j - m] + ne[n - m][j]);
            // T
            chmax(res, nw[i - m][n - m] + sw[i][j - m] + se[i][j]);
            // reversed T
            chmax(res, nw[i - m][j - m] + ne[i - m][j] + sw[i][n - m]);
        }
    }
    vector<ll> row(n - m + 1), col(n - m + 1);
    for (int i = 0; i < n - m + 1; ++i) {
        for (int j = 0; j < n - m + 1; ++j) {
            chmax(row[i], b[i][j]);
            chmax(col[j], b[i][j]);
    ...
    vector<ll> rs(n - m + 2), cs(n - m + 2);
    for (int i = n - m; ~i; --i) {
        rs[i] = max(rs[i + 1], row[i]);
        cs[i] = max(cs[i + 1], col[i]);
    }
    for (int i = 0; i < n - m + 1; ++i) {
        for (int j = i + m; j < n - m + 1; ++j) {
            if (j + m < n - m + 1) {
                chmax(res, row[i] + row[j] + rs[j + m]);
                chmax(res, col[i] + col[j] + cs[j + m]);
            }
        }
    }
    cout << res << '\n';
