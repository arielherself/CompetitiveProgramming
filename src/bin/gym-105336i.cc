...
void solve() {
    ...
    for (int t = 0; t <= N; ++t) {
        ...
        vector<int> ps(N);
        partial_sum(oc.begin(), oc.end(), ps.begin());

        for (int i = 1; i <= m; ++i) {
            int ptr = ps[b[i - 1]];
            int prev_same = oc[b[i - 1]];

            for (int j = n; j; --j) {
                dp[j][1] += ((dp[j - 1][1] * (ptr - (j - 1))) % PRIME)
                            + ((dp[j - 1][0] * prev_same) % PRIME);
                dp[j][1] %= PRIME;
                dp[j][0] += (dp[j - 1][0] * max(0, ptr - prev_same - (j - 1))) % PRIME;
                dp[j][0] %= PRIME;
            }
        }
