...
void solve() {
    ...
    vector<ll> pre(n + 1, -INFLL);
    pre[0] = 0;
    for (int i = 1; i <= n; ++i) {
        for (int j = 0; j < i; ++j) {
            chmax(pre[i], pre[j] + pre[i - j]);
        }
        chmax(pre[i], w[i]);
    }

    auto is_same = [&] (int l, int r) {
        return l > r or ps[r + 1] - ps[l] == 0 or ps[r + 1] - ps[l] == r - l + 1;
    };

    vector dp(n, vector(n, vector(n + 1, array<ll, 2> { -INFLL, -INFLL })));
    for (int i = n - 1; ~i; --i) {
        for (int j = i; j < n; ++j) {
            if (is_same(i, j)) {
                for (int k = 0; k <= j - i + 1; ++k) {
                    chmax(dp[i][j][k][a[i]], w[j - i + 1 - k]);
                }
                chmax(dp[i][j][0][0], pre[j - i + 1]);
                chmax(dp[i][j][0][1], pre[j - i + 1]);
            }

            for (int t = 0; t < 2; ++t) {
                for (int k = 0; k <= j - i + 1; ++k) {
                    for (int l = i; l < j; ++l) {
                        chmax(dp[i][j][k][t], max(dp[i][l][k][t] + dp[l + 1][j][0][0], dp[i][l][0][0] + dp[l + 1][j][k][t]));
                    }
                }
            }
            ...
            if (i != j) {
                for (int k = 0; k < j - i + 1; ++k) {
                    chmax(dp[i][j][k + 1][a[j]], dp[i][j - 1][k][a[j]]);
                    chmax(dp[i][j][k + 1][a[i]], dp[i + 1][j][k][a[i]]);
                }
            }

            for (int k = 1; k <= j - i + 1; ++k) {
                chmax(dp[i][j][0][0], pre[k] + max(dp[i][j][k][0], dp[i][j][k][1]));
            }
            chmax(dp[i][j][0][1], dp[i][j][0][0]);
        }
    }
