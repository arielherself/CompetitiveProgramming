...
void solve() {
    ...
    while (q--) {
        ...
        vector<vector<tiii>> sequences;
        vector<tiii> curr;
        auto dfs = [&] (auto dfs, int i) -> void {
            if (i == k) {
                sequences.emplace_back(curr);
            } else {
                curr.emplace_back(edges[a[i]]);
                dfs(dfs, i + 1);
                curr.pop_back();
                curr.emplace_back(get<1>(edges[a[i]]), get<0>(edges[a[i]]), get<2>(edges[a[i]]));
                dfs(dfs, i + 1);
                curr.pop_back();
            }
        };
