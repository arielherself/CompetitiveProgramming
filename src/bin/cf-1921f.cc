...
void solve() {
    ...
    readvec(ll, a, n);
    int sq = sqrt(n);
    vector<vector<vector<ll>>> serial_ps(sq + 1), ps(sq + 1);
    for (int step = 0; step <= sq; ++step) {  // O(n*sqrt(n))
        for (int i = 0; i < step; ++i) {
            serial_ps[step].push_back({0});
            ps[step].push_back({0});
            int cnt = 0;
            for (int j = i; j < n; j += step) {
                cnt += 1;
                serial_ps[step][i].push_back(serial_ps[step][i].back() + cnt * a[j]);
                ps[step][i].push_back(ps[step][i].back() + a[j]);
            }
        }
    }
    while (q--) {
        ...
        if (d <= sq) {
            int start = s % d;
            int begin_idx = (s - start) / d + 1;
            int end_idx = begin_idx + k - 1;
            // debug(begin_idx), debug(end_idx);
            cout << (serial_ps[d][start][end_idx] - serial_ps[d][start][begin_idx - 1]) - (begin_idx - 1) * (ps[d][start][end_idx] - ps[d][start][begin_idx - 1]) << " \n"[q == 0];
        } else {
            ll res = 0;
            for (int i = s, t = 1; i <= s + d * (k - 1); i += d, t += 1) {
                res += t * a[i];
            }
            cout << res << " \n"[q == 0];
        }
