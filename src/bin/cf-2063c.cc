...
void solve() {
	...
	multiset<int> st;
	for (int i = 1; i <= n; ++i) {
		st.emplace(ch[i].size());
	}
	int res = 0;
	for (int i = 1; i <= n; ++i) {
		for (auto&& u : ch[i]) {
			st.erase(st.find(ch[u].size()));
			st.emplace(ch[u].size() - 1);
		}
		st.erase(st.find(ch[i].size()));
		chmax(res, ch[i].size() + *st.rbegin());
		st.emplace(ch[i].size());
		for (auto&& u : ch[i]) {
			st.erase(st.find(ch[u].size() - 1));
			st.emplace(ch[u].size());
		}
	}
	cout << res - 1 << '\n';
}
