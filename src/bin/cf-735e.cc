...
void solve() {
    ...
    vector dp(n + 1, vector(k + 2, vector<mll>(k + 2)));
    {
        auto dfs = [&] (auto dfs, int v, int pa) -> void {
            ...
            for (auto&& u : ch[v]) {
                if (u == pa) continue;
                vector ndp(k + 2, vector<mll>(k + 2));
                for (int j = 0; j <= k + 1; ++j) {
                    for (int l = 0; l <= k + 1; ++l) {
                        for (int p = 0; p <= k + 1; ++p) {
                            for (int q = 0; q <= k + 1; ++q) {
                                int new_maxdis = -1;
                                if (j != k + 1 and j + q + 1 > k) {
                                    chmax(new_maxdis, j);
                                }
                                if (p != k + 1 and p + 1 + l > k) {
                                    chmax(new_maxdis, p + 1);
                                }
                                if (new_maxdis > k) continue;
                                if (new_maxdis == -1) {
                                    new_maxdis = k + 1;
                                }
                                ndp[new_maxdis][min({l, k + 1, q + 1})] += dp[v][j][l] * dp[u][p][q];
                            }
                        }
                    }
                }
    ...
    cout << accumulate(dp[1][k + 1].begin(), dp[1][k + 1].end(), mll(0)) << '\n';
}
