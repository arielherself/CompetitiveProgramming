...
struct integer {
    ...
    friend integer operator+(const integer& a, const integer& b) {
        ...
        int m = a.num.size(), n = b.num.size();
        integer res(0, a.k);
        // debug(res.num);
        int p = min(m, n), l = max(m, n);
        for (int i = 0; i < p; ++i) {
            // debug(a.num[i]), debug(b.num[i]);
            // debug(a.k), debug(a.num[i] + b.num[i]), debug(mod(a.num[i] + b.num[i], a.k));
            res.num.push_back(mod(a.num[i] + b.num[i], a.k));
        }
        // debug(res.num);
        if (n > m) {
            for (int i = m; i < n; ++i) {
        ...
        res.raw = 0;
        for (int i = l - 1; ~i; --i) {
            res.raw = res.raw * a.k + res.num[i];
        }
        // debug(res.raw), debug(res.num);
        // cerr << "======\n";
        return res;
    }
};
...
void solve() {
    assert((integer(2, 2) + integer(1, 2)).raw == 3);
    read(int, n, k);
    int parity = 0;
    integer acc(0, k);
    auto query = [&] (int i) -> bool {
        integer x(i, k);
        // debug(parity), debug(x.num), debug(acc.num);
        if (parity == 0) {
            cout << (x + acc).raw << endl;
        } else {
            cout << ((-x) + acc).raw << endl;
