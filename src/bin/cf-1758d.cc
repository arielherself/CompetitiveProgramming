...
void solve() {
    read(ll, n);
    ll target = -1, bottom = -1, top = -1;
    for (ll i = 2; i <= n + 10; ++i) {
        ll left = n - 1 + i + (n - 1) * (n - 2) / 2;
        ll right = (n - 1) * i - (n - 1) * (n - 2) / 2 + 1;
        ll diff2 = (i - 1) * (i - 1);
        ll k = max(ll(0), (diff2 - left) / n);
        ll real_left = left + n * k;
        ll real_right = right + n * k;
        if (diff2 >= real_left && diff2 <= real_right) {
            target = diff2;
            bottom = 1 + k;
            top = i + k;
            break;
    ...
    assert(target != -1);
    // debug(bottom), debug(top);
    ll sum = 0;
    ll prev = bottom - 1;
    for (int i = 0; i < n; ++i) {
        ll curr = max(target - sum - (2 * top - (n - 2 - i)) * (n - 1 - i) / 2, prev + 1);
        sum += curr;
        prev = curr;
        cout << curr << " \n"[i + 1 == n];
    }
