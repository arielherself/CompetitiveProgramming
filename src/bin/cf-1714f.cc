...
void solve() {
    read(int, n);
    read(int, x, y, z);
    auto work = [&] (int x, int y, int z, int v1, int v2, int v3) -> vector<pii> {
        if (x + y <= z or ((x + y - z) & 1)) return {};
        int more = (x + y - z) / 2;
        int v = 1;
        auto next_v = [&] () {
            while (v == v1 or v == v2 or v == v3) {
                ++v;
            }
            return v++;
        };
        ...
        int prev = v1;
        for (int i = 0; i < z; ++i) {
            int curr = i + 1 == z ? v3 : next_v();
            if (curr > n) return {};
            e.emplace_back(prev, curr);
            prev = curr;
            if (i + 1 == x - more) {
                mark = curr;
            }
