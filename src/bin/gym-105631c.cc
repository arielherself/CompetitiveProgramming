...
void solve() {
    read(int, n, m);
    n += 1;
    vector cntrj(n, vector<int>(26));
    vector ac(n, vector<bool>(26));
    vector<pil> status(n);

    vector<tuple<string, int, ll, int>> events;
    vector<ll> oc = { ll(26) << 32 };
    for (int i = 0; i < m; ++i) {
        ll time = 0;
        read(string, raw_time);
        time += stoi(raw_time.substr(0, 2)) * 60 + stoi(raw_time.substr(3, 2));
        read(int, team);
        read(char, raw_no);
        int no = raw_no - 'A';
        read(string, raw_stat);
        int stat = raw_stat == "AC";
        bool upd = team == 0 and stat and not ac[team][no];
        if (stat) {
            if (ac[team][no]) {
                ;;
            } else {
                ac[team][no] = 1;
                status[team].first += 1;
                status[team].second += time + ll(1) * 20 * cntrj[team][no];
            }
        } else {
            cntrj[team][no] += 1;
        }
        // deb(team, status[team]);
        ll currstatus = (26 - ll(1) * status[team].first) << 32 bitor status[team].second;
        oc.emplace_back(currstatus);
        events.emplace_back(raw_time, team, currstatus, upd ? no : -1);
    }

    sort(oc.begin(), oc.end());
    int t = unique(oc.begin(), oc.end()) - oc.begin();
    oc.resize(t);
    auto get = [&] (ll x) { return lower_bound(oc.begin(), oc.end(), x) - oc.begin(); };

    BIT<int> tr(t);

    vector<ll> curr(n, (ll(1) * 26) << 32);
    tr.add((ll(1) * 26) << 32, n);

    for (auto&& [raw_time, team, penalty, no] : events) {
        int old;
        if (no != -1) {
            old = tr.getsum(get(curr[0])) + 1;
        }
        tr.add(get(curr[team]) + 1, -1);
        curr[team] = penalty;
        tr.add(get(curr[team]) + 1, 1);
        if (no != -1) {
            cout << raw_time << ' ' << char(no + 'A') << " #" << old << " -> #" << tr.getsum(get(curr[0])) + 1 << '\n';
        }
    }
}
