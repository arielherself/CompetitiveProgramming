...
void solve() {
    read(int, k, n);
    readvec(int, a, n);
    transform(a.begin(), a.end(), a.begin(), expr(x - 1, int x));
    map<int, int> rev;
    for (int i = 0; i < n; ++i) {
        rev[a[i]] = i;
    }
    ...
    auto dis = [&] (int x, int y) {
        return min(mod(x - y, k), mod(y - x, k));
    };
    auto direction = [&] (int i, int j, int k) {
        // check order i->k->j
        if (j < i) {
            return not (j <= k and k <= i);
        } else {
            return i <= k and k <= j;
        }
    };
    for (int i = 0; i < n; ++i) {
        array<int, 2> cand = { a[i], a[i] };
        {
            auto it = rev.upper_bound((a[i] + k / 2) % k);
            if (it != rev.begin()) {
                --it;
                cand[0] = it->first;
            } else {
                cand[0] = rev.rbegin()->first;
            }
        }
        {
            auto it = rev.lower_bound((a[i] + (k + 1) / 2) % k);
            if (it != rev.end()) {
                cand[1] = it->first;
            } else {
                cand[1] = rev.begin()->first;
            }
        }
        vector<int> curr;
        if (cand[0] != a[i] and cand[1] != a[i]) {
            if (dis(cand[0], a[i]) >= dis(cand[1], a[i])) {
                curr.emplace_back(rev[cand[0]]);
            }
            if (dis(cand[0], a[i]) <= dis(cand[1], a[i])) {
                curr.emplace_back(rev[cand[1]]);
            }
        } else if (cand[0] != a[i]) {
            curr.emplace_back(rev[cand[0]]);
        } else if (cand[1] != a[i]) {
        ...
        int m = unique(curr.begin(), curr.end()) - curr.begin();
        far[i].insert(far[i].end(), curr.begin(), curr.begin() + m);
        if (direction(i, far[i][0], far[i].back())) {
            swap(far[i][0], far[i].back());
        }
    }
    // debug(far);
    int res = 0;
    ...
    for (int i = 0; i < n; ++i) {
        for (auto&& j : far[i]) {
            ...
            if (i < j) {
                {
                    auto l = upper_bound(from[i].begin(), from[i].end(), i);
                    auto r = lower_bound(from[i].begin(), from[i].end(), j);
                    left_a += r - l;
                    right_a += from[i].size() - (r - l);
                    if (far[j][0] == i or far[j].back() == i) right_a -= 1;
                }
                {
                    auto l = upper_bound(from[j].begin(), from[j].end(), i);
                    auto r = lower_bound(from[j].begin(), from[j].end(), j);
                    left_b += r - l;
                    right_b += from[j].size() - (r - l);
                    right_b -= 1;
                }
            } else {
                {
                    auto l = upper_bound(from[i].begin(), from[i].end(), j);
                    auto r = lower_bound(from[i].begin(), from[i].end(), i);
                    right_a += r - l;
                    left_a += from[i].size() - (r - l);
                    if (far[j][0] == i or far[j].back() == i) left_a -= 1;
                }
                {
                    auto l = upper_bound(from[j].begin(), from[j].end(), j);
                    auto r = lower_bound(from[j].begin(), from[j].end(), i);
                    right_b += r - l;
                    left_b += from[j].size() - (r - l);
                    left_b -= 1;
                }
            }
            chmax(res, 1 + max(left_a, left_b) + max(right_a, right_b));
        }
        {
            int left_a = 0, left_b = 0;
            int right_a = 0, right_b = 0;
            if (i < far[i][0]) {
                {
                    auto l = upper_bound(from[i].begin(), from[i].end(), i);
                    auto r = lower_bound(from[i].begin(), from[i].end(), far[i][0]);
                    left_a += r - l;
                    right_a += from[i].size() - (r - l);
                    if (far[far[i][0]][0] == i or far[far[i][0]].back() == i) right_a -= 1;
                    if (far[i].size() > 1 and (far[far[i][1]][0] == i or far[far[i][1]].back() == i)) {
                        if (i < far[i][1] and far[i][1] < far[i][0]) {
                            left_a -= 1;
                        } else {
                            right_a -= 1;
                        }
                ...
                {
                    auto l = upper_bound(from[far[i][0]].begin(), from[far[i][0]].end(), i);
                    auto r = lower_bound(from[far[i][0]].begin(), from[far[i][0]].end(), far[i][0]);
                    left_b += r - l;
                    // right_b += from[far[i][0]].size() - (r - l);
                    // right_b -= 1;
                }
            } else {
                {
                    auto l = upper_bound(from[i].begin(), from[i].end(), far[i][0]);
                    auto r = lower_bound(from[i].begin(), from[i].end(), i);
                    right_a += r - l;
                    left_a += from[i].size() - (r - l);
                    if (far[far[i][0]][0] == i or far[far[i][0]].back() == i) left_a -= 1;
                    if (far[i].size() > 1 and (far[far[i][1]][0] == i or far[far[i][1]].back() == i)) {
                        if (far[i][0] < far[i][1] and far[i][1] < i) {
                            right_a -= 1;
                        } else {
                            left_a -= 1;
                        }
                ...
                {
                    auto l = upper_bound(from[far[i][0]].begin(), from[far[i][0]].end(), far[i][0]);
                    auto r = lower_bound(from[far[i][0]].begin(), from[far[i][0]].end(), i);
                    // right_b += r - l;
                    left_b += from[far[i][0]].size() - (r - l);
                    left_b -= 1;
                }
            }
            if (far[i].size() > 1) {
                if (i < far[i][1]) {
                    auto l = upper_bound(from[far[i][1]].begin(), from[far[i][1]].end(), i);
                    auto r = lower_bound(from[far[i][1]].begin(), from[far[i][1]].end(), far[i][1]);
                    // left_b += r - l;
                    right_b += from[far[i][1]].size() - (r - l);
                    right_b -= 1;
                } else {
                    auto l = upper_bound(from[far[i][1]].begin(), from[far[i][1]].end(), far[i][1]);
                    auto r = lower_bound(from[far[i][1]].begin(), from[far[i][1]].end(), i);
                    right_b += r - l;
                    // left_b += from[far[i][1]].size() - (r - l);
                    // left_b -= 1;
                }
                if (far[far[i][0]][0] == far[i][1] or far[far[i][0]].back() == far[i][1]) {
                    left_b += 1, right_b += 1;
                }
            }
            // for (auto&& k : from[i]) {
            //     if (k == far[i][0] or k == far[i].back()) continue;
            //     if (direction(i, far[i][0], k)) {
            //         left_a += 1;
            //     } else {
            //         right_a += 1;
            //     }
            // }
            // for (auto&& k : from[far[i].back()]) {
            //     if (k == i) continue;
            //     if (not direction(i, far[i].back(), k)) {
            //         right_b += 1;
            //     }
            //     if (k == far[i][0]) {
            //         left_b += 1, right_b += 1;
            //     }
            // }
            // for (auto&& k : from[far[i][0]]) {
            //     if (k == i) continue;
            //     if (direction(i, far[i][0], k)) {
            //         left_b += 1;
            //     }
            // }
            // deb(i, left_a, left_b, right_a, right_b);
            chmax(res, 1 + max(left_a, left_b) + max(right_a, right_b));
        }
    }
    cout << res << '\n';
