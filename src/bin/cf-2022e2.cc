...
constexpr int N = 30;
constexpr int M = (2 * 100000 - 1) * N + 1;
ll pw[M];
quick_union qu[N];
...
void solve() {
	...
	auto update = [&] (int i, int j, int x) {
		...
		int r0 = zero + row + i, r1 = one + row + i;
		int c0 = zero + col + j, c1 = one + col + j;

		for (int k = 0; k < N; ++k) {
			if ((x >> k & 1) == 0) {
				// same
				if (qu[k].connected(r0, c1) or qu[k].connected(c0, r1)) {
					// deb(i, j, 1);
					f = 0;
				} else {
					// assert(qu.connected(zero + row + i, zero + col + j) == qu.connected(one + row + i, one + col + j));
					if (not qu[k].connected(r0, c0)) {
						qu[k].merge(r0, c0);
						qu[k].merge(r1, c1);
						ptr -= 1;
					}
				...
				// different
				// if (max({r0, c0, r1, c1}) >= (n + m) * 2 or min({r0, c0, r1, c1}) < 0) {
				// 	deb(r0, c0, r1, c1);
				// }
				if (qu[k].connected(r0, c0) or qu[k].connected(r1, c1)) {
					// deb(i, j, 2);
					f = 0;
				} else {
					// assert(qu.connected(zero + row + i, one + col + j) == qu.connected(one + row + i, zero + col + j));
					if (not qu[k].connected(r0, c1)) {
						qu[k].merge(r0, c1);
						qu[k].merge(r1, c0);
						ptr -= 1;
					}
