...
void solve() {
    ...
    int t = (n + sq - 1) / sq;
    // deb(sq, t);
    vector rel(t, segtree<Info, Tag>(t));
    for (int i = 0; i < t; ++i) {
        for (int j = 0; j < t; ++j) {
            int curr = 0;
            for (int k = j * sq; k < min(n, (j + 1) * sq); ++k) {
                if (revb[a[k]] >= i * sq and revb[a[k]] < (i + 1) * sq) {
                    ++curr;
                }
            }
            rel[i].set(j, { curr });
    ...
    while (m--) {
        ...
        read(int, op);
        if (op == 1) {
            read(int, la, ra, lb, rb);
            --la, --lb;
            int res = 0;
            if (ra - la < 2 * sq) {
                for (int j = la; j < ra; ++j) {
                    if (revb[a[j]] >= lb and revb[a[j]] < rb) {
                        ++res;
                    }
                }
            } else if (rb - lb < 2 * sq) {
                for (int j = lb; j < rb; ++j) {
                    if (reva[b[j]] >= la and reva[b[j]] < ra) {
                        ++res;
                    }
                }
            } else {
                ...
                int w = rb / sq * sq;
                res = 0;
                for (int j = z / sq; j < w / sq; ++j) {
                    if (x / sq < y / sq) res += rel[j].range_query(x / sq, y / sq - 1).val;
                }
                for (int j = la; j < x; ++j) {
                    if (revb[a[j]] >= z and revb[a[j]] < w) {
                        ++res;
                    }
                }
                for (int j = y; j < ra; ++j) {
            ...
            read(int, x, y);
            --x, --y;
            rel[x / sq].apply(reva[b[x]] / sq, { -1 });
            rel[x / sq].apply(reva[b[y]] / sq, { 1 });
            rel[y / sq].apply(reva[b[y]] / sq, { -1 });
            rel[y / sq].apply(reva[b[x]] / sq, { 1 });
            swap(revb[b[x]], revb[b[y]]);
            swap(b[x], b[y]);
            // debug(b);
        }
    }
