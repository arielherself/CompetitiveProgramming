 ...
 * My thought process
 * 1. Build a graph.
 * 2. For every CC, there is at most one contribution.
 * 3. Depending on the parity of edges.
 */
...
void solve() {
	...
	auto work = [&] (int root) {
		...
		sort_by_key(curr.begin(), curr.end(), expr(depth[v], auto&& v), greater());
		for (auto&& v : curr) {
			if (v == root) break;
			for (auto&& [u, i] : e[v]) {
				if (depth[u] > depth[v] or i == up[v]) continue;
				apply(i, u);
			}
			if (par[v]) {
				apply(up[v], v);
			} else {
				apply(up[v], edges[up[v]].first == v ? edges[up[v]].second : edges[up[v]].first);
			}
		}
	};
	...
	par.assign(n + 1, 0);
	for (int i = 0; i < m; ++i) {
		char choice = res[i] ? 'y' : 'x';
		int v = res[i] ? edges[i].second : edges[i].first;
		char op = par[v] ? '-' : '+';
		cout << choice << op << '\n';
		par[v] ^= 1;
	}
