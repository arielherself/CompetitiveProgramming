...
void solve() {
    ...
    vector<int> totl(n + 1), minl(n + 1);
    minl[0] = INF;
    for (int i = 1; i <= n; ++i) {
        totl[i] = totl[i - 1] + bk[i - 1];
        minl[i] = min(minl[i - 1], bk[i -1 ]);
    }

    vector<vector<mll>> f(n + 1);
    for (int i = 1; i <= n; ++i) {
        f[i].resize(minl[i] + 2);
        vector<mll> g(minl[i] + 2);
        g[0] = qpow<mll>(2, bk[i - 1]);
        f[i][0] = g[0] * (i == 1 ? 1 : f[i - 1][0]);
        for (int j = 1; j <= minl[i]; ++j) {
            g[j] = g[j - 1] - comb(bk[i - 1], j - 1);
            f[i][j] = g[j] * (i == 1 ? 1 : f[i - 1][j]);
        }
    }
    ...
    for (int i = 0; i < n; ++i) {
        for (int j = 1; j <= minl[i + 1]; ++j) {
            res += (f[i + 1][j] - f[i + 1][j + 1]) * qpow<mll>(2, totl[n] - totl[i + 1]) * j;
        }
    }
