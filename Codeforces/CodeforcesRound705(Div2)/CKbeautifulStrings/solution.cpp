...
t:
    if (pos != -1) {
        string permu;
        for (char i = 'a'; i <= 'z'; ++i) {
            permu += string((k - (ct[i] % k)) % k, i);
        }
        a[pos] = change_to;
        int add_as = n - 1 - pos - permu.size();
        for (int i = pos + 1; i <= pos + add_as; ++i) {
            a[i] = 'a';
        }
        int ptr = 0;
        for (int i = pos + add_as + 1; i < n; ++i) {
            a[i] = permu[ptr++];
        }
        cout << a << '\n';
    } else {
