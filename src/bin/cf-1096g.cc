...
void solve() {
    ...
    auto qpow = [&] (auto&& qpow, const vector<mll>& b, int p) -> vector<mll> {
        if (p == 1) return b;
        auto half = qpow(qpow, b, p / 2);
        if (p % 2 == 1) return multiply<PRIME>(multiply<PRIME>(half, half), b);
        else return multiply<PRIME>(half, half);
    };

    auto a = qpow(qpow, e, n / 2);
