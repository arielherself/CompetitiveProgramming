...
void solve() {
    ...
    auto seq = initarray(vector<int>(n), 26);
    vector<int> prev(26, -1);
    for (int i = 0; i < n; ++i) {
        seq[a[i] - 'a'][i] = i - prev[a[i] - 'a'];
        prev[a[i] - 'a'] = i;
    }
    ...
    vector<range_hash> hs;
    for (int i = 0; i < 26; ++i) {
        hs.emplace_back(seq[i]);
    }
    ...
    while (q--) {
        ...
        for (int j = 0; j < 26; ++j) {
            ...
            if (lfirst <= r1) {
                lhas = 1;
                if (lfirst + 1 <= r1) {
                    auto [p, q] = hs[j].range_query(lfirst + 1, r1);
                    l[lc++] = { p.val, q.val };
                }
            }
            int rfirst = nxt[l2][j];
            if (rfirst <= r2) {
                rhas = 1;
                if (rfirst + 1 <= r2) {
                    auto [p, q] = hs[j].range_query(rfirst + 1, r2);
                    r[rc++] = { p.val, q.val };
                }
            }
            // if (lhas != rhas) {
