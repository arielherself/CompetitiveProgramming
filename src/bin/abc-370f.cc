...
void solve() {
    ...
    vector<ll> ps(2 * n + 1);
    partial_sum(b.begin(), b.end(), ps.begin() + 1);
    int high = lg2(k) + 1;  // [0, high]
    auto work = [&] (ll x) -> int {
        vector nxt(2 * n, vector<int>(high + 1, INF));
        vector<bool> oc(n);
        for (int i = 2 * n - 1; ~i; --i) {
            {
                int l = i, r = 2 * n - 1;
            ...
            for (int j = 1; j <= high; ++j) {
                nxt[i][j] = nxt[i][j - 1] >= 2 * n ? INF : nxt[nxt[i][j - 1]][j - 1];
            }
            int dis = k;
            int pos = i;
            int pw = 0;
            while (dis and pos < 2 * n) {
                if (dis & 1) pos = nxt[pos][pw];
                dis >>= 1;
                ++pw;
            }
            // deb(x, i, pos);
            if (dis == 0 and pos - i <= n) {
                oc[i % n] = 1;
            }
        }
        return count(oc.begin(), oc.end(), true);
    };
    ll l = 1, r = INFLL;
    while (l < r) {
        ll mid = l + r + 1 >> 1;
