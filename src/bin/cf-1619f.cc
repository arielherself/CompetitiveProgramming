...
void solve() {
    ...
    int q_cnt = n - p * m;
    int p_cnt = m - q_cnt;
    int p_tot = p * p_cnt, q_tot = q * q_cnt;
    set<pii> cnt;
    for (int i = 1; i <= n; ++i) {
        cnt.emplace(0, i);
    }
    while (k--) {
        vector<bool> is_vip(n + 1);
        unordered_set<pii, pair_hash> open;
        for (int i = 0; i < q_tot; ++i) {
            auto [c, u] = *cnt.begin(); cnt.erase(cnt.begin());
            is_vip[u] = 1;
            open.emplace(c + 1, u);
        }
        for (auto&& [x, y] : open) cnt.emplace(x, y);
        int ptr = 1;
        for (int i = 0; i < p_cnt; ++i) {
            cout << p << ' ';
            for (int j = 0; j < p; ++j) {
                while (is_vip[ptr]) ++ptr;
                cout << (ptr++) << ' ';
            }
            cout << '\n';
        }
