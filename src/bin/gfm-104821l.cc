...
void solve() {
    ...
    while (i < m) {
        ...
        int nxt;
        {
            ...
            while (l < r) {
                ...
                if (mid == i) {
                    ...
                    auto [cnt1, cnt2, _] = cnt.range_query(i, mid - 1);
                    curr = cnt1 + 2 * cnt2;
                }
                if (curr <= k) {
                    l = mid;
            ...
            if (l != i) {
                auto [cnt1, cnt2, _] = cnt.range_query(i, l - 1);
                rem -= cnt1 + 2 * cnt2;
            }
            nxt = l;
        }
        if (nxt == m) break;
        {
            auto [cnt1, cnt2, f] = cnt.query(nxt);
            // use even
            {
            ...
            cnt.set(nxt, { cnt1, cnt2, f });
        }
        if (rem) {
            assert(rem == 1);
            if (ones.size()) {
                auto [f, cnt0] = *ones.rbegin();
