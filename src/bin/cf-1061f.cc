...
// determine if `v` is on the path from `s` to `t`
bool ask(int s, int t, int v) {
    cout << "? " << s << ' ' << v << ' ' << t << endl;
    read(string, x);
    return x == "Yes";
}
...
void solve() {
    read(int, n, k);
    int max_depth = log(n) / log(k);

    auto path_size = [&] (int u, int v) -> int {
        int res = 0;
        for (int i = 1; i <= n; ++i) {
            if (u == i or v == i or ask(u, v, i)) {
                ++res;
            }
        }
        return res;
    ...
    auto work = [&] (int s, int t) -> optional<tuple<int, int, vector<int>>> {
        for (int i = 1; i <= n; ++i) {
            if (ask(s, i, t)) {
                t = i;
            }
        ...
        vector<int> path;
        for (int i = 1; i <= n; ++i) {
            if (s == i or t == i or ask(s, t, i)) {
                path.emplace_back(i);
            }
        }
        int dis = path.size() - 1;
        if (dis == max_depth * 2) {
            return make_tuple(s, t, path);
        } else {
            return nullopt;
        }
    };
    ...
    while (1) {
        ...
        if (oc.count(minmax(s, t))) continue;
        oc.emplace(minmax(s, t));
        auto curr = work(s, t);
        if (curr.has_value()) {
            vector<int> path;
            tie(s, t, path) = curr.value();
            for (auto&& v : path) {
                if (path_size(s, v) - 1 == max_depth) {
                    cout << "! " << v << endl;
                    return;
                }
            }
