...
void solve() {
    ...
    for (int i = 1; i <= N; ++i) {
        vector<int> vs;
        auto add = [&] (int i) { if (oc[i] == 0) oc[i] = 1, vs.emplace_back(i); };
        auto del = [&] (int i) { oc[i] = 0; };
        for (int j = i; j <= N; j += i) {
            for (auto&& [u, v] : edges[j]) {
                add(u), add(v);
            }
            for (auto&& v : vertices[j]) {
                add(v);
            }
        }
        int m = vs.size();
        ...
        for (int j = 0; j < m; ++j) {
            if (qu.query(j) == j) {
                f[i] += ll(1) * qu.query_size(j) * (qu.query_size(j) + 1) / 2;
            }
        }
    }
    for (int i = N; i; --i) {
