    ...
    while (cin >> c) {
        ...
        if (!cnt) {
            if (c == '(') {
                ...
                if (curr) events.emplace_back(curr, 1);
                curr = c;
            }
        } else {
            if (c == ')') {
                events.emplace_back(curr, curr_cnt);
                cnt = false;
                curr = 0;
            } else {
                curr_cnt = curr_cnt * 10 + c - 48;
            }
        }
    }
    if (curr) events.emplace_back(curr, 1);
    ll prev_cnt = 0;
    unordered_set<char, safe_hash> prev_types;
    ll res = 0;
    for (auto&& [c, cc] : events) {
        // debug(prev_cnt), debug(cc), debugvec(prev_types);
        int if_add = !prev_types.count(c);
        if (prev_cnt + cc >= (k + prev_types.size() + if_add - 1) / (prev_types.size() + if_add)) {
            res += 1;
            ll use = max(size_t(if_add), (k + prev_types.size() + if_add - 1) / (prev_types.size() + if_add) - prev_cnt);
            // debug(use);
            prev_types.clear();
            prev_cnt = 0;
            cc -= use;
