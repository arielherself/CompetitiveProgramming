...
void solve() {
	...
	for (int i = 0; i < n; ++i) {
		int curr = i + 1 - a[i];
		need[i] = curr >= 0 and curr % 2 == 0 ? curr / 2 : INF;
	}
	...
	for (int i = n - 1; ~i; --i) {
		for (int j = i; j < n; ++j) {
			if ((j - i + 1) % 2 == 0) {
				// remove all
				if (i + 1 == j or dp[i + 1][j - 1] <= need[i]) {
					dp[i][j] = need[i];
				}
				// split
				for (int k = i + 1; k < j; k += 2) {
					chmin(dp[i][j], max(dp[i][k], dp[k + 1][j] - (k - i + 1) / 2));
					// if (dp[k + 1][j] <= dp[i][k] + (k - i + 1) / 2) {
					// 	chmin(dp[i][j], dp[i][k]);
					// }
				}
