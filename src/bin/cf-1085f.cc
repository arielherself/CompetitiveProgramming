...
void solve() {
    read(int, n, q);
    unordered_map<char, int, safe_hash> mp = {
        { 'R', 0 },
        { 'P', 1 },
        { 'S', 2 },
    };
    vector<int> a(n);
    array<set<int>, 3> bk;
    auto tr = initarray((segtree<Info, Tag>(n)), 3);
    for (int i = 0; i < n; ++i) {
        read(char, x);
        a[i] = mp[x];
        bk[a[i]].emplace(i);
    ...
    auto calc = [&] {
        int res = 0;
        for (int i = 0; i < 3; ++i) {
            if (bk[mod(i - 1, 3)].empty()) {
                if (bk[i].size() == n) {
                    res += n;
                }
            } else {
                int left = *bk[mod(i - 1, 3)].begin();
                int right = *bk[mod(i - 1, 3)].rbegin();
                res += tr[i].range_query(left, right).val;
                {
                    int last = n;
                    if (bk[(i + 1) % 3].size()) {
                        chmin(last, *bk[(i + 1) % 3].begin());
                    }
                    if (bk[(i + 2) % 3].size()) {
                        chmin(last, *bk[(i + 2) % 3].begin());
                    }
                    if (last != 0) {
                        res += tr[i].range_query(0, last - 1).val;
                    }
                }
                {
                    int last = -1;
                    if (bk[(i + 1) % 3].size()) {
                        chmax(last, *bk[(i + 1) % 3].rbegin());
                    }
                    if (bk[(i + 2) % 3].size()) {
                        chmax(last, *bk[(i + 2) % 3].rbegin());
                    }
                    if (last != n - 1) {
                        res += tr[i].range_query(last + 1, n - 1).val;
                    }
                }
            }
        }
