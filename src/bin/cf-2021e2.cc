...
class quick_union {
private:
	vector<size_t> c, sz, node;
public:
	quick_union() = default;
	always_inline quick_union(size_t n) : c(n), sz(n), node(n) {
	...
	always_inline void merge(size_t i, size_t j, size_t no) {
	...
	always_inline size_t query_node(size_t i) {
		return node[query(i)];
	}

	always_inline void set(size_t i, size_t size, size_t no) {
		sz[query(i)] = size;
		node[query(i)] = no;
	}
};
...
/**
 * My thought process
 * 1. Use Kruskal reconstruction tree.
 * 2. Knapsack on tree.
 * 3. Use min-root heap and push up greedily.
 * 4. Kruskal reconstruction tree is a binary tree.
 * 5. Wrong answer since it's not necessary to merge two children.
 * 6. Add more dynamicity.
 * 7. Actually the new version is identical to the old one.
 */
...
void solve() {
	...
	// vector<vector<pii>> ch1(n + 1);
	vector<ll> we(n + 1);

	// auto recalc = [&] (auto self, int v, int pa, int ww) -> pli {
	// 	ll res = mark[v] * ww;
	// 	int sz = 1;
	// 	for (auto&& [u, w] : ch1[v]) {
	// 		if (u == pa) continue;
	// 		auto [curr, subsz] = self(self, u, v, max(ww, w));
	// 		res += curr;
	// 		sz += subsz;
	// 	}
	// 	return { res, sz };
	...
	for (auto&& [u, v, w] : edges) {
		if (not qu.connected(u, v)) {
			if (qu.query_size(u) and qu.query_size(v)) {
				t += 1;
				ch.emplace_back();
				we.emplace_back();
				int uu = qu.query_node(u);
				int vv = qu.query_node(v);
				edge(ch, uu, t);
				edge(ch, vv, t);
				// deb(uu, t);
				// deb(vv, t);
				// int szu = qu.query_size(u);
				// int szv = qu.query_size(v); 
				we[t] = w;
				// we[t] = min(we[uu] + ll(1) * szv * w, we[vv] + ll(1) * szu * w);
				qu.merge(u, v, t);
			} else if (qu.query_size(u)) {
				qu.merge(v, u, qu.query_node(u));
			} else if (qu.query_size(v)) {
				qu.merge(u, v, qu.query_node(v));
			} else {
				qu.merge(u, v, 0);
			}
			// edgew(ch1, u, v, w);
	...
	auto pushup = [&] (int v) {
		if (not fa[v]) return;
		// sub[fa[v]] += we[v];
		chmin(choice[fa[v]], we[fa[v]] * leafcnt[v] - curr[v]);
		sub[fa[v]] += curr[v];
		if (++cnt[fa[v]] == 2) {
			assert(ch[fa[v]].size() <= 3);
			// q.emplace(we[fa[v]] - sub[fa[v]], fa[v]);
			q.emplace(choice[fa[v]], fa[v]);
			curr[fa[v]] = sub[fa[v]] + choice[fa[v]];
		}
	};
	...
	vector<ll> res(n + 1);
	while (q.size()) {
		auto [change, v] = poptop(q);
		pushup(v);
		res[k - 1] = res[k] + change;
		k -= 1;
	}
