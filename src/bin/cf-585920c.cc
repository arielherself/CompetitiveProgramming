...
/**
 * My thought process
 * 3. A common trick is to add number from small to large.
 * 4. For the row from which the number is added, should find a consecutive segment.
 * 5. For the other row, find the left/right bound that there exists an element that ain't be added yet.
 * 6. Use std::set.
 * 7. TLEd.
 * 8. Consider the 2e6 range. Use counting sort.
 */

// __attribute__((target("popcnt")))
void solve() {
	read(int, n);
	auto a = initarray((vector<int>(n)), 2);
	for (int i = 0; i < 2; ++i) {
		for (int j = 0; j < n; ++j) {
			cin >> a[i][j];
		}
	...
	sort_by_key(idx.begin(), idx.end(), expr(a[p.first][p.second], auto&& p));

	array<set<int>, 2> empty;
	for (int i = 0; i < 2; ++i) {
		for (int j = 0; j < n; ++j) {
			empty[i].emplace(j);
		}
	...
	for (auto&& [i, j] : idx) {
		...
		ll curr = ll(1) * a[i][j] * (ll(1) * (r - j) * (j - l) - ll(1) * (j - ols) * (ors - j));
		// if (left >= 1 and ols == j) {
		// 	curr -= a[i][j];
		// }
