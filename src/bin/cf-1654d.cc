...
using facts = unordered_map<int, ll, safe_hash>;

auto decompose(ll x) {
    facts ret;
    while (x > 1) {
        auto p = pollard_rho().solve(x);
        while (x % p == 0) {
            ret[p] += 1;
            x /= p;
        }
...
void multiply(const pair<facts&, facts&>& a, const pair<const facts&, const facts&>& b) {
    for (auto&& [k, v] : b.first) {
        if (a.second.count(k)) {
            ll use = min(a.second[k], v);
            a.second[k] -= use;
            a.first[k] += v - use;
        } else {
            a.first[k] += v;
...
void solve() {
    ...
    mll res = 0;
    facts val1;
    pair<facts, facts> curr;
    auto dfs = [&] (auto&& dfs, int v, int pa, mll prev) -> void {
        res += prev;
        for (auto& [u, x, y] : e[v]) {
            if (u == pa) continue;
            auto p = decompose(x), q = decompose(y);
            multiply({ curr.first, curr.second }, { p, q });
            for (auto&& [k, _] : q) {
                chmax(val1[k], curr.second[k]);
            }
            dfs(dfs, u, v, prev * x / y);
            multiply({ curr.first, curr.second }, { q, p });
        }
    };
    dfs(dfs, 1, 0, 1);

    cout << res * evaluate(val1) << '\n';
}
