...
void solve() {
    read(ll, n, m);
    auto intersect = [&] (const pll& a, const pll& b) -> pll {
        auto [k1, b1] = a;
        auto [k2, b2] = b;
        if (k1 != k2 && (k2 * b1 - k1 * b2) % (k2 - k1) == 0) {
            ll y = (k2 * b1 - k1 * b2) / (k2 - k1);
            ll x = (y - b1) / k1;
            if (x >= 1 && x <= n && y >= 1 && y <= m)
                return {x, y};
            else 
    ...
    cout << "? 1 1" << endl;
    read(ll, t1);
    lines.emplace_back(-1, t1 + 2);
    cout << "? 1 " << m << endl;
    read(ll, t2);
    lines.emplace_back(1, m - t2 - 1);
    auto [x1, y1] = intersect(lines[0], lines[1]);
    if (x1 == -1) {
        cout << "? " << n << " 1" << endl;
        read(ll, t3);
        lines.emplace_back(1, t3 - n + 1);
        auto [x, y] = intersect(lines[0], lines[2]);
        cout << "! " << x << ' ' << y << endl;
    } else {
        cout << "? " << x1 << ' ' << y1 << endl;
        read(ll, t3);
        if (t3 == 0) {
            ...
            ll x2 = x1 + t3 / 2, y2 = y1 - t3 / 2;
            ll x3 = x1 + t3 / 2, y3 = y1 + t3 / 2;
            if (y2 <= 0) {
                cout << "! " << x3 << ' ' << y3 << endl;
            } else if (y3 > m) {
                cout << "! " << x2 << ' ' << y2 << endl;
