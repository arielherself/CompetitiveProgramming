...
void solve() {
    ...
    sparse_table<ll> max_rmq(ps.rbegin(), ps.rend(), functor(max));

    vector<int> stack;
    vector<int> prev(n - 1);
    for (int i = 0; i < n - 1; ++i) {
        while (stack.size() and d[stack.back()] < d[i]) {
            stack.pop_back();
        }
        prev[i] = stack.size() ? stack.back() : -1;
    ...
    for (int i = n - 2; ~i; --i) {
        ...
        next[i] = stack.size() ? stack.back() : n - 1;
        stack.emplace_back(i);
    }

    ll res = *min_element(a.begin(), a.end());
    for (int i = 0; i < n - 1; ++i) {
        ll curr = ll(1) * d[i] * d[i];
        curr += min_rmq.query(i + 2, next[i] + 1);
        curr -= max_rmq.query(prev[i] + 1, i);
        chmin(res, curr);
    }
