...
// #define DUMP_TEST_CASE 15
// #define TOT_TEST_CASE 50
...
void solve() {
	read(int, l, n, m);
	readvec(int, s, l);
	transform(s.begin(), s.end(), s.begin(), expr(x - 1, auto&& x));
	...
	vector<int> oc(s.begin(), s.end());
	sort(oc.begin(), oc.end());
	int k = unique(oc.begin(), oc.end()) - oc.begin();
	oc.resize(k);
	vector mp(n * m, -1);
	for (int i = 0; i < k; ++i) {
		mp[oc[i]] = i;
