...
void solve() {
    ...
    vector<int> dp(n + 1);
    {
        auto dfs = [&] (auto dfs, int v, int pa) -> void {
            ...
            b.range_apply(dfn[v], dfn[v] + sz[v] - 1, { subsum });
            dp[v] = subsum;
            for (auto&& [u1, u2, w] : e[v]) {
                if (u2 == v) swap(u1, u2);
                if (u1 == v) {
                    chmax(dp[v], w + (b.query(dfn[u2]).val - b.query(dfn[v]).val + subsum) - (a.query(dfn[u2]).val - a.query(dfn[v]).val));
                } else {
                    chmax(dp[v], w + (b.query(dfn[u2]).val + b.query(dfn[u1]).val - 2 * b.query(dfn[v]).val + subsum) - (a.query(dfn[u2]).val + a.query(dfn[u1]).val - 2 * a.query(dfn[v]).val));
                }
            }
            a.range_apply(dfn[v], dfn[v] + sz[v] - 1, { dp[v] });
        };
        dfs(dfs, 1, 0);
    }
