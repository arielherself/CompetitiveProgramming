...
void solve() {
	...
	for (int i = 0; i < n; ++i) {
		for (int j = 0; j < m; ++j) {
			a[i][j] = ({ read(int, x); x == -1 ? x : x - 1; });
		}
	}

	segtree<Info, Tag> tr(k);
	vector<unordered_map<int, int, safe_hash>> cnt(n);
	vector<int> empty(n);
	...
	for (int i = 0; i < n; ++i) {
		...
		if (i) {
			for (auto&& [x, c] : cnt[i]) {
				...
				tr.apply(x, {
						.add = ll(1) * empty[i - 1] * c,
						.mx = 0,
						});
			}
		}

		tr.range_apply(0, k - 1, {
				.add = e,
				.mx = 0,
				});

		ll oldmax = tr.range_query(0, k - 1).val;

		if (i) {
			tr.range_apply(0, k - 1, {
					.add = ll(1) * empty[i] * empty[i - 1],
					.mx = 0,
					});
		}
