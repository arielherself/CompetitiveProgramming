...
void solve() {
    ...
    auto dfs = [&] (auto dfs, int v) -> pii {
        int cnt = 0;
        int sz = 0;
        int m = ch[v].size();
        vector<int> max_pairs(m), rem(m);
        int ptr = 0;
        for (int i = 0; i < m; ++i) {
            auto [x, y] = dfs(dfs, ch[v][i]);
            max_pairs[i] = x;
            sz += y;
            while (ptr < i && y) {
                int use = min(rem[ptr], y);
                y -= use;
                rem[ptr] -= use;
                cnt += use;
                if (!rem[ptr]) ++ptr;
        ...
        for (int i = 0; i < m; ++i) {
            cnt += min(max_pairs[i], rem[i] / 2);
        }
        sz += 1;
        debug(v), debug(cnt), debug(sz);
        return {cnt, sz};
    };
