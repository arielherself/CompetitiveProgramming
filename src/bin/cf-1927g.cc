...
void solve() {
	...
	for (int i = 1; i <= n; ++i) {
		dp[i] = dp[i - 1];
		// use current as suffix
		{
			int mn = i - 1;
			for (int j = i + 1; j < min(n + 1, i + a[i - 1]); ++j) {
				chmin(mn, j - a[j - 1]);
				// use mid
				chmin(dp[j][min(n, i + a[i - 1] - 1)], dp[i - 1][max(0, mn)] + 2);
			}
			// don't use mid
			chmin(dp[i][min(n, i + a[i - 1] - 1)], dp[i - 1][i - 1] + 1);
		}
		// use current as prefix
		{
			int mx = i;
			for (int j = i - 1; j > max(0, i - a[i - 1]); --j) {
				chmax(mx, j + a[j - 1] - 1);
				// use mid
				chmin(dp[i][min(n, mx)], dp[j - 1][max(0, i - a[i - 1])] + 2);
			}
			// don't use mid
			chmin(dp[i][i], dp[i - 1][max(0, i - a[i - 1])] + 1);
		}
		// suffix sum
		for (int j = n - 1; ~j; --j) {
