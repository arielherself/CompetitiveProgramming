...
void solve() {
	...
	auto vv = [&] (int i, int j) { return i >= 0 and i < N and j >= 0 and j < M; };

	while (q.size()) {
		auto [i, j] = popfront(q);
		for (auto&& [i1, j1] : {pii{i - 1, j}, pii{i + 1, j}, pii{i, j - 1}, pii{i, j + 1}}) {
			if (vv(i1, j1) and chmin(dis[i1][j1], dis[i][j] + 1)) {
	...
	while (Q--) {
		...
		auto check = [&] (int x) -> bool {
			...
			for (int i = 0; i < N; ++i) {
				for (int j = 0; j < M; ++j) {
					if (dis[i][j] >= x and A[i][j] != '#') {
						if (i - 1 >= 0 and dis[i - 1][j] >= x and A[i - 1][j] != '#') {
							qu.merge(i * M + j, (i - 1) * M + j);
						}
						if (j - 1 >= 0 and dis[i][j - 1] >= x and A[i][j - 1] != '#') {
			...
			vector<bool> vis(N * M);
			deque<pii> q = { start };
			vis[start.first * M + start.second] = 1;
			while (q.size()) {
				auto [i, j] = popfront(q);
				if (i == 0 or j == 0 or i == N - 1 or j == M - 1) return false;
				for (auto&& [i1, j1] : {pii{i - 1, j}, pii{i + 1, j}, pii{i, j - 1}, pii{i, j + 1}, pii{i - 1, j - 1}, pii{i - 1, j + 1}, pii{i + 1, j - 1}, pii{i + 1, j + 1}}) {
					if (vv(i1, j1) and (A[i1][j1] == '#' or not qu.connected(i1 * M + j1, X * M + Y)) and not vis[i1 * M + j1]) {
						vis[i1 * M + j1] = 1;
						q.emplace_back(i1, j1);
					}
				}
