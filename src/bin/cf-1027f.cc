...
void solve() {
    ...
    vector<int> st;
    for (int i = 0; i < n; ++i) {
        st.emplace_back(a[i].first);
        st.emplace_back(a[i].second);
    }
    sort(st.begin(), st.end());
    int m = unique(st.begin(), st.end()) - st.begin();
    vector<pii> d(n);
    for (int i = 0; i < n; ++i) {
        d[i].first = lower_bound(st.begin(), st.begin() + m, a[i].first) - st.begin();
        d[i].second = lower_bound(st.begin(), st.begin() + m, a[i].second) - st.begin();
    }
    adj(ch, m);
    vector<int> ind(m);
    for (int i = 0; i < n; ++i) {
        Edge(ch, d[i].first, d[i].second);
        // deb(m, d[i].second);
        ++ind[d[i].second];
    }
    vector<int> init;
    ...
    for (auto&& v : seq) {
        ...
        int target = -1;
        for (auto&& u : ch[v]) {
            if (target == -1 or dp[u][0] > dp[target][0]) {
                target = u;
            }
            // oc.emplace(dp[u][0]);
        }
        dp[v][1] = target == -1 ? 0 : dp[target][0];
        if (target != -1) {
            int target_cnt = 0;
            int nxt = max(st[v], dp[target][1]);
            int sec = 0;
            for (auto&& u : ch[v]) {
                if (u != target) {
                    if (oc[u]) {
        ...
        // for (auto&& u : ch[v]) {
        //     oc.erase(oc.lower_bound(dp[u][0]));
        //     chmin(dp[v][1], max(dp[u][1], max(st[v], oc.empty() ? 0 : *oc.rbegin())));
        //     oc.emplace(dp[u][0]);
        // }
        // not choose v
        dp[v][0] = st[v];
