...
void solve() {
    ...
    for (int i = 0; i < m; ++i) {
        ...
        read(int, x);
        read(ll, y);
        bk.emplace_back(t, x - 1, y);
    }
    sort(bk.begin(), bk.end(), greater());
    vector<vector<tlil>> events(N + 1);
    vector<int> cnt(N + 1);
    ll l = 0, r = INFLL;
    while (l < r) {
        ll mid = l + r + 1 >> 1;
        ...
        if (f) {
            ...
            for (auto&& [t, i, d0] : bk) {
                ll t0 = t;
                ll x;
                if (time < t0 or d0 - int128(1) * (time - t0 + 1) * k >= target) {
                    x = 0;
                } else {
                    int128 curr = (target - d0 + int128(1) * (time - t0 + 1) * k + k - 1) / k;
                    if (curr > time) {
                        f = false;
                        break;
                    }
                ...
                ss[i] = x;
                if (ss[i] > used[i] and not added[i]) {
                    oc[oc_sz++] = i;
                    added[i] = 1;
                }
                avail += nxt - t;
                while (oc_sz and avail) {
                    int j = oc[oc_sz - 1];
                    ll use = min(avail, ss[j] - used[j]);
                    avail -= use;
                    used[j] += use;
                    if (used[j] == ss[j]) {
                        --oc_sz;
