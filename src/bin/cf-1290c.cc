...
private:
    ...
    vector<int> choice;
public:
    quick_union(size_t n) : c(n), sz(n), choice(n, -1) {
    ...
    void merge(size_t i, size_t j) {
        ...
        chmax(choice[query(j)], choice[query(i)]);
        c[query(i)] = query(j);
    }
    ...
    int query_choice(size_t i) {
        return choice[query(i)];
    }
    void set_choice(size_t i, int val) {
        choice[query(i)] = val;
    }
};
...
void solve() {
    ...
    for (int i = 0; i < n; ++i) {
        if (bk[i].empty()) {
            ...
            int x = bk[i][0];
            if (init[i] == 0) {
                if (qu.query_choice(k + x) == -1) {
                    curr += qu.query_size(k + x);
                } else if (qu.query_choice(k + x) == 0) {
                    curr += qu.query_size(k + x) - qu.query_size(x);
                }
                qu.set_choice(k + x, 3);
                qu.set_choice(x, 2);
            } else {
                if (qu.query_choice(x) == -1) {
                    curr += qu.query_size(x);
                } else if (qu.query_choice(x) == 0) {
                    curr += qu.query_size(x) - qu.query_size(k + x);
                }
                qu.set_choice(x, 3);
                qu.set_choice(k + x, 2);
            }
            ...
            int f = 1;
            for (auto&& w : { x, k + x, y, k + y }) {
                if (qu.query_choice(w) == 3) {
                    f = 0;
                    break;
                }
            }
            ...
            for (auto&& w : { x, k + x, y, k + y }) {
                st.emplace(qu.query(w));
            }
            if (f == 0) {
                for (auto&& w : st) {
                    if (qu.query_choice(w) != -1) {
                        if (qu.query_choice(w) == 3 or qu.query_choice(w) == 1) {
                            curr -= qu.query_size(w);
                        }
                    }
                }
                if (init[i] == 0) {
                    qu.merge(x, k + y);
                    qu.merge(y, k + x);
                    if (qu.query_choice(x) == 3) {
                        curr += qu.query_size(x);
                        qu.set_choice(y, 2);
                    } else {
                        curr += qu.query_size(y);
                        qu.set_choice(x, 2);
                    }
                } else {
                    qu.merge(x, y);
                ...
                if (init[i] == 0) {
                    qu.merge(x, k + y);
                    qu.merge(y, k + x);
                    if (qu.query_size(x) < qu.query_size(y)) {
                        qu.set_choice(x, 1);
                        qu.set_choice(y, 0);
                        curr += qu.query_size(x);
                    } else {
                    ...
                    qu.merge(x, y);
                    qu.merge(k + x, k + y);
                    if (qu.query_size(x) < qu.query_size(k + x)) {
                        qu.set_choice(x, 1);
                        qu.set_choice(k + x, 0);
                        curr += qu.query_size(x);
                    } else {
