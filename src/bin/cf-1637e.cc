/**
 * Author:   subcrip
 * Created:  2024-05-30 23:34:35
 * Modified: 2024-05-30 23:56:21
 * Elapsed:  21 minutes
 */
...
void solve() {
    ...
    for (auto&& [x, t] : ct) cnt[t].emplace_back(x);
    vector<pair<int, vector<ll>>> bk;
    for (auto&& [t, v] : cnt) bk.emplace_back(t, v);
    for (auto&& [t, v] : bk) {
        sort(v.begin(), v.end(), greater());
    }
    int u = bk.size();
    unordered_set<pii, pair_hash> ps;
    for (int i = 0; i < m; ++i) {
        read(int, x, y);
        ps.emplace(minmax(x, y));
    }
    ll res = 0;
    for (int i = 0; i < u; ++i) {
        for (int j = 0; j < i; ++j) {
            auto cmp = [&] (const pii& a, const pii& b) {
                return bk[i].second[a.first] + bk[j].second[a.second] < bk[i].second[b.first] + bk[j].second[b.second];
            };
            ...
            while (pq.size()) {
                ...
                vis.emplace(u, v);
                if (not ps.count(minmax(bk[i].second[u], bk[j].second[v]))) {
                    res = max(res, (bk[i].first + bk[j].first) * (bk[i].second[u] + bk[j].second[v]));
                    break;
                }
                if (u + 1 < bk[i].second.size()) pq.emplace(u + 1, v);
                if (v + 1 < bk[j].second.size()) pq.emplace(u, v + 1);
            }
        }
    }
    // same
    for (int i = 0; i < u; ++i) {
        auto cmp = [&] (const pii& a, const pii& b) {
            return bk[i].second[a.first] + bk[i].second[a.second] < bk[i].second[b.first] + bk[i].second[b.second];
        };
        unordered_set<pii, pair_hash> vis;
        priority_queue<pii, vector<pii>, decltype(cmp)> pq(cmp);
        pq.emplace(0, 0);
        while (pq.size()) {
            ...
            if (vis.count({u, v})) continue;
            vis.emplace(u, v);
            if (u != v and not ps.count(minmax(bk[i].second[u], bk[i].second[v]))) {
                res = max(res, (bk[i].first + bk[i].first) * (bk[i].second[u] + bk[i].second[v]));
                break;
            }
            if (u + 1 < bk[i].second.size()) pq.emplace(u + 1, v);
            if (v + 1 < bk[i].second.size()) pq.emplace(u, v + 1);
        }
    }
    cout << res << '\n';
}
