...
void solve() {
    ...
    vector<int> rev(n + 1);
    for (int i = 0; i < m; ++i) {
        sort_by_key(bk[i].begin(), bk[i].end(), expr(dfn[v], auto v));
        int m = bk[i].size();
        vector<int> pivot;
        for (int j = 1; j < m; ++j) {
            pivot.emplace_back(bk[i][j - 1]);
            pivot.emplace_back(lca.query(bk[i][j - 1], bk[i][j]));
        }
        pivot.emplace_back(bk[i][m - 1]);
        sort_by_key(pivot.begin(), pivot.end(), expr(dfn[v], auto v));
        m = unique(pivot.begin(), pivot.end()) - pivot.begin();
        pivot.resize(m);
        for (int j = 0; j < m; ++j) {
            rev[pivot[j]] = j;
        }
        ...
        adj(ch, m - 1);
        for (int j = 1; j < m; ++j) {
            int l = rev[lca.query(pivot[j - 1], pivot[j])];
            int v = j;
            // deb(v, l, lca.query(pivot[j - 1], pivot[j]));
            edge(ch, v, l);
        }
        {
            auto dfs = [&] (auto dfs, int v, int pa) -> int {
                int curr = a[pivot[v]] == oc[i];
                for (auto&& u : ch[v]) {
                    if (u == pa) continue;
                    curr += dfs(dfs, u, v);
                }
                if (curr >= k) {
                    chmax(diff[pivot[v]], oc[i]);
                }
                return curr;
            };
            dfs(dfs, 0, 0);
