...
void solve() {
	...
	vector<bool> has(n);
	for (int t = 0; t < m; ++t) {
		sort_by_key(events[t].begin(), events[t].end(), expr(a[i].second, auto&& i));
		for (auto&& i : events[t]) {
			auto it = q.suffix_binary_search(expr(x.val >= a[i].first, auto&& x));
			q.insert(it, {
					.val = a[i].first,
					.id = i,
					});
		...
		auto curr = q.pop_front();
		if (not has[curr.id]) {
			has[curr.id] = true;
			cnt += 1;
		}
		if (t + a[curr.id].second + 1 <= m) {
			events[t + a[curr.id].second + 1].emplace_back(curr.id);
		}
