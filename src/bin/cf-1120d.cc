...
public:
    quick_union(size_t n) : c(n), sz(n), bk(n) {
    ...
    void merge(size_t i, size_t j) {
        ...
        i = query(i), j = query(j);
        if (bk[i].size() > bk[j].size()) swap(i, j);
        bk[j].insert(bk[j].end(), bk[i].begin(), bk[i].end());
        sz[j] += sz[i];
        c[i] = j;
    }
    ...
    int leaf_cnt = 0;
    {
        auto dfs = [&] (auto dfs, int v, int pa) -> void {
            ...
            for (auto&& u : ch[v]) {
                if (u == pa) continue;
                dfs(dfs, u, v);
                chmin(range[v].first, range[u].first);
                chmax(range[v].second, range[u].second);
            }
        };
        dfs(dfs, 1, 0);
    }

    map<int, vector<tiii>> edges;
    for (int i = 1; i <= n; ++i) {
        edges[c[i]].emplace_back(range[i].first, range[i].second + 1, i);
    }
    quick_union qu(leaf_cnt + 2);
    vector<int> cand;
    ll tot = 0;
    for (auto&& [l, vec] : edges) {
        for (auto&& [u, v, i] : vec) {
