...
void solve() {
    ...
    for (int j = 1; j < n; ++j) {
        if (j % 2) {
            a[1][j] = 1 ^ ask(0, j - 1, 1, j) ^ a[0][j - 1];
            if (j != 1) {
                a[0][j] = 1 ^ ask(0, j - 2, 0, j) ^ a[0][j - 2];
            }
            for (int i = 2; i < n; ++i) {
                a[i][j] = 1 ^ ask(i - 2, j, i, j) ^ a[i - 2][j];
            }
        } else {
            a[0][j] = 1 ^ ask(0, j - 2, 0, j) ^ a[0][j - 2];
            a[1][j] = 1 ^ ask(0, j - 1, 1, j) ^ a[0][j - 1];
    ...
    for (int i = 1; i < n; ++i) {
        if (i % 2) {
            a[i][0] = 1 ^ ask(i, 0, i + 1, 1) ^ a[i + 1][1];
        } else {
            a[i][0] = 1 ^ ask(i - 2, 0, i, 0) ^ a[i - 2][0];
        }
    }
    bool flip = false;
    for (int i = 0; i < n - 1; ++i) {
        for (int j = 0; j < n - 1; ++j) {
            if (a[i][j + 1] != a[i + 1][j]) {
                if (i == 0) {
                    flip = ask(i, j, i + 2, j + 1) != (a[i][j] == a[i + 2][j + 1]);
                } else {
                    flip = ask(i - 1, j, i + 1, j + 1) != (a[i - 1][j] == a[i + 1][j + 1]);
                }
                goto fi;
            }
        }
    ...
    for (int i = 0; i < n - 2; ++i) {
        for (int j = 0; j < n - 1; ++j) {
            if ((a[i][j] == a[i + 2][j]) == (a[i][j + 1] == a[i + 2][j + 1])) {
                flip = ask(i, j, i + 2, j + 1) != (a[i][j] == a[i + 2][j + 1]);
                goto fi;
            }
        }
