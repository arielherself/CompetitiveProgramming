...
int main() {
    ...
    int need_to_delete = n - 1 - q;
    vector<vector<int>> res(n + 1, vector<int>(need_to_delete + 1));
    for (int i = 0; i < n - 1; ++i) {
        read(int, u, v, w);
    ...
    vector<int> cnt(n + 1);
    auto count = [&] (auto count, int v, int pa) -> void {
        for (auto&& [u, _] : e[v]) {
            if (u == pa) continue;
            count(count, u, v);
            cnt[v] += cnt[u] + 1;
    ...
    auto dfs = [&] (auto dfs, int v, int pa) -> void {
        if (e[v].size() == 1) return;
        vector<int> chs, ws;
        for (auto&& [u, w] : e[v]) {
            if (u == pa) continue;
            dfs(dfs, u, v);
            chs.push_back(u), ws.push_back(w);
        }
        // don't delete
        for (int i = 0; i <= need_to_delete; ++i) {
            for (int j = 0; j <= i; ++j) {
                if (j > cnt[chs[0]] || i - j > cnt[chs[1]]) continue;
                res[v][i] = max(res[v][i], res[chs[0]][j] + res[chs[1]][i-j] + ws[0] + ws[1]);
            }
        }
        // delete lson
        for (int i = 1; i <= need_to_delete - cnt[chs[0]]; ++i) {
            if (i - 1 > cnt[chs[1]]) break;
            res[v][cnt[chs[0]] + i] = max(res[v][cnt[chs[0]] + i], res[chs[1]][i-1] + ws[1]);
        }
        // delete rson
        for (int i = 1; i <= need_to_delete - cnt[chs[1]]; ++i) {
            if (i - 1 > cnt[chs[0]]) break;
            res[v][cnt[chs[1]] + i] = max(res[v][cnt[chs[1]] + i], res[chs[0]][i-1] + ws[0]);
        }
        // delete everything is not 0
    };
    dfs(dfs, 1, 0);
    // debug(res[72]);
    // debug(res[39]);
    // debug(res[1]);
    cout << res[1][need_to_delete] << endl;
}