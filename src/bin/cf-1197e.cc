...
Info operator+(const Info &a, const Info &b) {
    return {
        ...
        .cnt = a.val == b.val ? a.cnt + b.cnt : a.val < b.val ? a.cnt : b.cnt,
    };
}
...
void solve() {
    ...
    for (int i = 0; i < n; ++i) {
        ...
        oc.emplace(a[i].first);
        oc.emplace(a[i].second);
    }
    unordered_map<int, int, safe_hash> mp;
    vector<int> rev(1);
    int N = 0;
    for (auto&& x : oc) mp[x] = ++N, rev.emplace_back(x);
    sort(a.begin(), a.end());
    segtree<Info, Tag> tr(N + 1);
    int prev_left = -1;  // exclusive
    vector<vector<int>> bk(N + 1), v(N + 1);
    for (int i = 0; i < n; ++i) {
        bk[mp[a[i].second]].emplace_back(mp[a[i].first]);
        v[mp[a[i].first]].emplace_back(mp[a[i].second]);
    }
    Info res;
    for (int i = 0; i <= N; ++i) {
        for (auto&& x : bk[i]) {
        ...
        for (auto&& r : v[i]) {
            auto [val, cnt] = tr.range_query(prev_left + 1, i);
            if (val == INFLL) {
                tr.set(r, Info { rev[i] - rev[r], 1 } + tr.query(r));
            } else {
                tr.set(r, Info { val + rev[i] - rev[r], cnt } + tr.query(r));
            }
        }
        if (i > mp[a[n - 1].first]) {
            auto [val, cnt] = tr.query(i);
            if (val != INFLL) {
                res = res + Info { val + rev[i], cnt };
            }
        }
    }
    cout << res.cnt << '\n';
