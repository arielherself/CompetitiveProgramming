...
public:
    quick_union(size_t n) : c(n), sz(n), edges(n) {
    ...
    void merge(size_t i, size_t j) {
        ...
        if (edges[query(i)].size() > edges[query(j)].size()) swap(i, j);
        edges[query(j)].merge(edges[query(i)]);
    ...
    for (int j = 0; j < m; ++j) {
        ...
        for (int i = 0; i < n; ++i) {
            if (j and a[i][j - 1] == a[i][j]) {
                if (qu.connected(i * m + j - 1, i * m + j)) {
                    assert(i > 0);
                    auto it = prev(prev(qu.edges[qu.query(i * m + j)].lower_bound({ i, 0 })));
                    auto [x, y] = *it;
                    // deb(j, i, y);
                    qu.edges[qu.query(i * m + j)].erase(it);
                    assert(x == i - 1);
                    tr.apply(y, { 1 });
                } else {
                    qu.merge(i * m + j - 1, i * m + j);
                }
                ps[j + 1] += 1;
            }
            if (i + 1 < n and a[i + 1][j] == a[i][j]) {
                cnt -= 1;
                qu.edges[qu.query(i * m + j)].emplace(i, j);
                if (qu.connected((i + 1) * m + j, i * m + j)) {
                    assert(false);
                } else {
                    qu.merge((i + 1) * m + j, i * m + j);
                }
            }
        }
        // deb(j, cnt);
        ...
        // cerr << endl;
        // debug(ps[j + 1] - ps[j]);
        for (auto&& [l, idx] : queries[j]) {
            res[idx] = tr.range_query(l, j).val - (ps[j + 1] - ps[l + 1]);
        }
    }
