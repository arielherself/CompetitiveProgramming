...
private:
    ...
    ll binary_search(size_type s, size_type t, size_type p, int cnt) {
        if (s == t) {
            ...
            ll single = d[p].val / d[p].cnt;
            return min(d[p].cnt, cnt) * single;
        }
        size_type m = s + (t - s >> 1);
        push(p, m - s + 1, t - m);
        if (d[p * 2].cnt >= cnt) return binary_search(s, m, p * 2, cnt);
        else return d[p * 2].val + binary_search(m + 1, t, p * 2 + 1, cnt - d[p * 2].cnt);
    }
...
void solve() {
    ...
    vector<int> oc(n);
    transform(t.begin(), t.end(), oc.begin(), expr((x + 1) / 2 - x, auto&& x));
    ...
    for (int i = n - 1; ~i; --i) {
        ...
        while (1) {
            ll time = tot_time + tr.binary_search(k);
            if (time > tot) {
                val -= a[r];
                tot_time -= t[r];
                int curr_d = (t[r] + 1) / 2 - t[r];
                tr.apply(get(curr_d), {
                        .cnt = -1,
                        .val = -curr_d,
                        });
