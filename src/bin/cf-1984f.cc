...
// #define DUMP_TEST_CASE 7
// #define TOT_TEST_CASE 500
...
void solve() {
	...
	// SSSSPPPP
	for (int i = 0; i <= n; ++i) {
		// The first i elements are S's, and the rest are P's.
		// Check if this situation is possible.
		int f = 1;
		ll all = i ? a[0] : a[n - 1];
		int prev = 0;
		ll prevsum = 0;
		for (int j = 0; j < n; ++j) {
			...
			assert(curr >= prev);
			if (abs(currsum - prevsum) > ll(1) * m * (curr - prev)) {
				f = 0;
				break;
			}
			prev = curr;
	...
	for (int i = 1; i < n; ++i) {
		...
		for (int j = 0; j < n; ++j) {
			...
			for (int k = 0; k < 2; ++k) {
				...
				for (int l = 0; l < 2; ++l) {
					char currchar = l == 0 ? 'P' : 'S';
					if (prevchar == 'P' and currchar == 'S' and j < i) {
						continue;
					}
					if (s[j] != '?' and s[j] != currchar) {
						continue;
					}
					if (j == i and currchar != 'S') {
						continue;
					}
					if (j == i - 1 and currchar != 'P') {
						continue;
					}
