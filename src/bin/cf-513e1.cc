...
void solve() {
	...
	vector dp(n, vector(k + 1, vector<int>(2, -INF)));
	int res = -INF;
	for (int i = 0; i < n; ++i) {
		// s1 end
		for (int j = i; ~j; --j) {
			chmax(dp[i][1][0], -(ps[i + 1] - ps[j]));
			chmax(dp[i][1][1], ps[i + 1] - ps[j]);
		}

		for (int j = 0; j < i; ++j) {
			for (int l = 2; l <= k; ++l) {
				int curr = max(dp[j][l - 1][0] + (ps[i + 1] - ps[j + 1]), dp[j][l - 1][1] - (ps[i + 1] - ps[j + 1]));
				chmax(dp[i][l][0], curr - (ps[i + 1] - ps[j + 1]));
				chmax(dp[i][l][1], curr + (ps[i + 1] - ps[j + 1]));
