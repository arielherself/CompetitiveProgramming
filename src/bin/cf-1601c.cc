...
void solve() {
    ...
    // optimal placement
    for (int i = 0; i < m; ++i) {
        while (st.size() and st.begin()->first > b[i]) {
            st.erase(st.begin());
        }
        debug(st.begin()->second);
        seq[st.begin()->second].emplace_back(b[i]);
    }
    vector<int> c;
    sort(seq[0].begin(), seq[0].end());
    c.insert(c.end(), seq[0].begin(), seq[0].end());
    for (int i = 1; i <= n; ++i) {
        c.emplace_back(a[i - 1]);
        sort(seq[i].begin(), seq[i].end());
        c.insert(c.end(), seq[i].begin(), seq[i].end());
    }

    // calculate the final result
    auto [N, M] = unordered_discretize<int>(c.begin(), c.end());
    BIT<int> bt(N);
    int res = 0;
    for (int i = 0; i < n + m; ++i) {
        res += bt.getsum(N) - bt.getsum(M[c[i]]);
        bt.add(M[c[i]], 1);
    }
