...
void solve() {
    ...
    for (int i = 0; i < n; ++i) {
        if (type_cnt + 1 == ask2(1, i + 1)) {
            type[i] = type_cnt++;
            prev[type_cnt - 1] = i;
        } else {
            // binary search for the type of current character
            vector<pii> cand;
            for (int j = 0; j < 26; ++j) {
                if (prev[j] != -1) cand.emplace_back(prev[j], j);
            }
            sort(cand.begin(), cand.end());
            int l = 0, r = type_cnt - 1;
            while (l < r) {
                int mid = l + r + 1 >> 1;
                if (ask2(cand[mid].first + 1, i + 1) == type_cnt - mid) {
                    l = mid;
                } else {
                    r = mid - 1;
                }
            }
            type[i] = cand[l].second;
            prev[cand[l].second] = i;
        }
    }
    string s(n, 0);
    for (int i = 0; i < type_cnt; ++i) {
        char c = ask1(1 + prev[i]);
        for (int j = 0; j < n; ++j) {
            if (type[j] == i) {
                s[j] = c;
            }
