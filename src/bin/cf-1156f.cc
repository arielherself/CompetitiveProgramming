...
void solve() {
    ...
    dp[0][0] = 1;
    partial_sum(dp[0].begin(), dp[0].end(), pp[0].begin());
    mll res = 0;
    for (int i = 1; i <= n; ++i) {
        for (int j = 1; j <= n; ++j) {
            dp[i][j] = mll(1) * a[j] * inv[n - i + 1] * pp[i - 1][j - 1];
            res += dp[i][j] * (a[j] - 1) * inv[n - i];
        }
        partial_sum(dp[i].begin(), dp[i].end(), pp[i].begin());
    }
