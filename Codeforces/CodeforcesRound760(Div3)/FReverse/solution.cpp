...
string remove_leading(const string& a) {
    int p = 0;
    while (a[p] == '0') ++p;
    return a.substr(p, a.size() - p);
}

string remove_trailing(const string& a) {
    int q = a.size() - 1;
    while (a[q] == '0') --q;
    return a.substr(0, q + 1);
}

string to_binary_string(ll a) {
    string res;
    while (a) {
        res.push_back(48 + (a & 1));
        a >>= 1;
    }
...
void solve() {
    ll t;
    cin >> t;
    string a = to_binary_string(t);
    cin >> t;
    string b = to_binary_string(t);
    // debug(a), debug(b);
    int n = b.size(), m = a.size();
    // debug(n), debug(m);
    string stripped = remove_trailing(a);
    string reversed_stripped = remove_leading(string(stripped.rbegin(), stripped.rend()));
    string o = a + '1';
    string reversed_o = remove_leading(string(o.rbegin(), o.rend()));
    array<string, 4> as {stripped, reversed_stripped, o, reversed_o};
    // debugvec(as);
    if (a == b) {
        cout << "YES\n";
        return;
    ...
    for (auto&& at : as) {
        ...
        for (int i = 0; i + m <= n; ++i) {
            // debug(b.substr(i, m));
            if (b.substr(i, m) == at) {
                // debug(i + m), debug(n), debug(n - i - m);
                string lead = b.substr(0, i), tail = i + m < n ? b.substr(i + m, n - i - m) : "";
                // if (lead.size()) {
                //     debug(lead);
                // }
                // if (tail.size()) {
                //     debug(tail);
                // }
                if (count(lead.begin(), lead.end(), '0') == 0 && count(tail.begin(), tail.end(), '0') == 0) {
                    // debug(at), debug(i);
                    cout << "YES\n";
                    return;
                }
