...
void solve() {
    ...
    vector<unordered_set<int, safe_hash>> nch(N + 1);
    vector<int> ind(N + 1);
    for (auto&& [u, v] : edges) {
        if (scc[u] != scc[v] and not nch[scc[u]].count(scc[v])) {
            nch[scc[u]].emplace(scc[v]);
            ++ind[scc[v]];
        }
    }
    ...
    while (q.size()) {
        ...
        if (v == scc[1]) {
            res[v] = cnt[v] > 1 or self_loop[rev[v]] ? -1 : 1;
        } else if (res[v] and (cnt[v] > 1 or self_loop[rev[v]])) {
            res[v] = -1;
        }
        for (auto&& u : nch[v]) {
            if (res[u] == 0) {
