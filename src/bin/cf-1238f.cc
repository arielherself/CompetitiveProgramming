...
void solve() {
    ...
    auto dfs = [&] (auto dfs, int v, int pa) -> void {
        ...
        // only add one
        if (s.size()) {
            chmax(dp[v][2], 1 + *s.rbegin());
        } else {
            chmax(dp[v][2], 1);
        }
        // add more than one
        if (t.size() >= 2) {
            chmax(dp[v][1], 1 + *t.rbegin() + *next(t.rbegin()) + t.size() - 2);
        } else {
            chmax(dp[v][1], 1 + accumulate(t.begin(), t.end(), 0));
        }
        if (t.size()) {
            chmax(dp[v][0], 1 + *t.rbegin() + t.size() - 1);
        } else {
            chmax(dp[v][0], 1);
        }
    };
    dfs(dfs, 1, 0);
    // deb(dp[6][0], dp[6][1]);
    vector<array<int, 3>> dp1(n + 1);
    auto dfs2 = [&] (auto dfs2, int v, int pa, int up0, int up1) -> void {
        // deb(v, pa, up0, up1);
        multiset<int> s, t;
        for (auto&& u : ch[v]) {
            if (u == pa) continue;
        ...
        if (up0) t.emplace(up0);
        if (up1) s.emplace(up1);
        for (auto&& u : ch[v]) {
            if (u == pa) continue;
            s.erase(s.lower_bound(dp[u][1]));
            t.erase(t.lower_bound(dp[u][0]));
            int curr_up0 = 0, curr_up1 = 0;
            // add more than one
            if (t.size() >= 2) {
                chmax(curr_up1, 1 + *t.rbegin() + *next(t.rbegin()) + t.size() - 2);
            } else {
                chmax(curr_up1, 1 + accumulate(t.begin(), t.end(), 0));
            }
            if (t.size()) {
                chmax(curr_up0, 1 + *t.rbegin() + t.size() - 1);
            } else {
                chmax(curr_up0, 1);
            }
            dfs2(dfs2, u, v, curr_up0, curr_up1);
            s.emplace(dp[u][1]);
            t.emplace(dp[u][0]);
        }
        // only add one
        if (s.size()) {
            chmax(dp1[v][2], 1 + *s.rbegin());
        } else {
            chmax(dp1[v][2], 1);
        }
        // add more than one
        if (t.size() >= 2) {
            chmax(dp1[v][1], 1 + *t.rbegin() + *next(t.rbegin()) + t.size() - 2);
        } else {
            chmax(dp1[v][1], 1 + accumulate(t.begin(), t.end(), 0));
        }
        if (t.size()) {
            chmax(dp1[v][0], 1 + *t.rbegin() + t.size() - 1);
        } else {
            chmax(dp1[v][0], 1);
        }
    };
    ...
    // cerr << endl;
    // for (int i = 1; i <= n; ++i) {
    //     cerr << "{" << dp1[i][0] << ", " << dp1[i][1] << "} ";
    // }
    // cerr << endl;
    for (auto&& [x, y, z] : dp1) {
        chmax(res, max(x, max(y, z)));
    }
    cout << res << '\n';
}
