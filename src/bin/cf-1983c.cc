...
void solve() {
    ...
    ll tot = accumulate(a[0].begin(), a[0].end(), ll(0));
    vector ps(3, vector<ll>(n + 1));
    for (int i = 0; i < 3; ++i) {
        for (int j = 1; j <= n; ++j) {
    ...
    auto calc = [&] (tuple<int, int, int> perm) -> pii {
        auto [x, y, z] = perm;
        int p = -1, q = -1;
        for (int i = 0; i <= n; ++i) {
            if (ps[x][i] >= (tot + 2) / 3) {
        ...
        for (int i = n; ~i; --i) {
            if (ps[z][n] - ps[z][i] >= (tot + 2) / 3) {
                q = i + 1;
                break;
            }
        }
        if (q > p and ps[y][q - 1] - ps[y][p] >= (tot + 2) / 3) {
            return { p + 1, q };
        } else {
            return { -1, -1 };
        }
    ...
    for (auto&& p : vector<tuple<int, int, int>> {{ 0, 1, 2 }, { 1, 0, 2 }, { 2, 0, 1 }, { 0, 2, 1 }, { 1, 2, 0 }, { 2, 1, 0 }}) {
        auto [i, j] = calc(p);
        if (i != -1) {
            array<pii, 3> res = {};
            auto [x, y, z] = p;
            ...
            res[y] = { i, j - 1 };
            res[z] = { j, n };
            cout << res[0].first << ' ' << res[0].second << ' ' << res[1].first << ' ' << res[1].second << ' ' << res[2].first << ' ' << res[2].second << '\n';
            return;
        }
    }
    cout << -1 << '\n';
