...
void solve() {
    read(int, n);
    read(ll, k);
    map<int, array<ll, 2>, greater<void>> cnt_mp;
    for (int i = 0; i < n; ++i) {
        read(int, c, w, f);
        cnt_mp[f][w - 1] += c;
    }
    int m = cnt_mp.size();
    segtree<Info, Tag> cnt(m);
    int t = 0;
    for (auto&& [f, v] : cnt_mp) {
        cnt.set(t, {
            .cnt1 = v[0],
    ...
    ll res = 0;
    while (i < m) {
        // at least send one per O(1) time
        {
            auto [cnt1, cnt2, _, f] = cnt.query(i);
            ll tot = cnt1 + 2 * cnt2;
            if (tot == 0) {
                i += 1;
                continue;
            }
            ll time = max<ll>(0, (tot + k - 1) / k - 1);
            res += (time + 1) * f;
            ll del_cap = time * k;
            // use even
            {
                ll use = min(cnt2, del_cap / 2);
                del_cap -= 2 * use;
                cnt2 -= use;
            }
            // use odd
            {
                ll use = min(cnt1, del_cap);
                del_cap -= use;
                cnt1 -= use;
            }
            cnt.set(i, {
        ...
        int nxt;
        {
            ...
            while (l < r) {
                ...
                if (mid == i) {
                    ...
                    auto [cnt1, cnt2, _1, _2] = cnt.range_query(i, mid - 1);
                    curr = cnt1 + 2 * cnt2;
                }
                if (curr <= k) {
                    l = mid;
            ...
            if (l != i) {
                auto [cnt1, cnt2, _1, _2] = cnt.range_query(i, l - 1);
                rem -= cnt1 + 2 * cnt2;
            }
            nxt = l;
        }
        if (nxt == m) break;
        {
            auto [cnt1, cnt2, _, f] = cnt.query(nxt);
            // use even
            {
                ll use = min(cnt2, rem / 2);
                rem -= 2 * use;
                cnt2 -= use;
            }
            // use odd
        ...
        if (rem) {
            // assert(rem == 1);
            auto [_1, _2, last1, _3] = cnt.range_query(nxt, m - 1);
            if (last1 != INF) {
                auto [cnt1, cnt2, _, f] = cnt.query(last1);
                cnt.set(last1, {
                    .cnt1 = cnt1 - 1,
                    .cnt2 = cnt2,
                    .last1 = cnt1 - 1 ? last1 : INF,
                    .f = f,
                });
            }
        }
