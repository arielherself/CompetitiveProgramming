...
ll u_max(ll a, ll b) { return max(a, b); }
ll u_min(ll a, ll b) { return min(a, b); }
...
void solve() {
    ...
    sparse_table<ll> min_rmq(ps.rbegin(), ps.rend(), u_min);
    sparse_table<ll> max_rmq(ps.rbegin(), ps.rend(), u_max);
    vector<int> prev(n), next(n);
    vector<int> q;
    for (int i = 0; i < n; ++i) {
        while (q.size() && a[q.back()] <= a[i]) q.pop_back();
        if (q.size()) prev[i] = q.back();
        else prev[i] = -1;
        q.push_back(i);
    }
    q.clear();
    for (int i = n - 1; ~i; --i) {
        while (q.size() && a[q.back()] <= a[i]) q.pop_back();
        if (q.size()) next[i] = q.back();
        else next[i] = n;
        q.push_back(i);
    }
    int f = 1;
    for (int i = 0; i < n; ++i) {
        ll max_ps = max_rmq.query(i + 1, next[i]) - min_rmq.query(prev[i] + 1, i);
        if (a[i] < max_ps) {
            f = 0;
            break;
        }
