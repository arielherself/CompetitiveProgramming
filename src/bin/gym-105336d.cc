...
void solve() {
    ...
    vector dp(n + 1, vector(m, vector<mll>(m)));
    ...
    for (int i = 1; i <= n; ++i) {
        for (int j = 0; j < m; ++j) {
            for (int k = j; k < m; ++k) {
                ...
                for (int l = j; l <= k; ++l) {
                    // two sides, including the current element
                    if (s[i - 1] == t[l]) {
                        dp[i][j][k] += (j <= l - 1 ? dp[i - 1][j][l - 1] : 1) * (l + 1 <= k ? dp[i - 1][l + 1][k] : 1);
                    }
                    // two sides without the current element
                    if (l < k) {
                        dp[i][j][k] += dp[i - 1][j][l] * dp[i - 1][l + 1][k];
                    }
                }
            }
        }
