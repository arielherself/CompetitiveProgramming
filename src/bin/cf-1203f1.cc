...
void solve() {
    ...
    sort_by_key(neg.begin(), neg.end(), expr(p.first - p.second, auto&& p), greater());
    dp[r] = 0;
    for (int i = 0; i < q; ++i) {
        for (int j = max(neg[i].first, neg[i].second); j <= r; ++j) {
            if (dp[j] != -INF) {
                chmax(dp[j - neg[i].second], dp[j] + 1);
            }
        }
    }
    cout << (res + *max_element(dp.begin(), dp.end()) == n ? "YES" : "NO") << '\n';
}
