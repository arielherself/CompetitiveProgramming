...
ll inversion(const vector<int>& a) {
	int n = a.size();
	BIT<int> tr(n + 10);
	ll res = 0;
	for (int i = 0; i < n; ++i) {
		res += tr.getsum(n + 10) - tr.getsum(a[i] + 10);
		tr.add(a[i] + 10, 1);
	}
	return res;
}
...
void solve() {
	...
	segtree<Info, Tag> mv(n);
	for (auto&& i : idx) {
		if (rem == 0) break;
		int avail = has.range_query(0, i).val - 1;
		int use = min<ll>(avail, rem);
		f[i] += mv.query(i).val - use;
		has.apply(i, { -1 });
		if (use == rem) {
			for (int j = i - 1; j >= 0; --j) {
				if (has.query(j).val == 1) {
					f[j] += mv.query(j).val + (use > 0);
					if (use > 0) {
						use -= 1;
					}
				}
