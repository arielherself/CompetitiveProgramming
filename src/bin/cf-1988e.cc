...
void solve() {
	...
	for (auto&& i : idx) {
		...
		int l = i - left,r  = right - i;
		if (left + 1 <= i - 1) {
			diff[left + 1] += ll(1) * a[i] * (l - 1) * r;
			diff[i] -= ll(1) * a[i] * (l - 1) * r;
		}
		if (i + 1 <= right - 1) {
			diff[i + 1] += ll(1) * a[i] * l * (r - 1);
			diff[right] -= ll(1) * a[i] * l * (r - 1);
		}

		if (left != -1) {
			auto it = cand.lower_bound(left);
			int left_len;
			if (it != cand.begin()) {
				left_len = left - *prev(it) - 1;
			} else {
				left_len = left;
			}
			diff[left] += ll(1) * a[i] * (l + left_len) * r;
			diff[left + 1] -= ll(1) * a[i] * (l + left_len) * r;
			if (0 < left) {
				diff[0] += ll(1) * a[i] * l * r;
				diff[left] -= ll(1) * a[i] * l * r;
			}
		}

		if (right != n) {
			auto it = cand.upper_bound(right);
			int right_len;
			if (it != cand.end()) {
				right_len = *it - right - 1;
			} else {
				right_len = n - right - 1;
			}
			diff[right] += ll(1) * a[i] * l * (r + right_len);
			diff[right + 1] -= ll(1) * a[i] * l * (r + right_len);
			if (right < n - 1) {
				diff[right + 1] += ll(1) * a[i] * l * r;
				diff[n] -= ll(1) * a[i] * l * r;
			}
		}
	}
