...
void solve() {
    ...
    for (int i = 1; i <= n; ++i) {
        ...
        for (int j = 2; j < i; ++j) {
            dp[i][0] = max(dp[i][0], (double)(a[0] + a[j - 1]) / (x[j - 1] - x[0]) * (a[j - 1] + a[i - 1]) / (x[i - 1] - x[j - 1]));
        }
        // this is the third tower
        for (int j = 2; j < i; ++j) {
            dp[i][1] = max(dp[i][1], dp[j][0] * (a[j - 1] + a[i - 1]) / (x[i - 1] - x[j - 1]));
        }
    }
    double res = 0;
    for (int i = 2; i < n; ++i) {
        res = max(res, dp[i][1] * (a[n - 1] + a[i - 1]) / (x[n - 1] - x[i - 1]));
    }
    printf("%.9lf\n", res);
