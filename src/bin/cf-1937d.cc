...
void solve() {
    ...
    vector<int> ps(n + 1), pd(n + 1);
    vector<int> ss(n + 1), sd(n + 1);
    for (int i = 1; i <= n; ++i) {
        ps[i] = ps[i-1], pd[i] = pd[i-1];
        if (grid[i-1] == '<') {
            ps[i] += 1;
            pd[i] += i;
        }
    ...
    for (int i = 0; i < n; ++i) {
        if (ps[n] - ps[i + 1] > ss[0] - ss[i]) {
            ...
            debug(ps[2] - ps[1]);
            debug(l);
            res[i] += (pd[l] - pd[i + 1] - (i + 1) * (db + 1)) * 2;
            debug(res[i]);
            res[i] += (sd[0] - sd[i] - (n - i) * db) * 2;
            debug(res[i]);
            res[i] += i + 1;
            debug(res[i]);
        } else if (ps[n] - ps[i + 1] < ss[0] - ss[i]) {
            debug(false);
            int db = ps[n] - ps[i + 1];
            int l = 0, r = i;
            while (l < r) {
            ...
            res[i] += (pd[n] - pd[i + 1] - (i + 1) * db) * 2;
            res[i] += (sd[l] - sd[i] - (n - i) * (db + 1)) * 2;
            res[i] += n - i;
        } else {
            debug(1);
            int db = ps[n]- ps[i + 1];
            res[i] += (pd[n] - pd[i + 1] - (i + 1) * db) * 2;
            res[i] += (sd[0] - ss[i] - (n - i) * db) * 2;
            if (grid[i] == '>') {
                res[i] += n - i;
            } else {
                res[i] += i + 1;
