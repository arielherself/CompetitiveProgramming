...
struct DynamicVector {
	...
	template <typename Pred>
		size_t f(size_t start, Pred pred) {
			optional<size_t> l, r;
			split(root, start + 1, l, r);
			size_t ret = prefix_binary_search(r, pred);
			root = merge(l, r);
			return start + ret;
		}
...
void solve() {
	...
	while (a.size() > 0) {
		...
		int ptr = 0;
		while (ptr < n) {
			target.emplace_back(ptr, a[ptr].val);
			int nxt = a.f(ptr, expr(x.val > target.back().second, auto&& x));
			ptr = nxt;
		}
		res.emplace_back();
		res.back().reserve(target.size());
		for (auto&& [i, x] : target) {
