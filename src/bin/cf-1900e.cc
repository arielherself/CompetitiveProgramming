...
namespace tarjan {
    ...
    vector<tuple<size_t, ll, vector<int>>> build_scc_with_size(const vector<vector<int>>& ch, const vector<ll>& w) {
        int n = ch.size();
        auto br = scc(ch);
        int cnt = *max_element(br.begin(), br.end());
        vector<unordered_set<int, safe_hash>> rb(cnt + 1);
        ...
        vector<tuple<size_t, ll, vector<int>>> res(cnt + 1);
        for (int i = 1; i <= cnt; ++i) {
            get<2>(res[i]) = vector<int>(rb[i].begin(), rb[i].end());
        }
        for (int i = 0; i < n; ++i) {
            get<0>(res[br[i]]) += 1;
            get<1>(res[br[i]]) += w[i];
...
void solve() {
    read(int, n, m);
    readvec(ll, a, n);
    vector<vector<int>> rch(n);
    while (m--) {
        read(int, u, v);
        --u, --v;
        Edge(rch, u, v);
    }
    auto ch = tarjan::build_scc_with_size(rch, a);
    int k = ch.size();
    vector<int> ind(k + 1);
    for (int i = 1; i < k; ++i) {
        for (auto&& u : get<2>(ch[i])) {
            ind[u] += 1;
        }
    }
    deque<int> dq;
    ...
    while (dq.size()) {
        ...
        dp[v].first += get<0>(ch[v]);
        dp[v].second += get<1>(ch[v]);
        for (auto&& u : get<2>(ch[v])) {
