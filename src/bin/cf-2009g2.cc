...
void solve() {
    read(int, n, k, q);
    // cin >> n;
    // memset(mx, 0, (n << 2) * sizeof(int));
    // memset(se, 0, (n << 2) * sizeof(int));
    // memset(cn, 0, (n << 2) * sizeof(int));
    // memset(tag, 0, (n << 2) * sizeof(int));
    // memset(sum, 0, (n << 2) * sizeof(ll));
    //
    // read(int, k, q);
    readvec(int, a, n);
    set<pii, greater<>> st;
    ...
    for (int i = 0; i < q; ++i) {
        ...
        --l, --r;
        r -= k - 1;
        // if (l < 0 or l >= n) exit(825);
        queries[l].emplace_back(r, i);
    }
    auto modify = [&] (int i, int x) -> void {
        // if (i < 0 or i > 2 * n) exit(826);
        // if (not st.count({cnt[i], i})) exit(827);
        st.erase({cnt[i], i});
        cnt[i] += x;
        st.emplace(cnt[i], i);
    };
    ...
    for (int i = 0; i < n; ++i) {
        modify(a[i] - i + n, 1);
        if (i >= k - 1) {
            // if (i - k + 1 >= n - k + 1 or i - k + 1 < 0) exit(828);
            // if (st.empty()) exit(829);
            b[i - k + 1] = k - st.begin()->first;
            modify(a[i - k + 1] - (i - k + 1) + n, -1);
        }
    }
    segtree<Info, Tag> tr(n - k + 1);
    ...
    // build();
    vector<ll> res(q);
    // auto verify = [&] (int l, int r) -> void {
    //     if (l >= 0 and r <= n - k and l <= r) ;
    //     else exit(830);
    // };
    for (int i = n - k; ~i; --i) {
        // verify(i, n - k);
        tr.range_apply(i, n - k, { b[i] });  // range min
        // modify_min(i + 1, n - k + 1, b[i]);
        for (auto&& [r, idx] : queries[i]) {
            // verify(i, r);
            // if (idx < 0 or idx >= q) exit(831);
            res[idx] = tr.range_query(i, r).val;  // range sum
            // res[idx] = query_sum(i + 1, r + 1);
        }
    }
    for (auto&& x : res) {
        cout << x << '\n';
