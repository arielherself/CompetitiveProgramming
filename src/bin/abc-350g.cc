...
void solve() {
    ...
    unordered_map<int, bitset<N>, safe_hash> large;
    auto extend = [&] (int v) -> void {
        for (auto&& u : small[v]) {
            large[v][u] = 1;
        }
        small[v] = {};
    ...
    for (int i = 0; i < q ;++i) {
        read(int, a, b, c);
        int op = 1 + (((ll(1) * a * (1 + last_res)) % PRIME) % 2);
        int u = 1 + (((ll(1) * b * (1 + last_res)) % PRIME) % n);
        int v = 1 + (((ll(1) * c * (1 + last_res)) % PRIME) % n);

        if (op == 1) {
            if (large.count(u)) {
                large[u][v] = 1;
            } else {
                small[u].emplace_back(v);
                if (small[u].size() > m) {
                    extend(u);
                }
            }
