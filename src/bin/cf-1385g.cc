...
void solve() {
    ...
    for (int i = 0; i < n; ++i) {
        cnt[a[i]].emplace_back(0, i + 1);
        cnt[b[i]].emplace_back(1, i + 1);
    }
    vector<tarjan::mutex_cond> conds;
    for (int i = 1; i <= n; ++i) {
        if (cnt[i].size() != 2) {
            cout << -1 << '\n';
            return;
        }
        if (cnt[i][0].first == cnt[i][1].first) {
            conds.emplace_back(cnt[i][0].second, 1, cnt[i][1].second, 1);
            conds.emplace_back(cnt[i][0].second, 0, cnt[i][1].second, 0);
        } else {
            conds.emplace_back(cnt[i][0].second, 1, cnt[i][1].second, 0);
            conds.emplace_back(cnt[i][0].second, 0, cnt[i][1].second, 1);
        }
    }
    auto res = tarjan::solve_twosat(n, conds);
    if (res == nullopt) {
        cout << -1 << '\n';
    } else {
        auto v = res.value();
        vector<int> out;
        if (count(v.begin(), v.end(), true) > n / 2) {
            for (int i = 0; i < n; ++i) {
                if (not v[i]) {
