...
void solve() {
    ...
    iota(p2.rbegin(), p2.rend(), 1);
    readvec(int, a, n);
    int cnt1 = 0, cnt2 = 0;
    int cnt1_ex = 0, cnt2_ex = 0;
    for (int i = 0; i < n; ++i) {
        if (a[i] != p1[i]) cnt1 += 1;
        if (a[i] != p2[i]) cnt2 += 1;
        if (a[i] != p1[i] && a[i] == p2[i]) cnt1_ex += 1;
        if (a[i] != p2[i] && a[i] == p1[i]) cnt2_ex += 1;
    }
    int win1 = 0, win2 = 0;
    
    if (cnt1_ex <= cnt2) win1 += 1;
    if (cnt2_ex < cnt1) win2 += 1;
    if (win1 && win2) {
        cout << "Tie\n";
        return;
    } else if (win1) {
