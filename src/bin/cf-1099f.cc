...
private:
    ...
    ll binary_search(size_type s, size_type t, size_type p, ll target) {
        if (s == t) {
            if (d[p].cnt == 0) return 0;
            int unit = d[p].tot / d[p].cnt;
            return min(d[p].cnt, target / unit);
        }
        size_type m = s + (t - s >> 1);
        push(p, m - s + 1, t - m);
        if (d[2 * p].tot <= target) return d[2 * p].cnt + binary_search(m + 1, t, p * 2 + 1, target - d[2 * p].tot);
        else return binary_search(s, m, p * 2, target);
    }
...
void solve() {
    ...
    vector<ll> a(n + 1);
    auto dfs = [&] (auto dfs, int v, int pa, ll avail) -> void {
        Info prev = tr.query(tm[v]);
        tr.set(tm[v], prev + Info {
            .cnt = cnt[v],
            .tot = ll(1) * cnt[v] * tm[v],
        });
        a[v] = tr.binary_search(avail);
        for (auto&& [u, w] : e[v]) {
            if (u == pa) continue;
            dfs(dfs, u, v, max<ll>(0, avail - 2 * w));
        }
        tr.set(tm[v], prev);
    };
    dfs(dfs, 1, 0, t);
