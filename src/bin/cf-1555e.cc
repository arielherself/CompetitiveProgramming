...
void solve() {
    ...
    for (int i = 0; i < n; ++i) {
        read(int, l, r, w);
        segments.emplace_back(l, r, w);
    }
    sort_by_key(segments.begin(), segments.end(), [] (const tiii& x) { return get<2>(x); });
    ...
    int res = INF;
    for (int i = 0; i < n; ++i) {
        if (get<0>(segments[i]) <= get<1>(segments[i]) - 1) {
            tr.range_apply(get<0>(segments[i]), get<1>(segments[i]) - 1, {1});
        }
        while (tr.range_query(get<0>(segments[l]), get<1>(segments[l]) - 1).val > 1) {
            if (get<0>(segments[l]) <= get<1>(segments[l]) - 1) {
                tr.range_apply(get<0>(segments[l]), get<1>(segments[l]) - 1, {-1});
            }
            ++l;
        }
        if (tr.range_query(1, m - 1).val > 0) {
            chmin(res, get<2>(segments[i]) - get<2>(segments[l]));
        }
    }
    cout << res << '\n';
}
