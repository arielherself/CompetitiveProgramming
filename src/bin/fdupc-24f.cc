...
void solve() {
    ...
    vector dp(n + 1, vector<int128>(2, -INFLL));
    dp[0] = { 0, 0 };
    for (int i = 1; i <= n; ++i) {
        for (int j = 1; j < 65 and i - j >= 0; ++j) {
            chmax(dp[i][0], dp[i - j][1] - c * ((int128(1) << j) - 1) + psa[i] - psa[i - j]);
            chmax(dp[i][1], dp[i - j][0] - c * ((int128(1) << j) - 1) + psb[i] - psb[i - j]);
        }
    }
