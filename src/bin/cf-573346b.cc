...
void solve() {
    ...
    auto unquote = [] (const string& x) {
        int m = x.size();
        return x.substr(1, m - 2);
    };

    unordered_map<string, ll> integers;
    unordered_map<string, string> strings;

    auto resolve_statement = [&] (string init, istream& in) {
        if (init == "") in >> init;
        // debug(init.substr(0, 8));
        if (init.substr(0, 8) == "Println(") {
            string arg = init.substr(8);
            arg = arg.substr(0, arg.size() - 1);
            if (arg[0] == '"') {  // string literal
                out << unquote(arg) << '\n';
            } else if (arg[0] >= '0' and arg[0] <= '9' or arg[0] == '-') {  // integer literal
                out << arg << '\n';
            } else {  // variable
                if (integers.count(arg)) {
                    out << integers[arg] << '\n';
                } else if (strings.count(arg)) {
                    out << strings[arg] << '\n';
                } else {
                    re();
                }
            ...
            // debug(op);
            int integer = 0;
            if (integers.count(lhs)) {
                integer = 1;
            } else if (strings.count(lhs)) {
                integer = 0;
            } else {
                re();
            }
            if (op == "=") {
                if (rhs[0] == '"') {  // string literal
                    if (integer) re();
                    strings[lhs] = unquote(rhs);
                } else if (rhs[0] >= '0' and rhs[0] <= '9' or rhs[0] == '-') {  // integer literal
                    if (not integer) re();
                    integers[lhs] = stoll(rhs);
                } else {  // variable
                    int rhs_integer = 0;
                    if (integers.count(rhs)) {
                        rhs_integer = 1;
                    } else if (strings.count(rhs)) {
                        rhs_integer = 0;
                    } else {
                        re();
                    }
                    if (integer != rhs_integer) re();
                    if (integer) {
                        integers[lhs] = integers[rhs];
                    } else {
    ...
    while (1) {
        ...
        // debug(a);
        if (a == "var") {
            read(string, name, type);
            // deb(name, type);
            if (integers.count(name) or strings.count(name)) re();
            if (type == "int") {
                integers[name] = 0;
            } else {
                assert(type == "string");
            ...
            { string _; std::getline(cin, _); }  // EOL
            while (1) {
                string b;
                std::getline(cin, b);
                if (b == "}") {
