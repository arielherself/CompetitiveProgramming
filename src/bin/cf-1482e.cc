...
void solve() {
    ...
    for (int i = 1; i <= n; ++i) {
        while (stack.size() and a[stack.back()] > a[i]) {
            stack.pop_back();
        }

        if (stack.back() != 0) {
            dp.set(i, {max(dp.query(stack.back()).val, dp.range_query(stack.back(), i - 1).val + b[i])});
        } else {
            // deb(i, dp.range_query(stack.back(), i - 1).val);
            dp.set(i, {dp.range_query(stack.back(), i - 1).val + b[i]});
        }
