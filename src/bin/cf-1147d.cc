...
void solve() {
    ...
    mll res = 0;
    for (int i = 1; i < n; ++i) {
        quick_union qu(4 * n + 1);

        auto get = [&] (int which, int idx, int value) {
            return which * 2 * n + value * n + idx;
        };
        int impossible = 4 * n;

        qu.merge(get(0, i - 1, 0), impossible);
        for (int j = i; j < n; ++j) {
            qu.merge(get(0, j, 1), impossible);
        }

        for (int j = 0; j < i / 2; ++j) {
            qu.merge(get(0, j, 0), get(0, i - 1 - j, 0));
            qu.merge(get(0, j, 1), get(0, i - 1 - j, 1));
        }
        for (int j = 0; j < n / 2; ++j) {
            qu.merge(get(1, j, 0), get(1, n - 1 - j, 0));
            qu.merge(get(1, j, 1), get(1, n - 1 - j, 1));
        }
        for (int j = 0; j < n; ++j) {
            if (s[j] == '1') {
                qu.merge(get(0, j, 0), get(1, j, 1));
                qu.merge(get(0, j, 1), get(1, j, 0));
            } else if (s[j] == '0') {
                qu.merge(get(0, j, 0), get(1, j, 0));
                qu.merge(get(0, j, 1), get(1, j, 1));
            }
        }

        mll curr = 1;
        vector<int> stale(4 * n + 1);
        for (int j = 0; j < n; ++j) {
            int up = qu.query(get(0, j, 0));
            int down = qu.query(get(0, j, 1));
            if (qu.connected(up, down)) {
                goto fi;
            } else if (not stale[up] and not stale[down]) {
                if (qu.connected(up, impossible) and qu.connected(down, impossible)) {
                    goto fi;
                } else if (not qu.connected(up, impossible) and not qu.connected(down, impossible)) {
                    curr *= 2;
                }
                stale[up] = stale[down] = true;
            }
        }
        for (int j = 0; j < n; ++j) {
            int up = qu.query(get(1, j, 0));
            int down = qu.query(get(1, j, 1));
            if (qu.connected(up, down)) {
                goto fi;
            } else if (not stale[up] and not stale[down]) {
                if (qu.connected(up, impossible) and qu.connected(down, impossible)) {
