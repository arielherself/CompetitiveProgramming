...
void solve() {
	...
	vector<vector<int>> info(m << 2);
	auto range_apply = [&] (auto self, int s, int t, int p, int l, int r, int x) -> void {
		if (l <= s and t <= r) {
			info[p].emplace_back(x);
			return;
		}
		int m = s + t >> 1;
		if (l <= m) {
			self(self, s, m, p * 2, l, r, x);
		}
		if (r > m) {
			self(self, m + 1, t, p * 2 + 1, l, r, x);
		}
	};
	...
	vector<int> rev(m);
	{
		int block = sqrt((long double)(2 * n));
	...
	int t = 0;
	{
		...
		for (int i = 0; i < m; ++i) {
			...
			for (int j = 0; j < t; ++j) {
				state[ops[j]] ^= 1;
				if (j == tm[ops[j]]) {
					if (last[ops[j]] != m and state[ops[j]] == 0) {
						range_apply(range_apply, 0, m - 1, 1, last[ops[j]], i - 1, a[ops[j]]);
						// deb(last[ops[j]], i - 1, a[ops[j]]);
						last[ops[j]] = m;
					} else if (last[ops[j]] == m and state[ops[j]] == 1) {
						last[ops[j]] = i;
					}
				}
			}
		...
		for (int i = 1; i <= n; ++i) {
			if (last[i] != m) {
				assert(state[i] == 1);
				range_apply(range_apply, 0, m - 1, 1, last[i], m - 1, a[i]);
				// deb(last[i], m - 1, a[i]);
			}
		}
	}
	...
	auto dfs = [&] (auto dfs, int s, int t, int p, basis<int> b) -> void {
		for (auto&& x : info[p]) {
			b.insert(x);
		}
		if (s == t) {
			b.insert(extra[idx[s]]);
			int target = get<2>(queries[idx[s]]);
			auto v = b.value();
			for (auto&& x : v) {
				if (target == 0) break;
				if (msp(x) == msp(target)) {
