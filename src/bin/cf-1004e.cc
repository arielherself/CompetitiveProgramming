...
void solve() {
    ...
    dfs(dfs, s, 0, 0);
    int t = far;
    vector<int> on_path(n + 1);
    vector<int> path;
    ll tot = 0;
    auto find_path = [&] (auto find_path, int v, int pa) -> bool {
        if (v == t) {
            on_path[v] = 1;
            path.emplace_back(v);
            return true;
    ...
    ll curr = 0;
    for (int i = 1; i < m; ++i) {
        for (auto&& [u, w] : e[path[i - 1]]) {
            if (u == path[i]) {
                curr += w;
                break;
            }
        }
        if (max(curr, tot - curr) < mx) {
            mx = max(curr, tot - curr);
            root = path[i];
            rootidx = i;
        }
    }
    vector<ll> max_depth(n + 1), depth(n + 1);
    auto dfs2 = [&] (auto dfs2, int v, int pa, ll d) -> void {
        depth[v] = d;
        max_depth[v] = d;
        for (auto&& [u, w] : e[v]) {
            if (u == pa) continue;
    ...
    dfs2(dfs2, root, 0, 0);
    vector<ll> left(k + 1), right(k + 1);
    int left_root = -1, right_root = -1;
    if (rootidx > 0) {
        // has left
        left_root = path[rootidx - 1];
        // debug(left_root);
        left.assign(k + 1, INF);
        left[0] = max_depth[left_root];
        auto dfs = [&] (auto dfs, int v, int pa, int d, ll prop) -> void {
            if (d > k) return;
            left[d] = max(prop, max_depth[v] - depth[v]);
            int mxu = -1;
            ll mx = 0;
            for (auto&& [u, w] : e[v]) {
                if (u == pa) continue;
            ...
            if (mxu != -1) {
                ...
                for (auto&& [u, w] : e[v]) {
                    if (u == pa) continue;
                    if (u != mxu) {
                        chmax(sec, max_depth[u] - depth[v]);
                    }
                }
                dfs(dfs, mxu, v, d + 1, max(prop, sec));
            }
        };
        dfs(dfs, left_root, root, 1, 0);
        // get prefix minimum
        for (int i = 1; i <= k; ++i) {
            chmin(left[i], left[i - 1]);
        }
    }
    if (rootidx < m - 1) {
        // has right
        right_root = path[rootidx + 1];
        // debug(right_root);
        right.assign(k + 1, INF);
        right[0] = max_depth[right_root];
        auto dfs = [&] (auto dfs, int v, int pa, int d, ll prop) -> void {
            if (d > k) return;
            right[d] = max(prop, max_depth[v] - depth[v]);
            int mxu = -1;
            ll mx = 0;
            for (auto&& [u, w] : e[v]) {
                if (u == pa) continue;
        ...
        dfs(dfs, right_root, root, 1, 0);
        for (int i = 1; i <= k; ++i) {
            chmin(right[i], right[i - 1]);
        }
    }
    ll fixed = 0;
    for (auto&& [u, w] : e[root]) {
        if (u != left_root and u != right_root) {
            chmax(fixed, max_depth[u]);
        }
    }
    ll res = INFLL;
    for (int i = 0; i < k; ++i) {
        chmin(res, max(fixed, max(left[i], right[k - 1 - i])));
    }
    cout << res << '\n';
}
