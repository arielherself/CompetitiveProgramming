...
void solve() {
	...
	vector<int> nxtsufa(n);
	nxtsufa[n - 1] = n;
	for (int i = n - 2; ~i; --i) {
		if (sufa[i] != sufa[i + 1]) {
			nxtsufa[i] = i + 1;
		} else {
			nxtsufa[i] = nxtsufa[i + 1];
		}
	}
	vector<int> nxtsufb(n);
	nxtsufb[n - 1] = n;
	for (int i = n - 2; ~i; --i) {
		if (sufb[i] != sufb[i + 1]) {
			nxtsufb[i] = i + 1;
		} else {
			nxtsufb[i] = nxtsufb[i + 1];
		}
	}

	vector<vector<pii>> seqa(n + 1);
	vector<vector<pii>> seqb(n + 1);
	...
	for (int i = n - 1; ~i; --i) {
		...
		}
		{
			seqa[i] = {{a[i], i}};
			int curr = a[i];
			for (auto&& [x, j] : seqa[nxta[i]]) {
				int next = x == 0 ? 0 : gcd(curr, x);
				if (next != curr) {
					seqa[i].emplace_back(next, j);
		...
		}
		{
			seqb[i] = {{b[i], i}};
			int curr = b[i];
			for (auto&& [x, j] : seqb[nxtb[i]]) {
				int next = x == 0 ? 0 : gcd(curr, x);
				if (next != curr) {
					seqb[i].emplace_back(next, j);
					pivots.emplace_back(j);
					curr = next;
				}
			}
		}
		{
			int j = nxtsufa[i];
			while (j < n) {
				pivots.emplace_back(j - 1);
				j = nxtsufa[j];
			}
			pivots.emplace_back(n - 1);
		}
		{
			int j = nxtsufb[i];
			while (j < n) {
		...
		int m = unique(pivots.begin(), pivots.end()) - pivots.begin();
		pivots.resize(m);
		assert(pivots.size() <= 140);
		// deb(i, pivots);

		int mida = a[i], midb = b[i];
		for (int j = 0; j < m - 1; ++j) {
			mida = gcd(mida, a[pivots[j]]);
			midb = gcd(midb, b[pivots[j]]);
			int ca = midb;
			int cb = mida;
			if (i) {
				ca = gcd(ca, prea[i - 1]);
			...
			if (pivots[j] < n - 1) {
				ca = gcd(ca, sufa[pivots[j] + 1]);
				cb = gcd(cb, sufb[pivots[j] + 1]);
			}
			if (ca + cb > val) {
				val = ca + cb;
				cnt = pivots[j + 1] - pivots[j];
			} else if (ca + cb == val) {
				cnt += pivots[j + 1] - pivots[j];
			}
		}
