...
void solve() {
    ...
    auto diff = [] (const vector<int>& a, const vector<int>& b) {
        int res = 0;
        for (auto&& [x, y] : zip<const int&, const int&>(a.begin(), a.end(), b.begin(), b.end())) {
            if (x == y) {
                ++res;
            }
        }
    ...
    for (int i = 0; i < n; ++i) {
        vector<int> b(m);
        reverse_copy(a[i].begin(), a[i].end(), b.begin());
        for (int j = i + 1; j < n; ++j) {
            int x = diff(a[i], a[j]), y = diff(b, a[j]);
            if (x >= k and y >= k) {
                ;;
            } else if (x >= k and y < k) {
                qu.merge(i, j);
    ...
    vector<int> res;
    vector<vector<int>> bk(2 * n);
    for (int i = 0; i < n; ++i) {
        if (qu.connected(i, n + i)) {
            cout << -1 << '\n';
            return;
        }
        bk[qu.query(i)].emplace_back(i + 1);
    }
    for (int i = 0; i < n; ++i) {
        if (bk[i].size() < bk[i + n].size()) {
            res.insert(res.end(), bk[i].begin(), bk[i].end());
        } else {
            res.insert(res.end(), bk[i + n].begin(), bk[i + n].end());
        }
    }
    cout << res.size() << '\n';
    putvec(res);
