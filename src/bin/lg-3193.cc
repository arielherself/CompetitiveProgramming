...
void solve() {
	...
	vector fail(m, vector<int>(U));
	for (int i = 0; i < m; ++i) {
		for (int j = 0; j < U; ++j) {
			if (s[i] == j) {
				fail[i][j] = i + 1;
			} else {
				fail[i][j] = fail[i == 0 ? 0 : pi[i - 1]][j];
			}
		}
	}
	...
	for (int i = 0; i < m; ++i) {
		for (int j = 0; j < U; ++j) {
			if (fail[i][j] == m) continue;
			(mat[fail[i][j]][i] += 1) %= M;
		}
	}

	auto matmul = [&] (const vector<vector<ll>>& a, const vector<vector<ll>>& b) {
		vector c(m, vector<ll>(m));
		for (int i = 0; i < m; ++i) {
			for (int j = 0; j < m; ++j) {
				for (int k = 0; k < m; ++k) {
	...
	auto matrix_qpow = [&] (auto self, const vector<vector<ll>>& a, int n) -> vector<vector<ll>> {
		if (n == 1) {
			return a;
		}
		auto half = self(self, a, n / 2);
		auto res = matmul(half, half);
		if (n % 2) {
			res = matmul(res, a);
		}
		return res;
	...
	auto a = matrix_qpow(matrix_qpow, mat, n);
