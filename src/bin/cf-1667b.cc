/**
 * Author:   subcrip
 * Created:  2024-05-26 11:28:58
 * Modified: 2024-05-26 11:50:40
 * Elapsed:  21 minutes
 */
...
void solve() {
    ...
    unordered_map<ll, int, safe_hash> mp;
    int N = 0;
    for (auto&& x : st) mp[x] = ++N;
    segtree<Info, Tag> left_tr(N + 1), right_tr(N + 1), mid_tr(N + 1);
    left_tr.set(mp[0], {0}), right_tr.set(mp[0], {0}), mid_tr.set(mp[0], {0});
    ps = 0;
    ll curr;
    for (int i = 1; i <= n; ++i) {
        curr = -INF;
        ps += a[i - 1];
        if (0 <= mp[ps] - 1) curr = max(curr, i + left_tr.range_query(0, mp[ps] - 1).val);
        if (mp[ps] + 1 <= N) curr = max(curr, right_tr.range_query(mp[ps] + 1, N).val - i);
        curr = max(curr, mid_tr.query(mp[ps]).val);
        left_tr.apply(mp[ps], {curr - i});
        right_tr.apply(mp[ps], {curr + i});
        mid_tr.apply(mp[ps], {curr});
    }
    cout << curr << '\n';
}
