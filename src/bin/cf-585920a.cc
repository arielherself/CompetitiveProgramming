...
/**
 * My thought process
 * 1. Each path could be independent.
 * 2. Binary representation.
 * 3. p is redundant.
 * 4. Use Zeckendorf's theorem.
 */

// __attribute__((target("popcnt")))
void solve() {
	constexpr int N = 84;  // 0 to 83
	read(ll, k, p);
	ll target = k;
	int start = 1, end = 2;
	int t = 3;
	...
	vector<pii> edges;
	edges.emplace_back(item[1], item[2]);
	for (int i = 3; i < N; ++i) {
		edges.emplace_back(item[i - 1], item[i]);
		edges.emplace_back(item[i - 2], item[i]);
	}

	int ptr = N - 1;
	while (target) {
		while (fibo[ptr] > target) {
			ptr -= 1;
		}
		edges.emplace_back(item[ptr], end);
		target -= fibo[ptr];
	}

	int n = t - 1, m = edges.size();
	cout << "YES\n";
	cout << n << ' ' << m << '\n';
	cout << start << ' ' << end << '\n';
	for (auto&& [u, v] : edges) {
