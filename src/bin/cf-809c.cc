...
void solve() {
    ...
    auto square = [&] (int l, int r) -> mll {
        auto work = [] (int n) -> mll {
            return mll(1) * n * (n + 1) * (2 * n + 1) / 6;
        };
        return work(r) - work(l - 1);
    };

    auto margin = [] (int u, int d, int y, int k) -> mll {
        mll res = 0;
        int l = (u + 1) / 2;
        int r = d / 2;
        if (l <= r) {
            res += y * mll(l + r) * (r - l + 1) + y * mll(1 - y) * (r - l + 1) / 2;
        }
        int l1 = 2 * l - 1 >= u ? l - 1 : l;
        int r1 = 2 * r + 1 <= d ? r : r - 1;
        if (l1 <= r1) {
            res += y * mll(l1 + r1) * (r1 - l1 + 1) + y * mll(1 + y) * (r1 - l1 + 1) / 2;
        }
        return res;
    };

    auto box = [&] (int u, int d, int l, int r, int k) -> mll {
        return margin(u, d, r, k) - margin(u, d, l - 1, k);
    };

    auto ladder = [&] (int u, int d) -> mll {
        mll res = d - u + 1;
        int l = (u + 1) / 2;
        int r = d / 2;
        if (l <= r) {
            res += 2 * square(l, r) + mll(l + r) * (r - l + 1) / 2 - mll(r - l + 1);
        }
        int l1 = 2 * l - 1 >= u ? l - 1 : l;
        int r1 = 2 * r + 1 <= d ? r : r - 1;
        if (l1 <= r1) {
            res += 6 * square(l1, r1) + mll(l1 + r1) * (r1 - l1 + 1) / 2;
        }
        return res;
    };

    auto triangle = [&] (int u, int d) -> mll {
        return ladder(u, d) - margin(u, d, u - 1);
    };

    read(int, q);
    while (q--) {
        read(int, x1, y1, x2, y2, k);
    }
}
