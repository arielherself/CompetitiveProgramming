...
void solve() {
    ...
    for (int i = 0; i < n; ++i) {
        if (a[i] != a[(i + 1) % n]) {
            ...
            if (start != -1) {
                length[start] = i - start;
                start = -1;
            }
        }
    }
    if (start != -1 and start >= 2) {
        length[start] = n - start;
    }
    // debug(a);
    // debug(length);
    int i = 0;
    while (i < n) {
        if (length[i] != 0) {
            int left = a[mod(i - 1, n)], right = a[(i + length[i]) % n];
            int change = min(k, (length[i] + 1) / 2);
            for (int j = i; j < i + change; ++j) {
                a[j] = left;
            }
            for (int j = i + length[i] - change; j < i + length[i]; ++j) {
                a[j] = right;
            }
            for (int j = i + change; j < i + length[i] - change; ++j) {
                a[j] = 1 ^ a[j - 1];
            }
            i += length[i];
        } else {
    ...
    rotate(a.rbegin(), a.rbegin() + s, a.rend());
    for (int i = 0; i < n; ++i) {
        cout << (a[i] ? 'B' : 'W');
    }
    cout << endl;
