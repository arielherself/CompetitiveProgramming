...
void solve() {
    read(int, n);
    unordered_map<int, int, safe_hash> mp;  // map number to index
    vector<int> a;
    ...
    ll target = sum / n;
    vector<int> rev(tot);  // map index to bucket
    vector<ll> s(n);
    vector<int> ch(tot, -1);
    for (int i = 0; i < n; ++i) {
        s[i] = accumulate(bk[i].begin(), bk[i].end(), ll(0));
        for (auto&&  x : bk[i]) {
            ll y = target + x - s[i];
            if (y <= INT_MAX and y >= INT_MIN and mp.count(y)) {
                ch[mp[x]] = mp[y];
            }
            rev[mp[x]] = i;
        }
    ...
    vector<bitset<1 << 15>> dp((1 << n) + 1);
    dp[0][0] = 1;
    for (int k = 1; k <= (1 << n); ++k) {
        dp[k] = dp[k - 1];
        if (rings[k - 1].empty()) continue;
        int inv = ((1 << n) - 1) ^ (k - 1);
        for (int s = inv; ; s = (s - 1) & inv) {
            dp[k][s | (k - 1)] = dp[k][s | (k - 1)] | dp[k - 1][s];
            if (s == 0) break;
        }
    }
    mask = (1 << n) - 1;
    ...
    for (int i = 1 << n; i > 0; --i) {
        if (not dp[i - 1][mask]) {
            int l = rings[i - 1].size();
            for (int j = 0; j < l; ++j) {
                res[rev[rings[i - 1][(j + 1) % l]]] = {a[rings[i - 1][(j + 1) % l]], rev[rings[i - 1][j]] + 1};
            }
            mask ^= i - 1;
        }
    }
