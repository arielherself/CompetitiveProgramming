...
void solve() {
    read(int, n);
    readvec(string, a, n);
    vector<pair<array<int, 26>, pll>> trie(1);
    auto insert = [&] (const string& s) -> void {
        ...
        for (auto&& x : v) {
            stk.push_back(curr);
            if (!trie[curr].first[x - 97]) {
                trie.push_back({});
                trie[curr].first[x - 97] = trie.size() - 1;
            }
            curr = trie[curr].first[x - 97];
        }
        trie[curr].second.first += 1;
        trie[curr].second.second += v.size();
        for (auto&& i : stk) {
            trie[i].second.first += 1;
            trie[i].second.second += v.size();
        }
    };
    auto query = [&] (const string& s) -> ll {
        ...
        ll cp = 0;
        for (auto&& x : s) {
            int next = trie[curr].first[x - 97];
            auto [cnt, tot] = trie[curr].second;
            ll in_cnt = next ? trie[next].second.first : 0;
            ll in_tot = next ? trie[next].second.second : 0;
            res += tot - in_tot - cp * (cnt - in_cnt) + (cnt - in_cnt) * (s.size() - cp);
            // debug(res);
            if (!next) {
                return res;
            }
    ...
    // debug(trie[trie[trie[trie[0].first[0]].first[1]].first[0]].second);
    ll res = 0;
    for (auto&& s : a) {
        res += query(s);
        // cerr << endl;
