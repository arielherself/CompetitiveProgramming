...
void solve() {
    ...
    vector<pii> dpr(1 << n, { INF, 0 });
    vector<pii> dpb(1 << n, { INF, 0 });
    auto update = [] (pii& lhs, const pii& rhs) {
        if (rhs.first < lhs.first or rhs.first == lhs.first and rhs.second > lhs.second) {
            lhs = rhs;
        }
    };
    for (int i = 1; i < (1 << n); ++i) {
        if (popcount(i) == 1) {
            int j = msp(i);
            int red_need = a[j][1], blue_need = a[j][2];
            int extra = max(red_need, blue_need);
            if (extra - red_need == 0) {
                update(dpb[i], pii(extra, extra - blue_need));
            }
            if (extra - blue_need == 0) {
                update(dpr[i], pii(extra, extra - red_need));
            }
        } else {
            int cnt[2] = {};
            for (int j = 0; j < n; ++j) {
            ...
            for (int j = 0; j < n; ++j) {
                if (i >> j & 1) {
                    ...
                    int red_need = max(a[j][1] - cnt[0], 0);
                    int blue_need = max(a[j][2] - cnt[1], 0);
                    int from = i xor 1 << j;

                    {
                        auto [cost, red] = dpr[from];
                        int extra = max(max(0, red_need - red), blue_need);
                        if (red + extra - red_need == 0) {
                            update(dpb[i], pii(cost + extra, extra - blue_need));
                        }
                        if (extra - blue_need == 0) {
                            update(dpr[i], pii(cost + extra, red + extra - red_need));
                        }
                    }

                    {
                        auto [cost, blue] = dpb[from];
                        int extra = max(red_need, max(0, blue_need - blue));
                        if (blue + extra - blue_need == 0) {
                            update(dpr[i], pii(cost + extra, extra - red_need));
                        }
                        if (extra - red_need == 0) {
                            update(dpb[i], pii(cost + extra, blue + extra - blue_need));
                        }
                    }
    ...
    int all = (1 << n) - 1;
    cout << n + min(dpb[all].first, dpr[all].first) << '\n';
}
