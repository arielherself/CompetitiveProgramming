...
void solve() {
	...
	vector<bool> good(n);
	for (int i = 1; i < n - 1; ++i) {
		if (gcd(a[i], a[i - 1]) == 1 and gcd(a[i], a[i + 1]) == 1 and a[i - 1] != 1 and a[i + 1] != 1) {
			good[i] = 1;
		}
	}
	...
	for (int i = 0; i < n; ++i) {
		if (a[i] == 1) {
			...
			// [l, i - 1]
			vector<int> curr(i - l);
			iota(curr.begin(), curr.end(), l);
			if (l != 0) {
				two.emplace_back(curr);
			} else {
				one.emplace_back(curr);
			}
			l = i + 1;
		}
	}
	...
	sort_by_key(two.begin(), two.end(), expr(v.size(), auto&& v));
	for (auto&& v : two) {
		int t = v.size();
		if (t == 0) continue;
