...
class quick_union {
private:
    vector<size_t> c, t, sz;
public:
    quick_union(size_t n) : c(n), t(n), sz(n) {
...
void solve() {
    ...
    vector<int> sz;
    for (int i = 1; i <= n; ++i) {
        if (not bk[i]) continue;
        res += ll(1) * qu.query_size(i) * (qu.query_size(i) - 1) / 2 - qu.query_t(i);
