...
void solve() {
    ...
    for (int i = 0; i < n; ++i) {
        ...
        if (a[i] >= 'A' and a[i] <= 'Z') {
            for (int j = 0; j < 8; ++j) {
                ndp[10 + a[i] - 'A'][j | 0b010] += sum[j] - dp[10 + a[i] - 'A'][j];
            }
        } else if (a[i] >= '0' and a[i] <= '9') {
            for (int j = 0; j < 8; ++j) {
                ndp[a[i] - '0'][j | 0b100] += sum[j] - dp[a[i] - '0'][j];
            }
            ...
            // lowercase
            for (int j = 0; j < 8; ++j) {
                ndp[10 + 26 + a[i] - 'a'][j | 0b001] += sum[j] - dp[10 + 26 + a[i] - 'a'][j];
            }
        } else {  // is question mark
            // numeric
            for (int k = 0; k < 10; ++k) {
                for (int j = 0; j < 8; ++j) {
                    ndp[k][j | 0b100] += sum[j] - dp[k][j];
                }
            }
            // uppercase
            for (int k = 0; k < 26; ++k) {
                for (int j = 0; j < 8; ++j) {
                    ndp[10 + k][j | 0b010] += sum[j] - dp[10 + k][j];
                }
            }
            // lowercase
            for (int k = 0; k < 26; ++k) {
                for (int j = 0; j < 8; ++j) {
                    ndp[10 + 26 + k][j | 0b001] += sum[j] - dp[10 + 26 + k][j];
                }
            }
        }
        for (int k = 0; k < N; ++k) {
