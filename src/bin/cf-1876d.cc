...
void solve() {
	...
	for (int i = 0; i < n; ++i) {
		if (q.empty()) {
			for (int j = prev; j < i; ++j) {
				while (oc[a[j]].size()) {
					if (not qu.connected(oc[a[j]].back(), t - 1)) {
						m += 1;
						qu.merge(oc[a[j]].back(), t - 1);
					}
					oc[a[j]].pop_back();
				}
				oc[a[j]].emplace_back(t - 1);
			}
			prev = i;
			t += 1;
		}
	...
	ll res = mod(pw[all] - exclude, M) * inverse(2, M) % M;
	cout << res << '\n';
}
