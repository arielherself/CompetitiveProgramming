...
void solve() {
    ...
    vector<int> l(n), r(n);
    for (int i = 0; i < n; ++i) {
        oc.emplace_back(a[i].first);
        oc.emplace_back(a[i].second);
        tie(l[i], r[i]) = a[i];
    }
    sort(oc.begin(), oc.end());
    int m = unique(oc.begin(), oc.end()) - oc.begin();
    sparse_table<int> range_max(l.rbegin(), l.rend(), functor(max));
    sparse_table<int> range_min(r.rbegin(), r.rend(), functor(min));
    vector<array<ll, 2>> dp(n + 1);
    vector<array<pii, 2>> dpnxt(n + 1);
    for (int i = n - 1; ~i; --i) {
        for (auto&& [j, x] : enumerate(vector<int> { a[i].first, a[i].second })) {
            int l = i + 1, r = n - 1;
            while (l < r) {
                int mid = l + r >> 1;
                if (range_max.query(i + 1, mid) > x or range_min.query(i + 1, mid) < x) {
                    r = mid;
                } else {
                    l = mid + 1;
                }
            ...
            if (l >= i + 1 and l < n) {
                int mx = range_max.query(i + 1, l);
                int mn = range_min.query(i + 1, l);
                if (mx > x) {
                    dp[i][j] = mx - x + dp[l][0];
                    dpnxt[i][j] = { l, 0 };
                } else if (mn < x) {
                    if (x - mn + dp[l][1] < x - a[l].first + dp[l][0]) {
                        dp[i][j] = x - mn + dp[l][1];
                        dpnxt[i][j] = { l, 1 };
                    } else {
                        dp[i][j] = x - a[l].first + dp[l][0];
                        dpnxt[i][j] = { l, 0 };
                    }
                } else {
                    dpnxt[i][j] = { n, 0 };
    ...
    vector<ll> val(m, INFLL);
    vector<pii> nxt(m);
    for (int i = 0; i < m; ++i) {
        if (oc[i] < a[0].first or oc[i] > a[0].second) continue;
        int x = oc[i];
        int l = 1, r = n - 1;
        while (l < r) {
            int mid = l + r >> 1;
            if (range_max.query(1, mid) > x or range_min.query(1, mid) < x) {
                r = mid;
            } else {
                l = mid + 1;
            }
        ...
        if (l >= 1 and l < n) {
            int mx = range_max.query(1, l);
            int mn = range_min.query(1, l);
            if (mx > x) {
                val[i] = mx - x + dp[l][0];
                nxt[i] = { l, 0 };
            } else if (mn < x) {
    ...
    int ptr = 0, loc = 0;
    while (ptr < n) {
        pii nxt_pos = ptr == 0 ? nxt[varmin] : dpnxt[ptr][loc];
        int curr_val = ptr == 0 ? oc[varmin] : loc == 0 ? a[ptr].first : a[ptr].second;
        for (int j = ptr; j < nxt_pos.first; ++j) {
            res[j] = curr_val;
        }
        tie(ptr, loc) = nxt_pos;
    }
    for (int i = n - 1; ~i; --i) {
        if (i < n - 1 and res[i + 1] < res[i] and res[i + 1] >= a[i].first and res[i + 1] <= a[i].second) {
            res[i] = res[i + 1];
        }
    }
    putvec(res);
