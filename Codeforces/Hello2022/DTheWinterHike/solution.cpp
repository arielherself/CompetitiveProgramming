...
void solve() {
    ...
    vector<vector<pil>> ch((2 * n) * (2 * n));
    for (int i = 0; i < 2 * n; ++i) {
        for (int j = 0; j < 2 * n; ++j) {
            int curr = serialize(i, j), up = serialize(mod(i - 1, 2 * n), j), down = serialize(mod(i + 1, 2 * n), j);
            int left = serialize(i, mod(j - 1, 2 * n)), right = serialize(i, mod(j + 1, 2 * n));
            // up
            ch[curr].emplace_back(up, c[up]);
            // down
            ch[curr].emplace_back(down, c[down]);
            // left
            ch[curr].emplace_back(left, c[left]);
            // right
            ch[curr].emplace_back(right, c[right]);
        }
    }
    vector<ll> dis(2 * 2 * n * n, INFLL);
