...
void solve() {
    ...
    for (auto&& x : st) mp[x] = ++N;
    BIT<int> tr(N);
    set<ll> left, right, oc;
    auto update = [&] (set<ll>::iterator it) -> void {
        left.erase(*it), right.erase(*it);
        if (next(it) == oc.end() or *next(it) - *it > k) {
            right.emplace(*it);
        }
        if (it == oc.begin() or *it - *prev(it) > k) {
            left.emplace(*it);
        }
    };
    for (auto&& [op, x] : queries) {
        if (op == 1) {
            if (oc.count(x)) {
                // delete it
                right.erase(x), left.erase(x);
                oc.erase(x);
                tr.add(mp[x], -1);
                auto it = oc.lower_bound(x);
                if (it != oc.end()) {
                    update(it);
                }
                if (it != oc.begin()) {
            ...
            ll r = *right.lower_bound(x);
            ll l = *prev(left.upper_bound(x));
            cout << tr.getsum(mp[r]) - tr.getsum(mp[l] - 1) << '\n';;
        }
    }
}
