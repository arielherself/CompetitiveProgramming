...
void solve() {
    ...
    {
        auto dfs = [&] (auto dfs, int v, int pa) -> void {
            down[v].second = sz[v] - 1;
            int tot_sz = 0;
            for (auto&& u : ch[v]) {
                if (u == pa) continue;
                dfs(dfs, u, v);
                down[v].first += down[u].first + down[u].second;
                down[v].second += down[u].second;
                down[v].second += ll(1) * tot_sz * sz[u];
                tot_sz += sz[u];
            }
        };
        dfs(dfs, 1, 0);
    ...
    vector<pll> up(n + 1);
    {
        auto dfs = [&] (auto dfs, int v, int pa) -> void {
            ...
            for (auto&& u : ch[v]) {
                if (u == pa) continue;
                // up[v].first += down[u].first;
                // up[v].second += down[u].second;
            }
            up[v].second += n - sz[v];
            up[v].second += ll(1) * (n - sz[v]) * (sz[v] - 1);
            // if (v == 1) {
            //     debug(up[v]);
            //     debug(down[v]);
            // }
            for (auto&& u : ch[v]) {
                if (u == pa) continue;
                up[v].second -= ll(1) * sz[u] * (n - sz[v]);
                down[v].second -= ll(1) * sz[u] * (sz[v] - sz[u]);
                down[v].first -= down[u].first + down[u].second;
                down[v].second -= down[u].second;
                // if (v == 1) {
                //     debug(u);
                ...
                up[u].first += up[v].first + up[v].second;
                up[u].first += down[v].first + down[v].second;
                up[u].second += up[v].second;
                up[u].second += down[v].second;
                dfs(dfs, u, v);
