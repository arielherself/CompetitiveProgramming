...
void solve() {
	...
	vector<int> heavy(n + 1);
	{
		auto dfs = [&] (auto dfs, int v, int pa) -> void {
			max_depth[v] = depth[v] = depth[pa] + a[v];
			for (auto&& u : ch[v]) {
				if (u == pa) continue;
				dfs(dfs, u, v);
				if (max_depth[u] >= max_depth[heavy[v]]) {
					heavy[v] = u;
				}
	...
	vector<ll> cand;
	{
		auto dfs = [&] (auto dfs, int v, int pa, int start) -> ll {
			ll curr = a[v];
			if (heavy[v] != 0) {
				curr += dfs(dfs, heavy[v], v, false);
			}
			if (start) {
				cand.emplace_back(curr);
			}
			for (auto&& u : ch[v]) {
	...
	chmin(k, cand.size());
	partial_sort(cand.begin(), cand.begin() + k, cand.end(), greater());
	cout << accumulate(cand.begin(), cand.begin() + k, ll(0)) << '\n';
}
