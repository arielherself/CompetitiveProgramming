...
void solve() {
	...
	read(int, n);
	readvec(pii, a, n);
	vector cnt(U, vector<int>(n + 1));
	for (int j = 0; j < U; ++j) {
		for (int i = 0; i < n; ++i) {
			int target = a[i].second & ((1 << U) - (1 << j + 1)) | ((1 << j) - 1);
			if (target >= a[i].first and target <= a[i].second) {
				cnt[j][i + 1] = 1;
			}
		}
		for (int i = 1; i <= n; ++i) {
	...
	vector has(U, vector<int>(n + 1));
	for (int j = 0; j < U; ++j) {
		for (int i = 0; i < n; ++i) {
			has[j][i + 1] = has[j][i] + (a[i].second >> j & 1);
		}
	}

	vector<int> b(n);
	transform(a.begin(), a.end(), b.begin(), expr(p.second, auto&& p));
	sparse_table<int> st(b.rbegin(), b.rend(), expr(x | y, auto&& x, auto&& y));
	...
	while (q--) {
		...
		for (int j = U - 1; ~j; --j) {
			if (has[j][r + 1] - has[j][l] >= 2 and cnt[j][r + 1] - cnt[j][l] >= 1) {
				res |= (1 << j) - 1;
				break;
			}
		}
