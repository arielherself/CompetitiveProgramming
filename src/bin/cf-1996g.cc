...
void solve() {
    ...
    segtree<Info, Tag> tr(2 * n);
    for (int i = 0; i < 2 * n; ++i) tr.set(i, { 0, 1 });

    vector<vector<int>> q(n + 1);
    while (m--) {
        read(int, u, v);
        q[v].emplace_back(u);
        tr.range_apply(n + u, n + v - 1, { 1 });
    }
    // for (auto&& [i, x] : enumerate(tr.serialize())) {
    //     cerr << i << ": " << x.minval << ", ";
    // }
    // cerr << endl;
    ...
    int res = 0;
    for (int i = 2 * n; i > n; --i) {
        auto [v, c] = tr.range_query(i - n + 1, i - 1);
        if (v == 0) {
            chmax(res, c);
        }
        for (auto&& j : q[i - n]) {
            if (j > i - n) {
                tr.range_apply(j, i - 1, { -1 });
                tr.range_apply(i - n, j - 1, { 1 });
            } else {
                tr.range_apply(n + j, i - 1, { -1 });
                tr.range_apply(i - n, n + j - 1, { 1 });
                q[j].emplace_back(i - n);
            }
        }
        // for (auto&& [i, x] : enumerate(tr.serialize())) {
        //     cerr << i << ": " << x.minval << ", ";
