...
void solve() {
    ...
    auto calc_pre = [&] (int i) {
        return mll(1) * max(0, c0 - i) * max(0, c1 - i) / (n * (n - 1) / 2);
    };

    auto calc_nxt = [&] (int i) {
        return mll(1) * i * i / (n * (n - 1) / 2);
    };

    vector c(n + 1, vector<mll>(n + 1));
    for (int i = 0; i <= n; ++i) {
        if (i > 0) c[i][i - 1] = calc_pre(i - 1);
        if (i < n) c[i][i + 1] = calc_nxt(i + 1);
        c[i][i] = 1 - calc_pre(i) - calc_nxt(i);
    }

    vector unit(n + 1, vector<mll>(n + 1));
    for (int i = 0; i <= n; ++i) {
        unit[i][i] = 1;
    }

    auto multiply = [&] (const vector<vector<mll>>& a, const vector<vector<mll>>& b) -> vector<vector<mll>> {
        vector res(n + 1, vector<mll>(n + 1));
        for (int i = 0; i <= n; ++i) {
            for (int j = 0; j <= n; ++j) {
                for (int k = 0; k <= n; ++k) {
                    res[i][j] += a[i][k] * b[k][j];
    ...
    auto qpow = [&] (auto&& qpow, const vector<vector<mll>>& curr, int p) -> vector<vector<mll>> {
        if (p == 0) return unit;
        auto half = qpow(qpow, curr, p / 2);
        if (p % 2 == 1) {
            return multiply(multiply(half, half), curr);
        } else {
