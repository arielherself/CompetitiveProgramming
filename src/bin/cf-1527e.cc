...
Info operator+(const Info &a, const Info &b) {
    ...
    for (int i = 0; i <= 100; ++i) {
        ret.val[i] = min(a.val[i] + a.extra, b.val[i] + b.extra);
    }
...
void solve() {
    ...
    for (int i = 0; i < n; ++i) {
        if (prev[a[i]] != -1) {
            dp.range_apply(0, prev[a[i]], { i - prev[a[i]] });
        }
        prev[a[i]] = i;
        Info best = dp.range_query(0, i);
        Info curr;
        for (int j = 0; j < k; ++j) {
            curr.val[j + 1] = best.val[j] + best.extra;
        }
        dp.set(i + 1, curr);
    }

    cout << dp.query(n).val[k] << '\n';
}
