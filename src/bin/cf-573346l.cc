...
void solve() {
    ...
    for (int i = 0; i < n; ++i) {
        ...
        ps[i][0] = c[i][idx[i][0].second];
        for (int j = 1; j < m; ++j) {
            ps[i][j] = min(ps[i][j - 1], c[i][idx[i][j].second]);
        }
        // debug(idx[i]);
    }
    auto check = [&] (int mx) -> bool {
        ...
        for (int i = 0; i < n; ++i) {
            rem += k;
            int j = lower_bound(idx[i].begin(), idx[i].end(), pii(mx + 1, 0)) - idx[i].begin();
            if (j == 0) return false;
            int curr = ps[i][j - 1];
            rem -= curr;
