...
void solve() {
    read(int, n);
    read(string, a);
    transform(a.begin(), a.end(), a.begin(), expr(x - 'a', char x));
    string b(a.rbegin(), a.rend());
    vector<deque<int>> bk(26);
    for (int i = 0; i < n; ++i) {
        bk[a[i]].emplace_back(i);
    }
    segtree<Info, Tag> stale(n);
    ll res = 0;
    int ptr = 0;
    for (int i = 0; i < n; ++i) {
        while (stale.query(ptr).val) ++ptr;
        if (a[ptr] != b[i]) {
            while (bk[b[i]].front() < ptr) {
                bk[b[i]].pop_front();
            }
            int pos = bk[b[i]].front();
            bk[b[i]].pop_front();
            res += pos - ptr - stale.range_query(ptr, pos).val;
            stale.set(pos, { 1 });
        } else {
            ++ptr;
        }
