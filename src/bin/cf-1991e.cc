...
void solve() {
    ...
    bool alice = false;
    auto dfs = [&] (auto dfs, int v, int curr) {
        if (col[v] == 1 - curr) {
            alice = true;
            return;
        } else if (col[v] == curr) {
    ...
    if (alice) {
        ...
        for (int i = 1; i <= n; ++i) {
            q[col[i]].emplace_back(i);
        }

        for (int i = 0; i < n; ++i) {
            read(int, x, y);
            if (q[0].size() == 0) {
                cout << q[1].back() << ' ' << (x == 2 or y == 2 ? 2 : 3) << endl;
                q[1].pop_back();
            } else if (q[1].size() == 0) {
                cout << q[0].back() << ' ' << (x == 1 or y == 1 ? 1 : 3) << endl;
                q[0].pop_back();
            } else {
                if (x == 1 or y == 1) {
                    cout << q[0].back() << ' ' << 1 << endl;
                    q[0].pop_back();
                } else {
                    cout << q[1].back() << ' ' << 2 << endl;
                    q[1].pop_back();
                }
            }
        }
