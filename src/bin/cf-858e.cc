...
void solve() {
    ...
    for (int i = 0; i < n; ++i) {
        read(string, s);
        read(int, c);
        a[1 - c].emplace_back(s);
    }
    int left = a[0].size(), right = a[1].size();
    vector<string> lr, la;
    vector<bool> use(n + 1);
    for (int i = 0; i < left; ++i) {
        ...
        if (f == 1 and a[0][i][0] != '0') {
            int x = stoi(a[0][i]);
            if (x >= 1 and x <= left) {
                use[x] = 1;
            } else if (x > left and x <= n) {
                use[x] = 1;
                lr.emplace_back(a[0][i]);
            } else {
                la.emplace_back(a[0][i]);
            }
        } else {
            la.emplace_back(a[0][i]);
        }
    ...
    for (int i = 0; i < right; ++i) {
        ...
        if (f == 1 and a[1][i][0] != '0') {
            int x = stoi(a[1][i]);
            if (x > left and x <= n) {
                use[x] = 1;
            } else if (x >= 1 and x <= left) {
                use[x] = 1;
                rl.emplace_back(a[1][i]);
            } else {
                ra.emplace_back(a[1][i]);
            }
        } else {
            ra.emplace_back(a[1][i]);
        }
    ...
    if (lr.size() and lr.size() == rl.size() and le.empty() and re.empty()) {
        assert(la.empty() and ra.empty());
        string x = popback(lr);
        int j = stoi(x);
        use[j] = 0;
        re.emplace_back(j);
        la.emplace_back("subcpp");
        res.emplace_back("move " + x + " subcpp");
    }

    while (1) {
        int f = 0;
        if (le.size() and lr.size()) {
            f = 1;
            int i = popback(le);
            use[i] = 1;
            string x = popback(lr);
            ...
            use[j] = 0;
            re.emplace_back(j);
            res.emplace_back("move " + x + " " + to_string(i));
        }
        if (re.size() and rl.size()) {
            f = 1;
            int i = popback(re);
            use[i] = 1;
            string x = popback(rl);
