...
struct Info {
    ...
    Info(array<int, 31>&& val) : val(val) {}
    void apply(const Tag& rhs, size_t len) { }
};

array<int, 31> merge(const array<int, 31>& a, const array<int, 31>& b) {
    int l = 0, r = 0;
    array<int, 31> res;
    for (int i = 0; i < 31; ++i) {
        if (l == 31 or r != 31 and a[l] > b[r]) {
            res[i] = b[r];
            r += 1;
        } else {
            res[i] = a[l];
...
Info operator+(const Info &a, const Info &b) {
    return { merge(a.val, b.val) };
}

void solve() {
    read(int, n);
    readvec(int, a, n);
    segtree<Info, Tag> st(n);
    for (int i = 0; i < n; ++i) {
        array<int, 31> curr;
        curr.fill(INT_MAX);
        curr[0] = a[i];
        st.set(i, { std::move(curr) });
    }
    read(int, q);
    while (q--) {
        read(int, l, r);
        --l, --r;
        array<int, 31> curr = st.range_query(l, r).val;
        int res = INT_MAX;
        for (int i = 0; i < 31; ++i) {
            for (int j = i + 1; j < 31; ++j) {
                chmin(res, curr[i] | curr[j]);
            }
        }
        cout << res << '\n';
    }
