...
void solve() {
    ...
    sparse_table<int> min_rmq(a.rbegin(), a.rend(), functor(min));
    sparse_table<int> max_rmq(a.rbegin(), a.rend(), functor(max));

    set<tuple<int, int, int, int>> info;
    int last = 0;
    for (int i = 1; i < n; ++i) {
        if (s[i - 1] == 'L' and s[i] == 'R') {
            info.emplace(last, min_rmq.query(last, i - 1), max_rmq.query(last, i - 1), i - 1);
            last = i;
        }
    }
    info.emplace(last, min_rmq.query(last, n - 1), max_rmq.query(last, n - 1), n - 1);

    int invalid = 0;

    for (auto it = info.begin(); it != info.end(); ++it) {
        if (it != info.begin() and get<1>(*it) < get<2>(*prev(it))) {
            invalid += 1;
        }
    }

    auto check = [&] (auto&& a, auto&& b) {
        return get<3>(*a) + 1 != get<0>(*b) or get<1>(*b) >= get<2>(*a);
    };

    auto insert = [&] (int l, int r) {
        auto nw = make_tuple(l, min_rmq.query(l, r), max_rmq.query(l, r), r);
        info.emplace(nw);
        auto it = info.lower_bound(nw);
        if (it != info.begin()) {
            if (not check(prev(it), it)) {
                ++invalid;
            }
        }
        if (next(it) != info.end()) {
            if (not check(it, next(it))) {
                ++invalid;
            }
        }
    };
    ...
    auto remove = [&] (auto&& it) {
        if (it != info.begin() and not check(prev(it), it)) {
            --invalid;
        }
        if (next(it) != info.end() and not check(it, next(it))) {
            --invalid;
        }
        info.erase(it);
    };
    ...
    while (q--) {
        ...
        if (s[i] == 'L' and i + 1 < n and s[i + 1] == 'R') {
            auto r = info.lower_bound(make_tuple(i + 1, 0, 0, 0));
            auto l = prev(r);
            int left = get<0>(*l), right = get<3>(*r);
            remove(l), remove(r);
            insert(left, right);
        } else if (s[i] == 'R' and i - 1 >= 0 and s[i - 1] == 'L') {
            auto r = info.lower_bound(make_tuple(i, 0, 0, 0));
            auto l = prev(r);
            int left = get<0>(*l), right = get<3>(*r);
            remove(l), remove(r);
            insert(left, right);
        ...
        if (s[i] == 'L' and i + 1 < n and s[i + 1] == 'R') {
            auto l = prev(info.lower_bound(make_tuple(i, 0, 0, 0)));
            int left1 = get<0>(*l), right2 = get<3>(*l);
            int right1 = i, left2 = i + 1;
            remove(l);
            insert(left1, right1);
            insert(left2, right2);
        } else if (s[i] == 'R' and i - 1 >= 0 and s[i - 1] == 'L') {
