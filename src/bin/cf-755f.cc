...
void solve() {
    ...
    // evaluate minimum
    {
        ...
        for (int i = 1; i < M; ++i) {
            deque<pair<ull, int>> q;
            ull mask = 0;
            for (int j = 0; j * B <= M; ++j) {
                while (q.size() and j - q.front().second > cnt[i])
            }
        }
        for (auto&& x : c) {
            if (x >= M) {
    ...
    // evaluate maximum
    {
        ...
        int res = 0;
        int rem = 0;
        for (int i = 0; i < m and cnt < k; ++i) {
            int use = min(k - cnt, c[i] / 2);
            cnt += use;
            res += use * 2;
            if (c[i] - use * 2 > 0) {
                rem += 1;
