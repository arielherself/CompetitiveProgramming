...
void solve() {
    ...
    vector<int> freelist;
    for (int i = 1; i <= n; ++i) {
        if (not heavy[i]) continue;
        int extra = 0;
        // add virtual vertices and count
        {
            auto dfs = [&] (auto dfs, int v, int idx) -> void {
                for (int j = 0; j < 26; ++j) {
                    int u = trie[v][j].first;
                    if (u) {
                        if (trie[idx][j].first == 0) {
                            // deb(u, idx);
                            extra += 1;
                            if (freelist.empty()) {
                                freelist.emplace_back(trie.size());
                                trie.emplace_back();
                            }
                            trie[idx][j] = { popback(freelist), true };
                        }
                        dfs(dfs, u, trie[idx][j].first);
                    }
                }
            };
            for (int j = 0; j < 26; ++j) {
                int u = trie[i][j].first;
                if (u and u != heavy[i]) {
                    dfs(dfs, u, heavy[i]);
                }
            }
        ...
        // deb(i, extra);
        c[depth[i]] += sz[i] - (sz[heavy[i]] + extra);
        // remove virtual vertices
        {
            auto dfs = [&] (auto dfs, int v, int idx) -> void {
                for (int j = 0; j < 26; ++j) {
                    int u = trie[v][j].first;
                    if (u) {
                        if (trie[idx][j].first == 0) continue;
                        if (trie[idx][j].second == true) {
                            int o = trie[idx][j].first;
                            trie[idx][j] = {};
                            auto dfs = [&] (auto dfs, int v) -> void {
                                ...
                                for (int j = 0; j < 26; ++j) {
                                    int u = trie[v][j].first;
                                    if (u) {
                                        assert(trie[v][j].second == true);
                                        int o = trie[v][j].first;
                                        trie[v][j] = {};
                                        dfs(dfs, o);
                                    }
                                }
