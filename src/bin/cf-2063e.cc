...
void solve() {
	...
	BIT<Info> tr(n);
	{
		auto dfs = [&] (auto dfs, int v, int pa, bool keep) -> void {
			for (auto&& u : ch[v]) {
			...
			auto add = [&] (auto add, int u, int pa) -> void {
				auto [cnt, sum] = tr.getsum(depth[u] - 1);
				{
					// auto [cnt, sum] = tr.range_query(0, depth[u] - 1);
					res += 2 * (sum - ll(1) * cnt * depth[v]) - cnt;
				}
				{
					int cc = totcnt - cnt;
					// ll ss = totsum - sum;
					// auto [cnt, sum] = tr.range_query(depth[u], n);
					res += ll(2) * cc * (depth[u] - depth[v]) - cc;
				}
				for (auto&& w : ch[u]) {
					if (w == pa) continue;
					add(add, w, u);
			...
			auto apply = [&] (auto apply, int u, int pa) -> void {
				totcnt += 1;
				tr.add(depth[u], {
						.cnt = 1,
						.sum = depth[u],
			...
			auto del = [&] (auto del, int u, int pa) -> void {
				totcnt -= 1;
				tr.add(depth[u], {
						.cnt = -1,
						.sum = -depth[u],
