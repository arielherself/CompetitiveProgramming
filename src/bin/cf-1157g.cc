...
void solve() {
    ...
    auto work = [&] (const vector<vector<int>>& b) -> optional<pair<vector<int>, vector<int>>> {
        vector<int> row(n), col(m);
        for (int j = 0; j < m; ++j) {
            col[j] = a[0][j] != b[0][j];
        }
        for (int i = 1; i < n; ++i) {
            row[i] = a[i][0] != (b[i][0] ^ col[0]);
        }
        int f = 1;
        int prev = 0;
        for (int i = 0; i < n; ++i) {
            for (int j = 0; j < m; ++j) {
                if (prev > (a[i][j] ^ row[i] ^ col[j])) {
                    f = 0;
                    goto fi;
                }
                prev = a[i][j] ^ row[i] ^ col[j];
            }
        }
    fi:
        if (f) {
