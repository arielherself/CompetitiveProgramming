...
void prep() {
    fact[0] = factrev[0] = 1;
    for (int i = 1; i < N; ++i) {
        fact[i] = (ll(1) * fact[i - 1] * i) % PRIME;
        factrev[i] = inverse(fact[i], PRIME);
    }
}
...
void solve() {
    ...
    int sq = sqrt(2 * m) + 5;
    vector dp(m + 1, vector<int>(sq));
    vector<int> val(m + 5);
    dp[0][2] = (ll(1) * g * g) % PRIME;
    for (int i = 3; i < m + 5; ++i) {
        for (int j = m; ~j; --j) {
            for (int k = sq - 1; ~k; --k) {
                // choose 'e'
                int nw = j + k * (k - 1) / 2;
                if (nw <= m) {
                    dp[nw][k] = (dp[nw][k] + ((ll(1) * e * dp[j][k]) % PRIME)) % PRIME;
                }
                // choose 'g'
                if (k + 1 < sq) {
                    dp[j][k + 1] = (dp[j][k + 1] + ((ll(1) * g * dp[j][k])) % PRIME) % PRIME;
                }
            }
        }
        for (int k = 0; k < sq; ++k) {
            val[i] = (val[i] + dp[m][k]) % PRIME;
        }
    }
    debug(val);
    vector<int> ps(n + 1);
    ...
    epw[0] = opw[0] = 1;
    for (int i = 1; i <= n; ++i) {
        epw[i] = (ll(1) * epw[i - 1] * e) % PRIME;
        opw[i] = (ll(1) * opw[i - 1] * o) % PRIME;
    }
    for (int i = 1; i <= n; ++i) {
        if (i > m + 4) {
            ps[i] = ps[i - 1];
        } else {
            ps[i] = (ps[i - 1] + ((ll(1) * val[i] * epw[n - i]) % PRIME)) % PRIME;
        }
    }
    vector<int> by_len(n + 1);
    for (int i = 3; i <= n; ++i) {
        by_len[i] = (ll(1) * ps[i] * inverse(epw[n - i], PRIME)) % PRIME;
    }
    int res = 0;
    for (int i = 1; i <= n; ++i) {
        int curr = 0;
        for (int j = 0; j <= i; ++j) {
            curr += (((((((ll(1) * fact[i] * factrev[j]) % PRIME) * factrev[i - j]) % PRIME) * by_len[j]) % PRIME) * opw[i - j]) % PRIME;
            curr %= PRIME;
        }
        res += (curr * (n - i + 1)) % PRIME;
        res %= PRIME;
    }
    cout << res << '\n';
}
