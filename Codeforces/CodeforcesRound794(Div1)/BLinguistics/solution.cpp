...
void solve() {
    ...
    read(string, s);
    int n = a + b + 2 * c + 2 * d;
    int cnt_a = count(s.begin(), s.end(), 'A'), cnt_b = n - cnt_a;
    ...
    for (int i = 1; i <= n; ++i) {
        if (s[i] == s[i - 1]) {
            if (prev_pos + 1 == i) {}
                ...
                if ((i - prev_pos) % 2 == 0) {
                    if (prev == 'A') {
                        int use = min(d, (i - prev_pos) / 2 - 1);
                        d -= use;
                        int rem = use == 0 ?  (i - prev_pos) / 2 : (i - prev_pos) / 2 - 1 - use;
                        c = max(0, c - rem);
                    } else {
                        int use = min(c, (i - prev_pos) / 2 - 1);
                        c -= use;
                        int rem = use == 0 ?  (i - prev_pos) / 2 : (i - prev_pos) / 2 - 1 - use;
                        d = max(0, d - rem);
                    }
                } else {
                    int use = min(c, (i - prev_pos) / 2);
                    c -= use;
                    int rem = (i - prev_pos) / 2 - use;
                    d = max(0, d - rem);
                }
            }
            prev_pos = i;
