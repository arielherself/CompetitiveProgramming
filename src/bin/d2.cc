...
void solve() {
    ...
    readvec(int, a, n);
    readvec(int, b, m);
    vector dp(m + 1, segtree<Info, Tag>(n + 1));
    for (int j = 0; j <= m; ++j) {
        dp[j].set(0, {
            .val = 0,
            .cnt = 1,
    ...
    for (int i = 1; i <= n; ++i) {
        ...
        for (int j = m; j; --j) {
            ...
            if (p != i) {
                auto prev = dp[j].range_query(p, i - 1);
                prev.val += m - j;
                dp[j].set(i, prev);
            }
        }
        for (int j = 1; j <= m; ++j) {
            dp[j].set(i, dp[j].query(i) + dp[j - 1].query(i));
        }
    }
    auto&& [val, cnt] = dp[m].query(n);
    if (val >= INF) {
        cout << -1 << '\n';
    } else {
        cout << val << ' ' << cnt << '\n';
