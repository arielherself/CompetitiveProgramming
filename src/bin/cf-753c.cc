...
void solve() {
    ...
    if (left.size() != 3 or right == -1) {
        exit(827);
    }

    vector<bool> vis(10);
    for (auto&& x : left) vis[x] = 1;
    vis[right] = 1;
    ...
    auto [a1, _1] = ask({ left[0], left[1], left[1], ex });
    auto [a2, _2] = ask({ left[1], left[1], left[0], ex });
    array<int, 4> res;
    if (a1 == 2 and a2 == 1) {
        res = { left[0], left[1], left[2], right };
    } else if (a1 == 2 and a2 == 0) {
        res = { left[0], left[2], left[1], right };
    } else if (a1 == 1 and a2 == 0) {
        res = { left[2], left[0], left[1], right };
    } else if (a1 == 0 and a2 == 1) {
        res = { left[1], left[0], left[2], right };
    } else if (a1 == 0 and a2 == 2) {
        res = { left[1], left[2], left[0], right };
    } else if (a1 == 1 and a2 == 2) {
        res = { left[2], left[1], left[0], right };
    } else {
        exit(828);
    }
