...
struct Info {
    ...
    void apply(const Tag& rhs, size_t len) {
        if (rhs.val < 0 or val <= LLONG_MAX - rhs.val) {
            val += rhs.val;
        } else {
            val = LLONG_MAX;
        }
...
void solve() {
    ...
    readvec(int, a, k);
    readvec(int, b, k);
    segtree<Info, Tag> row(k + 1), col(k + 1);
    // vector<bool> row_enable(k + 1), col_enable(k + 1);
    row.set(0, {0});
    col.set(0, {0});
    int prev = 0;
    for (auto&& i : idx) {
        if (i == prev) {
            row.range_apply(0, k, {b[i - 1]});
            col.range_apply(0, k, {b[i - 1]});
        } else {
            ll row_query = row.query(i).val;
            ll col_query = col.query(i).val;
            ll prev_row = min(row.range_query(0, k).val + a[i - 1], row_query != LLONG_MAX ? row_query + b[i - 1] : LLONG_MAX);
            ll prev_col = min(col.range_query(0, k).val + a[i - 1], col_query != LLONG_MAX ? col_query + b[i - 1] : LLONG_MAX);
            row.range_apply(0, k, {a[i - 1]});
            col.range_apply(0, k, {a[i - 1]});
            row.apply(i, {b[i - 1] - a[i - 1]});
            col.apply(i, {b[i - 1] - a[i - 1]});
            row.set(prev, {prev_col});
            col.set(prev, {prev_row});
            // for (auto&& x : row.serialize()) {
            //     cerr << x.val << ' ';;
            // }
            // cerr << endl;
            // for (auto&& x : col.serialize()) {
            //     cerr << x.val << ' ';;
            // }
            // cerr << endl;
        }
    ...
    cout << min(row.range_query(0, k).val, col.range_query(0, k).val) << '\n';
}
