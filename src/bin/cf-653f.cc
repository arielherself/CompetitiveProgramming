...
void solve() {
	...
	vector<int> right(n + 1);
	{
		vector<int> stack;
		for (int i = n; ~i; --i) {
			while (stack.size() and ss[stack.back()] <= ss[i]) {
				stack.pop_back();
			}
			if (stack.size()) {
				right[i] = stack.back();
	...
	for (int i = 0; i < n; ++i) {
		...
		int l = j + 1, r = right[j];
		if (i) {
			l = j + sa.lcp(j, sa.sa[i - 1]) + 1;
		}
		// deb(i, j, l, r);
		// debug(bk[ss[j]]);
		res += max<long>(0, lower_bound(bk[ss[j]].begin(), bk[ss[j]].end(), r) - lower_bound(bk[ss[j]].begin(), bk[ss[j]].end(), l));
	}
