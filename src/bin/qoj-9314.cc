...
void solve() {
    ...
    auto check = [&] (int x) -> bool {
        // stage 1: a[i]'s contribution to b[l, r]
        ...
        vector ps(n, vector<int>(n + 1));
        for (int i = 0; i < n; ++i) {
            for (int j = 1; j - 1 <= i; ++j) {
                ps[i][j] = ps[i][j - 1] + b[j - 1][i];
            }
        }

        // stage 2: b[l, r]'s contribution to c[l, r]
        int valid = 0;
        for (int i = 0; i < n; ++i) {
            int curr = 0;
            for (int j = i; j < n; ++j) {
                curr += ps[j][j + 1] - ps[j][i];
                int cnt = (j - i + 1) * (j - i + 2) / 2;
                if (curr >= (cnt + 1) / 2) {
                    valid += 1;
                }
            }
