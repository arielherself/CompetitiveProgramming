...
void solve() {
    ...
    while (q--) {
        read(int, ax, ay, bx, by);
        ld cx = (ax + bx) / 2.;
        ld cy = (ay + by) / 2.;
        ld r = sqrt(dis(ax, ay, bx, by)) / 2;
        ld px = 0, py = 0;
        ld cr = 0;
        int f = 1;
        for (int i = 0; i < n; ++i) {
            ld currx = p[i].first - cx, curry = p[i].second - cy;
            ld currcr = cross(currx, curry, px, py);
            if (i > 1 and (currcr >= 0) != (cr >= 0)) {
                f = 0;
                break;
            }
            cr = currcr, px = currx, py = curry;
        }
        if (f) {
            cout << r * r / 2 << '\n';
            continue;
        ...
        for (int i = 0; i < n; ++i) {
            ...
            if (x1 == x2) {
                ...
                ld hk = -1 / k;
                ld hb = cy - hk * cx;
                x = clamp<ld>((b - hb) / (hk - k), x1, x2);
                y = k * x + b;
            }
            chmin(res, dis(x, y, cx, cy));
        }
        cout << res + r * r / 2 << '\n';
    }
}
