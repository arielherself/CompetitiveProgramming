...
void solve() {
    ...
    for (int i = 0; i < n; ++i) {
        read(int, t);
        a.emplace_back(t, msp(t));
    }
    vector<int> cnt(30);
    vector<vector<array<int, 3>>> trie(30, vector<array<int, 3>>(1));
    auto insert = [&] (int trie_idx, int num, int i) {
        int curr = 0;
        for (int bi = trie_idx; ~bi; --bi) {
            int bit = (num >> bi) & 1;
            if (!trie[trie_idx][curr][bit]) {
                trie[trie_idx][curr][bit] = ++cnt[trie_idx];
                trie[trie_idx].push_back({});
            }
            curr = trie[trie_idx][curr][bit];
        }
        trie[trie_idx][curr][2] = i;
    };

    vector<int> slot(n);
    iota(slot.begin(), slot.end(), 0);
    sort(slot.begin(), slot.end(), [&](int i, int j){return a[i].second < a[j].second;});
    sort(a.begin(), a.end(), [](auto&& x, auto&& y){return x.second < y.second;});
    int f_fact = 0, f_res = -1, f_this = 0, f_other = 0;
    for (int j = 0; j < 30; ++j) {
        insert(j, a[0].first, 0);
    }
    for (int i = 1; i < n; ++i) {
        auto [x, msd] = a[i];
        int curr = 0;
        ...
        for (int d = msd; ~d; --d) {
            int bit = (x >> d) & 1;
            if (trie[msd][curr][bit]) {
                curr = trie[msd][curr][bit];
                fact |= (1 ^ bit) << d;
                res |= 1 << d;
                other |= bit << d;
            } else {
                curr = trie[msd][curr][1^bit];
                other |= (1 ^ bit) << d;
            }
        }
        int addition = ((1 << (k - 1 - msd)) - 1) << (msd + 1);
        res |= addition;
        fact |= addition;
        if (res > f_res) {
            f_res = res;
    ...
    cout << slot[f_this] + 1 << ' '<< slot[f_other] + 1 << ' '<< f_fact << endl;
}
