...
void solve() {
	...
	int m = 0;
	for (int i = n - 1; ~i; --i) {
		point unit = point(1, i);
		while (m >= 2 and unit * (stack[1] - stack[0]) >= 0) {
			stack.pop_front();
			m -= 1;
		}
		if (m > 0) {
			dp[i] = stack[0].y - i * stack[0].x;
		}
		point curr(a[i], a[i] * (dp[i] + i));
		while (m >= 2 and (stack[m - 1] - stack[m - 2]) * (curr - stack[m - 1]) >= 0) {
			stack.pop_back();
			m -= 1;
		}
	...
	ld res = 0;
	for (int i = 0; i < n; ++i) {
		chmax(res, a[i] * (dp[i] + i + 1));
	}
	
	cout << fixed << setprecision(12) << (long double)res << '\n';
	//
}
