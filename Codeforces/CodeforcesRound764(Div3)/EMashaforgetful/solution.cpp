...
void solve() {
    read(int, n, m);
    unordered_map<string, tuple<int, int, int>, small_string_hash> st;
    for (int k = 1; k <= n; ++k) {
        read(string, s);
        for (int i = 1; i <= m; ++i) {
            if (i >= 2) {
                st[s.substr(i - 2, 2)] = {i - 1, i, k};
            }
            if (i >= 3) {
                st[s.substr(i - 3, 3)] = {i - 2, i, k};
            }
        }
    }
    read(string, x);
    ...
    for (int i = 1; i <= m; ++i) {
        if (i >= 2) {
            if (st.count(x.substr(i - 2, 2)) && dp[i - 2] != -1) {
                dp[i] = i - 2;
            }
        }
        if (i >= 3) {
            if (st.count(x.substr(i - 3, 3)) && dp[i - 3] != -1) {
                dp[i] = i - 3;
            }
        }
    }
    if (dp[m] == -1) {
        ...
        int v = m;
        while (v != 0) {
            trace.push_back(st[x.substr(dp[v], v - dp[v])]);
            v = dp[v];
        }
        cout << trace.size() << '\n';
