...
void solve() {
    ...
    vector seq(n + 1, vector<vector<int>>(n));
    vector<int> sz(n + 1);
    {
        auto dfs = [&] (auto dfs, int v, int pa) -> void {
            ...
            for (int i = 1; i < (1 << m); ++i) {
                int u = msp(i);
                s[i] = sz[sons[u]] + s[i xor 1 << u];
            }
            // debug(s);
            for (int i = 0; i + sz[v] <= n; ++i) {
                vector<int> dp(1 << m);
                vector<int> nxt(1 << m, -1);
                for (int j = 1; j < (1 << m); ++j) {
                    for (int k = 0; k < m; ++k) {
                        if (j >> k & 1) {
                            int nw = dp[j xor 1 << k] + f[sons[k]][i + 1 + s[j xor 1 << k]];
                            if(nw >= dp[j]) {
                                // deb(v, i, j, k, nw, f[sons[k]][i + 1 + s[j xor 1 << k]]);
                                dp[j] = nw;
                                nxt[j] = k;
                            }
                        }
                ...
                while (curr != -1) {
                    // deb(mask, curr);
                    seq[v][i].emplace_back(sons[curr]);
                    mask ^= 1 << curr;
                    curr = nxt[mask];
                }
                reverse(seq[v][i].begin(), seq[v][i].end());
                // deb(v, i, f[v][i], seq[v][i]);
            }
        };
        dfs(dfs, 1, 0);
    }
